...
(function () {
  ...
  UC =
    useragent.toLowerCase().indexOf("ucbrowser") > -1 ||
    useragent.toLowerCase().indexOf("ubrowser") > -1;
  ...
  IE_10_AND_BELOW =
    !IE_11_AND_ABOVE && useragent.toLowerCase().indexOf("msie") > -1;
...
(function () {
  ...
  (function () {
    if (!CHROME_5_LOCAL) {
      ...
      try {
        while (
          topAxureWindow.parent &&
          topAxureWindow.parent !== topAxureWindow &&
          topAxureWindow.parent.$axure
        )
  ...
  if (CHROME_5_LOCAL) {
    document.addEventListener("DOMContentLoaded", function () {
      $("body").append(
        "<div id='axureEventReceiverDiv' style='display:none'></div>" +
      ...
      $("#axureEventReceiverDiv")[0].addEventListener(
        "axureMessageReceiverEvent",
        function () {
  ...
  var _handleRequest = function (request) {
    ...
    for (var i = 0; i < _listeners.length; i++)
      _listeners[i](request.message, request.data);
  ...
  _messageCenter.dispatchMessageRecursively = function (message, data) {
    ...
    $("iframe").each(function (index, frame) {
      ...
      try {
        if (
          frame.contentWindow.$axure &&
          frame.contentWindow.$axure.messageCenter
        ) {
          frame.contentWindow.$axure.messageCenter.dispatchMessageRecursively(
            message,
            data
          );
        }
  ...
  _messageCenter.sendCompositeEventMessage = function () {
    _messageCenter.postMessage(
      "axCompositeEventMessage",
  ...
  _messageCenter.postMessage = function (message, data) {
    if (_combineEventMessages) {
      if (
        ...
        message == "axCase" ||
        message == "axAction" ||
        message == "axEventComplete" ||
        message == "axInfo"
      ) {
        _compositeEventMessageData.push({ message: message, data: data });
        if (_compositeEventMessageData.length >= 10)
          _messageCenter.sendCompositeEventMessage();
        return;
      }
    }
