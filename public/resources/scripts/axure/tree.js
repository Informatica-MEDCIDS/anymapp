...
$axure.internal(function ($ax) {
  ...
  $ax.menu.InitializeSubmenu = function (subMenuId, cellId) {
    ...
    $("#" + cellId)
      ...
      .mouseleave(function (e) {
        ...
        if (
          e.pageX + 3 < offset.left ||
          e.pageX > offset.left + subcontwidth ||
          e.pageY + 3 < offset.top ||
          e.pageY > offset.top + subcontheight
        ) {
          $submenudiv
            .find(".sub_menu")
            .addBack()
  ...
  var _fireEventForSubmenu = function (targetId, eventName) {
    ...
    var event =
      diagramObject.interactionMap && diagramObject.interactionMap[eventName];
    if (event) {
    ...
    while (!$(current).hasClass("treeroot")) {
      ...
      for (i = 0; i < parent.childNodes.length; i++) {
        ...
        if (after && child.id && $(child).hasClass("treenode")) {
          ...
          child.style.top =
            $ax.getNumFromPx($(child).css("top")) + delta + "px";
          var ann = window.document.getElementById(elementId + "_ann");
          if (ann)
            ann.style.top = $ax.getNumFromPx($(ann).css("top")) + delta + "px";
        }
    ...
    $("#" + childContainerId)
      ...
      .each(function () {
        if ($(this).hasClass("treenode")) {
          height += $(this).height();
          var subContainer = window.document.getElementById(
            this.id + "_children"
          );
  ...
  $ax.tree.InitializeTreeNode = function (
    nodeId,
    plusminusid,
    childContainerId,
    selectText
    ...
    if (childContainer) {
      ...
      if (!isCollapsed && plusminusid != "")
        $ax.style.SetWidgetSelected(plusminusid, true);
    }
    ...
    if (plusminusid != "") {
      $jobj(plusminusid)
        .click(function () {
          ...
          if (visibleSet)
            $ax.tree.CollapseNode(nodeId, childContainerId, plusminusid);
  ...
  var _getButtonShapeId = function (id) {
    var obj = $obj(id);
    return $ax.public.fn.IsTreeNodeObject(obj.type)
