...
$axure.internal(function ($ax) {
  ...
  _geometry.tick = function (x, y, end) {
    ...
    for (var i = 0; i < regionList.length; i++) {
      ...
      for (var j = 0; j < points.length; j++) {
        ...
        var intersectInfo = linesIntersect(line, {
          p1: startSegment,
          p2: endSegment,
        });
        if (intersectInfo) {
      ...
      if (!intersects) {
        ...
        if (!_checkInside(points, line.p1)) region.callback({ outside: true });
      }
    }
  ...
  var linesIntersect = function (l1, l2) {
    ...
    var l1p1 =
      l1.p1.x < l1.p2.x || (l1.p1.x == l1.p2.x && l1.p1.y < l1.p2.y)
        ? l1.p1
        : l1.p2;
    var l1p2 =
      l1.p1.x < l1.p2.x || (l1.p1.x == l1.p2.x && l1.p1.y < l1.p2.y)
    ...
    var l2p1 =
      l2.p1.x < l2.p2.x || (l2.p1.x == l2.p2.x && l2.p1.y < l2.p2.y)
        ? l2.p1
        : l2.p2;
    var l2p2 =
      l2.p1.x < l2.p2.x || (l2.p1.x == l2.p2.x && l2.p1.y < l2.p2.y)
    ...
    if (l1Vert || l2Vert) {
      ...
      if (xVert >= x1 && xVert <= x2 && y >= y1 && y <= y2) {
        ...
        retval.exiting =
          (l2Vert == (y1 == (l2Vert ? l2.p1.y : l1.p1.y))) ==
          (x1 == (l2Vert ? l1.p1.x : l2.p1.x));
        retval.entering = !retval.exiting;

        // Calculate ties
        if (x1 == xVert) {
          ties[l2Vert ? "l1" : "l2"] =
            x1 == (l2Vert ? l1.p1.x : l2.p1.x) ? "start" : "end";
          retval.ties = ties;
        } else if (x2 == xVert) {
          ties[l2Vert ? "l1" : "l2"] =
            x2 == (l2Vert ? l1.p2.x : l2.p2.x) ? "end" : "start";
          retval.ties = ties;
        }
        if (y1 == y) {
          ties[l2Vert ? "l2" : "l1"] =
            y1 == (l2Vert ? l2.p1.y : l1.p1.y) ? "start" : "end";
          retval.ties = ties;
        } else if (y2 == y) {
          ties[l2Vert ? "l2" : "l1"] =
            y2 == (l2Vert ? l2.p2.y : l1.p2.y) ? "end" : "start";
          retval.ties = ties;
        }
  ...
  var _checkInside = function (polygon, point) {
    ...
    for (i = 0; i < polygon.length; i++) {
      ...
      if (intersectInfo.parallel) {
        ...
        if (
          intersectInfo.parallel.first.x <= point.x &&
          intersectInfo.parallel.second.x >= point.x
        )
          return true;
        continue;
      }
      ...
      if (intersectInfo.ties) {
        ...
        if (intersectInfo.ties.l2 == "end")
          polyLine = { p1: polyLine.p2, p2: polyLine.p1 };
      } else {
        // It is on both side, so you can take the larger one
        if (polyLine.p1.x > polyLine.p2.x)
          polyLine = { p1: polyLine.p2, p2: polyLine.p1 };
      }
