$axure.internal(function ($ax) {
  ...
  $ax.public.fn.vectorMidpoint = function (vector1, vector2) {
    return {
      x: (vector1.x + vector2.x) / 2.0,
      y: (vector1.y + vector2.y) / 2.0,
    };
  };

  $ax.public.fn.fourCornersToBasis = function (fourCorners) {
    return {
      widthVector: $ax.public.fn.vectorMinus(
        fourCorners.widgetTopRight,
        fourCorners.widgetTopLeft
      ),
      heightVector: $ax.public.fn.vectorMinus(
        fourCorners.widgetBottomLeft,
        fourCorners.widgetTopLeft
      ),
    };
  };
  ...
  var _getLayerChildrenDeep = ($ax.public.fn.getLayerChildrenDeep = function (
    layerId,
    includeLayers,
    includeHidden
  ) {
    ...
    for (var index = 0; index < children.length; index++) {
      ...
      if ($ax.public.fn.IsLayer($obj(childId).type)) {
        ...
        var recursiveChildren = _getLayerChildrenDeep(
          childId,
          includeLayers,
          includeHidden
        );
        for (var j = 0; j < recursiveChildren.length; j++)
          deep.push(recursiveChildren[j]);
  ...
  var _getPointAfterRotate = ($ax.public.fn.getPointAfterRotate = function (
    angleInDegrees,
    pointToRotate,
    centerPoint
  ) {
  ...
  var _isCompoundVectorHtml = ($ax.public.fn.isCompoundVectorHtml = function (
    hElement
  ) {
    return (
      hElement.hasAttribute("compoundmode") &&
      hElement.getAttribute("compoundmode") == "true"
    );
  });
  ...
  $ax.public.fn.convertToSingleImage = function (jobj) {
    ...
    for (var i = 0; i < components.length; i++) {
      var componentJobj = $jobj(
        $ax.public.fn.getComponentId(widgetId, components[i])
      );
  ...
  $ax.public.fn.getContainerDimensions = function (query) {
    ...
    for (var i = 0; i < query[0].children.length; i++) {
      ...
      if (
        node.id.indexOf(query[0].id) >= 0 &&
        node.id.indexOf("container") >= 0
      ) {
  ...
  $ax.public.fn.getCornersFromComponent = function (id) {
    ...
    var matrix = element
      ? $ax.public.fn.transformFromElement(element)
    ...
    return {
      ...
      relativeTopRight: $ax.public.fn.matrixMultiply(
        currentMatrix,
        preTransformTopRight
      ),
      relativeBottomRight: $ax.public.fn.matrixMultiply(
        currentMatrix,
        preTransformBottomRight
      ),
      centerPoint: {
        x: dimensions.left + halfWidth,
        y: dimensions.top + halfHeight,
      },
  ...
  $ax.public.fn.inversePathLengthFunction = function (pathFunction) {
    ...
    var makeDivisionNode = function (node1, node2) {
      ...
      var lowerDisplacement = $ax.public.fn.vectorMinus(
        node1.Position,
        inBetweenNode.Position
      );
      ...
      var higherDisplacement = $ax.public.fn.vectorMinus(
        node2.Position,
        inBetweenNode.Position
      );
    ...
    var getIntervalFromPathLength = function (node, length) {
      if (length < node.Cumulative) {
        return node.LowerChild
          ? getIntervalFromPathLength(node.LowerChild, length)
          : node.LowerInterval;
      } else
        return node.HigherChild
          ? getIntervalFromPathLength(node.HigherChild, length)
          : node.HigherInterval;
    };
    ...
    var getParameterFromPathLength = function (node, length) {
      ...
      return (
        lowerNode.Param +
        ((higherNode.Param - lowerNode.Param) *
          (length - lowerNode.Cumulative)) /
          (higherNode.Cumulative - lowerNode.Cumulative)
      );
    };
    ...
    var insertIntoSortedList = function (longer, shorter, toInsert) {
      while (true) {
        if (!longer) {
          ...
          if (
            longer.Length >= toInsert.Length &&
            shorter.Length <= toInsert.Length
          ) {
    ...
    var currentCurveLength =
      root.LowerInterval.Length + root.HigherInterval.Length;
    var longestInterval;
    ...
    while (longestInterval.Length * 100.0 > currentCurveLength) {
      ...
      currentCurveLength +=
        newNode.LowerInterval.Length +
        newNode.HigherInterval.Length -
