...
(function () {
  ...
  var _updateWindowInfo = ($axure.updateWindowInfo = function () {
    ...
    var jWin = $(window);
    var scrollWin =
      $("#ios-safari-html").length > 0 ? $("#ios-safari-html") : jWin;
  ...
  $ax.IsImageFocusable = function (type) {
    return (
      $ax.public.fn.IsImageBox(type) ||
      $ax.public.fn.IsVector(type) ||
      $ax.public.fn.IsTreeNodeObject(type) ||
      $ax.public.fn.IsTableCell(type)
    );
  };
    ...
    if (query == "*") {
      ...
      if (firstString.charAt(0) == "@") {
        ...
        if (firstString == "label") {
          _axQueryObject.filterFunctions[0] = function (diagramObject) {
            return (
              (queryArg instanceof Array &&
                queryArg.indexOf(diagramObject.label) > 0) ||
              (queryArg instanceof RegExp &&
                queryArg.test(diagramObject.label)) ||
          ...
          _axQueryObject.filterFunctions[0] = function (
            ...
            return (
              (queryArg instanceof Array && queryArg.indexOf(elementId) > 0) ||
              elementId == queryArg
            );
          };
        }
  ...
  $ax.public.fn.filter = function (query, queryArg) {
    ...
    for (var i = 0; i < this.query.filterFunctions.length; i++)
      returnVal.query.filterFunctions[i + offset] =
        this.query.filterFunctions[i];
  ...
  $ax.public.fn.each = function (fn) {
    ...
    var elementIds = this.query.elementId
  ...
  $ax.public.fn.getParents = function (deep, filter) {
    if (filter == "*")
      filter = ["layer", "rdo", "repeater", "item", "dynamicPanel", "state"];
    ...
    var getParent = function (elementId) {
      ...
      if (containerIndex !== -1)
        elementId = elementId.substring(0, containerIndex);
      if (elementId.indexOf("_text") !== -1)
        elementId = $ax.GetShapeIdFromText(elementId);
      ...
      if (parentRepeater == scriptId) {
        ...
        if (itemNum)
          return filter.indexOf("repeater") != -1
            ? scriptId
            : getParent(scriptId);
      ...
      var masterPath = $ax.getPathFromScriptId(
        $ax.repeater.getScriptIdFromElementId(elementId)
      );
      masterPath.pop();
      if (masterPath.length > 0) {
        var masterId = $ax.getElementIdFromPath(
          masterPath,
          { itemNum: itemNum },
          true
        );
        if (!masterId) return undefined;
        var masterRepeater = $ax.getParentRepeaterFromElementId(
          $ax.repeater.getScriptIdFromElementId(masterId)
        );
      ...
      if (parentDynamicPanel) {
        ...
        panelId = $ax.getElementIdFromPath(
          panelPath,
          { itemNum: itemNum },
          true
        );
      ...
      return !parent || filter.indexOf(parentType) != -1
        ? parent
        : getParent(parent);
    };
  ...
  $ax.public.fn.getChildren = function (deep, ignoreUnplaced) {
    // ignoreUnplaced should probably be the default, but when that is done a full audit of usages should be done
    var elementIds = this.getElementIds();
    var children = [];
    ...
    var getChildren = function (elementId) {
      ...
      if (
        isRepeater &&
        $ax.repeater.getScriptIdFromElementId(elementId) != elementId
      ) {
      ...
      if (isRepeater) {
        ...
        for (var itemIndex = 0; itemIndex < itemIds.length; itemIndex++)
          parent = parent.add(
            $jobj($ax.repeater.createElementId(elementId, itemIds[itemIndex]))
          );
        ...
        while ($(parent[0]).attr("id").indexOf("container") != -1)
          parent = parent.children();
        // Now at states, but want states content
        parent = parent.children();
      } else if (isTreeNode)
        parent = $jobj(
          $ax.repeater.applySuffixToElementId(elementId, "_children")
        );

      // Menu doesn't want all children, only tables and menus, so it must be handled specially
      var children = isMenu
        ? parent.children(".ax_table").add(parent.children(".ax_menu"))
        : parent.children();
      ...
      if (isTreeNode) {
        ...
        for (
          var treeNodeIndex = 0;
          treeNodeIndex < treeNodeChildren.length;
          treeNodeIndex++
        ) {
          var treeNodeChild = $(treeNodeChildren[treeNodeIndex]);
          var childObj = $obj(treeNodeChild.attr("id"));
          if (childObj && $ax.public.fn.IsVector(childObj.type))
            children = children.add(treeNodeChild);
        }
      }
      ...
      for (var childIndex = 0; childIndex < children.length; childIndex++) {
        ...
        if (typeof id == "undefined" && childObj.is("a"))
          id = $(childObj.children()[0]).attr("id");
    ...
    for (var i = 0; i < elementIds.length; i++) {
      ...
      if (elementId.indexOf("_state") > -1 && elementId.indexOf("_content") < 0)
        elementId = elementId + "_content";
      children[children.length] = {
        id: elementId,
        children: getChildren(elementId),
      };
    }
    return children;
  };
