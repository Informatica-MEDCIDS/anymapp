...
$axure.internal(function ($ax) {
  ...
  $ax.recording.recordEvent = function (
    element,
    eventInfo,
    axEventObject,
    timeStamp
  ...
  $ax.messageCenter.addMessageListener(function (message, data) {
    if (message === "startRecording") {
      ...
      $ax.recording.maybeRecordEvent = function (
        element,
        eventInfo,
        axEventObject,
        timeStamp
      ...
      var dObj =
        data.element === ""
          ? $ax.pageData.page
          : $ax.getObjectFromElementId(data.element);
      if (!data.axEventObject) {
        data.axEventObject =
          dObj && dObj.interactionMap && dObj.interactionMap[eventType];
      }
      ...
      if (
        data.eventInfo.inputType &&
        data.eventInfo.inputType === "ax_checkbox"
      ) {
      ...
      if (
        data.eventInfo.inputType &&
        data.eventInfo.inputType === "ax_text_field"
      ) {
        inputElement = $jobj(data.element + "_input");
