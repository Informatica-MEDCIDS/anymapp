...
$axure.internal(function ($ax) {
  ...
  _eventNames.mouseDownName = _supports.touchstart
    ? "touchstart"
    : _supports.pointerdown
    ? "pointerdown"
    : "mousedown";
  _eventNames.mouseUpName = _supports.touchend
    ? "touchend"
    : _supports.pointerup
    ? "pointerup"
    : "mouseup";
  _eventNames.mouseMoveName = _supports.touchmove
    ? "touchmove"
    : _supports.pointermove
    ? "pointermove"
    : "mousemove";
});
...
$axure.internal(function ($ax) {
  ...
  $axure.messageCenter.addStateListener("isTracing", function (key, value) {
    ...
    if (isConsoleTracing) {
      for (var i = 0; i < savedMessages.length; i++) {
        $ax.messageCenter.postMessage(
          savedMessages[i]["message"],
          savedMessages[i]["data"]
        );
      }
    }
  ...
  for (var i = 0; i < EVENT_NAMES.length; i++) {
    ...
    _event[eventName] = (function (event_Name) {
      return function (elementId, fn) {
        ...
        if (elementIdQuery.is("a"))
          _attachCustomObjectEvent(elementId, event_Name, fn);
        //see notes below
        else if ($ax.IsTreeNodeObject(type))
          _attachTreeNodeEvent(elementId, event_Name, fn);
        else if (
          $ax.IsImageFocusable(type) &&
          (event_Name == "focus" || event_Name == "blur")
        ) {
          var suitableChild;
          ...
          for (var j = 0; j < elementIdQuery[0].children.length; j++) {
            if (elementIdQuery[0].children[j].id == imgChild)
              suitableChild = imgChild;
            if (!suitableChild && elementIdQuery[0].children[j].id == divChild)
              suitableChild = divChild;
          }
          if (!suitableChild) suitableChild = imgChild;
          _attachDefaultObjectEvent(
            $jobj(suitableChild),
            elementId,
            event_Name,
            fn
          ...
          var id =
            isInput && (event_Name == "focus" || event_Name == "blur")
              ? inputId
              : elementId;
  ...
  AXURE_TO_JQUERY_EVENT_NAMES.onMouseDown =
      ...
      _event[jQueryEventName](
        ...
        (function (axEventObject) {
          return function (e) {
            ...
            var eventInfo = $ax.getEventInfoFromEvent(
              $ax.getjBrowserEvent(),
              false,
              elementId
            );
  ...
  var _createProxies = function (diagramObject, elementId) {
    ...
    if (createFocus)
      diagramObject.interactionMap.onFocus = {
        proxy: true,
        eventType: "OnFocus",
      };
    if (createLostFocus)
      diagramObject.interactionMap.onLostFocus = {
        proxy: true,
  ...
  var _setCanClick = function (canClick) {
    _canClick = canClick;
    if (_canClick)
      _startScroll = [$(window).scrollLeft(), $(window).scrollTop()];
  };
  ...
  var _getCanClick = function () {
    ...
    return (
      _canClick &&
      _startScroll[0] == endScroll[0] &&
      _startScroll[1] == endScroll[1]
    );
  };
  ...
  var _notAllowedInvisible = function (id) {
    ...
    if (
      $ax.public.fn.IsReferenceDiagramObject(type) ||
      $ax.public.fn.IsLayer(type)
    )
      return false;
  ...
  var _isSuppressedEvent = function (id, event) {
    ...
    return (
      (suppressedEvents && suppressedEvents.indexOf(event) != -1) ||
      (blockedEvents && blockedEvents[event])
    );
  };
  ...
  var _removeSuppressedEvent = function (id, event) {
    ...
    if (events.length == 1) {
      ...
      for (var i = eventIndex + 1; i < events.length; i++)
        events[i - 1] = events[i];
      events.pop();
    }
  };
  ...
  var _handleEvent = ($ax.event.handleEvent = function (
    elementId,
    eventInfo,
    axEventObject,
    skipShowDescriptions,
    synthetic
  ) {
    ...
    if (axEventObject.proxy) {
      ...
      if (firingId) {
        ...
        var nextEventObj =
          firingObj.interactionMap &&
          firingObj.interactionMap[_eventTypeToKey[eventType]];
    ...
    if (
      !synthetic &&
      !_getCanClick() &&
      (eventType == "OnClick" || eventType == "OnPageClick")
    )
      return;
    ...
    if (
      ...
      currentEvent.originalEvent.handled &&
    ...
    if (
      !synthetic &&
      elementId &&
      !$ax.style.getObjVisible(elementId) &&
      _notAllowedInvisible(elementId)
    )
      return;
    ...
    if (!skipShowDescriptions || eventType == "OnPageLoad")
      postMessage("axEvent", {
        label: axObjLabel,
        type: axObjType,
        event: axEventObject,
        elementId: $ax.repeater.getScriptIdFromElementId(elementId),
      });

    var bubble = true;
    var showCaseDescriptions =
      !skipShowDescriptions && _shouldShowCaseDescriptions(axEventObject);
    if (!showCaseDescriptions) {
      ...
      for (var i = 0; i < caseGroups.length; i++) {
        ...
        for (var key in guaranteedRefreshes) {
          if (guaranteedRefreshes[key] == i)
            groupRefreshes[groupRefreshes.length] = key;
        }
        bubble =
          _handleCaseGroup(eventInfo, caseGroups[i], groupRefreshes) && bubble;
      }
    } else {
    ...
    while ($ax.action.repeatersToRefresh.length)
      $ax.action.repeatersToRefresh.pop();
    for (i = 0; i < repeaters.length; i++)
      $ax.repeater.refreshRepeater(repeaters[i], eventInfo);

    if (currentEvent && currentEvent.originalEvent) {
      currentEvent.originalEvent.handled =
        !synthetic && !bubble && allowBubble.indexOf(eventType) == -1;
      //currentEvent.originalEvent.donotdrag = currentEvent.donotdrag || (!bubble && eventDescription == 'OnMouseDown');

      // Prevent default if necessary
      if (
        currentEvent.originalEvent.handled &&
        preventDefaultEvents.indexOf(eventType) != -1
      ) {
  ...
  var _handleScrollEvent = function (
    elementId,
    eventInfo,
    originalEvent,
    scrolledUp,
    ...
    if (interactionMap.onScroll)
      _handleEvent(
        elementId,
        eventInfo,
        interactionMap.onScroll,
        skipShowDescription,
        synthetic
      );
  ...
  var _showCaseDescriptions = function (
    elementId,
    eventInfo,
    axEventObject,
    synthetic
    ...
    if (!_isEventSimulating(axEventObject)) {
      ...
      for (var i = 0; i < axEventObject.cases.length; i++) {
        var $link = $(
          "<div class='intcaselink'>" +
            axEventObject.cases[i].description +
            "</div>"
        );
        $link.click(
          (function (j) {
            return function () {
              ...
              for (var k = 0; k < currentCase.actions.length; k++) {
                ...
                for (var target in currentAction.actionInfoDescriptions) {
                  var infoDescriptions =
                    currentAction.actionInfoDescriptions[target];
              ...
              var bubble = $ax.action.dispatchAction(
                copy,
                axEventObject.cases[j].actions
              );
      ...
      for (var i = 0; i < axEventObject.cases.length; i++) {
        ...
        fullDescription +=
          "&nbsp;&nbsp;" +
          currentCase.description.replace(/<br>/g, "<br>&nbsp;&nbsp;") +
          ":<br>";
        for (var j = 0; j < currentCase.actions.length; j++) {
          fullDescription +=
            "&nbsp;&nbsp;&nbsp;&nbsp;" +
            currentCase.actions[j].description.replace(
              /<br>/g,
              "<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"
            ) +
            "<br>";
      ...
      fullDescription = fullDescription.substring(
        0,
        fullDescription.length - 4
      );
  ...
  var _shouldShowCaseDescriptions = function (axEventObject) {
    if ($ax.document.configuration.linkStyle == "alwaysDisplayTargets")
      return true;
    if ($ax.document.configuration.linkStyle == "neverDisplayTargets")
      return false;
  ...
  var _handleCaseGroup = function (eventInfo, caseGroup, groupRefreshes) {
    for (var i = 0; i < caseGroup.length; i++) {
      ...
      if (
        ...
        _processCondition(currentCase.condition, eventInfo)
      ) {
        postMessage("axCase", {
          item: currentCase.description,
          description: currentCase.conditionString,
  ...
  var _attachDefaultObjectEvent = function (
    elementIdQuery,
    elementId,
    eventName,
    fn
    ...
    var func = function (e) {
      ...
      if (inputIndex == -1) {
        if (
          $ax.style.IsWidgetDisabled(elementId) ||
          _shouldIgnoreLabelClickFromCheckboxOrRadioButton(e)
        )
          return true;
      } else {
        if (
          $ax.style.IsWidgetDisabled(elementId.substring(0, inputIndex)) ||
          _shouldIgnoreLabelClickFromCheckboxOrRadioButton(e)
        )
          return false;
      }
  ...
  var _shouldIgnoreLabelClickFromCheckboxOrRadioButton = function (e) {
    return (
      ((_hasParentWithMatchingSelector(e.target, ".checkbox") &&
        $(e.target).closest("label").length != 0) ||
        (_hasParentWithMatchingSelector(e.target, ".radio_button") &&
          $(e.target).closest("label").length != 0)) &&
      e.type == "click"
    );
  };
  ...
  var _fireObjectEvent = function (elementId, event, originalArgs) {
    ...
    var handlerList =
      _objectIdToEventHandlers[elementId] &&
      _objectIdToEventHandlers[elementId][event];
    if (handlerList) {
      for (var i = 0; i < handlerList.length; i++)
        handlerList[i].apply(element, originalArgs);
    }
  ...
  _event.layerMapFocus = function (layer, elementId) {
    ...
    var mainId = $ax.getElementIdFromPath([mainObj.id], {
  ...
  var _initializeObjectEvents = function (query, refreshType) {
    ...
    query.each(function (dObj, elementId) {
      ...
      // Set up focus
      if (
        $ax.public.fn.IsTextArea(dObj.type) ||
        $ax.public.fn.IsTextBox(dObj.type) ||
        $ax.public.fn.IsCheckBox(dObj.type) ||
        $ax.public.fn.IsRadioButton(dObj.type) ||
        $ax.public.fn.IsListBox(dObj.type) ||
        $ax.public.fn.IsComboBox(dObj.type) ||
        $ax.public.fn.IsButton(dObj.type) ||
        (dObj.tabbable &&
          ($ax.public.fn.IsImageBox(dObj.type) ||
            $ax.public.fn.IsVector(dObj.type) ||
            $ax.IsTreeNodeObject(dObj.type) ||
            $ax.public.fn.IsTableCell(dObj.type)))
      ) {
      ...
      if (!itemId || itemReset) {
        ...
        if (
          ...
          $ax.public.fn.IsImageBox(dObj.type) ||
          isDynamicPanel ||
          $ax.public.fn.IsLayer(dObj.type) ||
          $ax.public.fn.IsTextBox(dObj.type) ||
          $ax.public.fn.IsTextArea(dObj.type) ||
          $ax.public.fn.IsComboBox(dObj.type) ||
          $ax.public.fn.IsListBox(dObj.type) ||
          ...
          if (dObj.selected && !skipSelectedIds.has(elementId)) {
            ...
            if (group)
              for (var item of $("[selectiongroup='" + group + "']"))
                skipSelectedIds.add(item.id);
      ...
      var hasPlaceholder =
        dObj.placeholderText == "" ? true : Boolean(dObj.placeholderText);
      if (isInput && hasPlaceholder) {
        ...
        inputJobj.on("input", function () {
          if (!dObj.HideHintOnFocused) {
            ...
            if ($ax.placeholderManager.isActive(inputId)) {
              ...
              $ax.placeholderManager.updatePlaceholder(
                inputId,
                false,
                emptyInputValue && validInput
              );
        ...
        $ax.placeholderManager.registerPlaceholder(
          elementId,
          dObj.placeholderText,
          inputJobj.attr("type") == "password"
        );
        ...
        // More info - RP-2077
        if (
          !$ax.placeholderManager.isActive(elementId) &&
          inputJobj.val() === dObj.placeholderText
        ) {
          inputJobj.val("");
        }
        ...
        $element
          .keyup(function (e) {
            if (e.keyCode == "13") {
              ...
              if (
                itemNum &&
                $ax.getParentRepeaterFromScriptId(submitId) ==
                  $ax.getParentRepeaterFromScriptId(scriptId)
              ) {
      ...
      if (
        ...
        !(dObj.interactionMap && dObj.interactionMap.onClick)
      ) {
      ...
      if ($ax.public.fn.IsTreeNodeObject(dObj.type)) {
        ...
        if (children.length > 0) {
          var plusMinusId =
            "u" +
            (parseInt(
              $ax.repeater.getScriptIdFromElementId(elementId).substring(1)
            ) +
              1);
          if (itemId)
            plusMinusId = $ax.repeater.createElementId(plusMinusId, itemId);
          if (!$jobj(plusMinusId).children().first().is("img"))
            plusMinusId = "";
      ...
      if ($ax.public.fn.IsMenuObject(dObj.type)) {
        if ($element.hasClass("sub_menu")) {
          var tableCellElementId = $ax.getElementIdFromPath(
            [dObj.parentCellId],
            { relativeTo: elementId }
          );
      ...
      if (
        (isDynamicPanel || $ax.public.fn.IsLayer(dObj.type) || isItem) &&
        dObj.propagate
      ) {
        $element
          .mouseenter(function () {
      ...
      if (
        ...
        if (isDynamicPanel) {
          ...
          for (var i = 0; i < diagrams.length; i++) {
            var panelId = $ax.repeater.applySuffixToElementId(
              elementId,
              "_state" + i
            );
      ...
      if (
        isDynamicPanel &&
        map &&
        (map.onScroll || map.onScrollUp || map.onScrollDown)
      ) {
        var diagrams = dObj.diagrams;
        for (var i = 0; i < diagrams.length; i++) {
          var panelId = $ax.repeater.applySuffixToElementId(
          ...
          (function (id) {
            if ($("#" + id).data("lastScrollTop") == undefined)
              $("#" + id).data("lastScrollTop", "0");
            _attachDefaultObjectEvent(
              $("#" + id),
              elementId,
              "scroll",
              function (e) {
                $ax.setjBrowserEvent(e);
                var currentEvent = $ax.getjBrowserEvent();
                var eventInfoFromEvent = $ax.getEventInfoFromEvent(
                  currentEvent,
                  false,
                  elementId
                );
      ...
      if (map && map.onMouseHover) {
        ...
        $element
          .mouseenter(function (e) {
            $ax.setjBrowserEvent(e);
            hoverEventInfo = $ax.getEventInfoFromEvent(
              $ax.getjBrowserEvent(),
              false,
              elementId
            );
      ...
      if (map && map.onLongClick) {
        ...
        $element
          .bind($ax.features.eventNames.mouseDownName, function (e) {
            (function (currCount) {
              $ax.setjBrowserEvent(e);
              var eventInfo = $ax.getEventInfoFromEvent(
                $ax.getjBrowserEvent(),
              ...
              window.setTimeout(function () {
                if (currCount == longClickMouseCount)
                  _raiseSyntheticEvent(
                    elementId,
                    "onLongClick",
                    false,
      ...
      if (
        ...
        if ($ax.public.fn.IsRadioButton(dObj.type)) {
          ...
          onClick = function (e) {
            $ax.style.SetWidgetSelected(
              elementId,
              !$ax.style.IsWidgetSelected(elementId),
              true
            );
      ...
      if (dObj.type == "hyperlink") {
        $element
          .mouseenter(function () {
            ...
            if (
              mouseOverObjectId &&
              $ax.style.IsWidgetDisabled(mouseOverObjectId)
            )
              return true;
          ...
          .mouseleave(function () {
            ...
            $ax.splice(
              _event.mouseOverIds,
              _event.mouseOverIds.indexOf(linkId),
              1
            );
            var mouseOverObjectId = _event.mouseOverObjectId;
            if (
          ...
          .bind($ax.features.eventNames.mouseDownName, function () {
            ...
            if (mouseOverObjectId)
              $ax.style.SetWidgetMouseDown(mouseOverObjectId, true);
          ...
          .bind($ax.features.eventNames.mouseUpName, function () {
            ...
            if (mouseOverObjectId)
              $ax.style.SetWidgetMouseDown(mouseOverObjectId, false);
      ...
      if (dObj.type == "inlineFrame") {
        ...
        if (target.includeVariables && target.url) {
          ...
          url =
            origSrc.toLowerCase().indexOf("http://") == -1
              ? $ax.globalVariableProvider.getLinkUrl(origSrc)
              : origSrc;
        } else if (target.urlLiteral) {
          url = $ax.expr.evaluateExpr(
            target.urlLiteral,
            $ax.getEventInfoFromEvent(undefined, true, elementId),
            true
          );
        }
  ...
  $ax.event.updateIxStyleEvents = function (elementId) {
    ...
    _attachIxStyleEvents(
      $ax.getObjectFromElementId(elementId),
      elementId,
      $jobj(elementId),
      true
    );
  ...
  var _attachIxStyleEvents = function (
    ...
    var isDynamicPanel = $ax.public.fn.IsDynamicPanel(dObj.type);
    var needsMouseFilter =
      (ignoreHasIxStyles || $ax.event.HasIxStyles(dObj)) &&
      dObj.type != "hyperlink" &&
      !$ax.public.fn.IsLayer(dObj.type) &&
      !isDynamicPanel &&
      dObj.type != $ax.constants.TEXT_TYPE &&
      !$ax.public.fn.IsRepeater(dObj.type) && //&& !$ax.public.fn.IsCheckBox(dObj.type) && !$ax.public.fn.IsRadioButton(dObj.type)
      !$ax.public.fn.IsTreeNodeObject(dObj.type);
    if (needsMouseFilter) {
      //$element.mouseenter(function () {
      $element
        .on("mouseenter.ixStyle", function () {
          var elementId = this.id;
          var parent = $ax.dynamicPanelManager.parentHandlesStyles(elementId);
        ...
        .on("mouseleave.ixStyle", function () {
          ...
          $ax.splice(
            _event.mouseOverIds,
            $.inArray(elementId, _event.mouseOverIds),
            1
          );
      ...
      //$element.bind($ax.features.eventNames.mouseDownName, function () {
      $element
        .on($ax.features.eventNames.mouseDownName + ".ixStyle", function () {
          var elementId = this.id;
          var parent = $ax.dynamicPanelManager.parentHandlesStyles(elementId);
          if (parent) {
  ...
  (function () {
    ...
    $ax.event.initKeyEvents = function ($query) {
      $query.keydown(function (e) {
        ...
        if (modifierCodes.indexOf(e.keyCode) == -1)
          _keyState.keyCode = e.keyCode;
      ...
      $tapQuery
        .bind("touchstart", function (e) {
          ...
          if (time - lastClickEventTime < DBLCLICK_THRESHOLD_MS) {
            ...
            var axEventObject =
              dObj &&
              dObj.interactionMap &&
              dObj.interactionMap["onDoubleClick"];
        ...
        .bind("touchend", function (e) {
          var touch =
            ...
            e.originalEvent.changedTouches &&
            e.originalEvent.changedTouches[0];
          if (!touch || !tapDownLoc || $ax.style.IsWidgetDisabled(elementId))
            return;
          ...
          if (xDiff * xDiff + yDiff * yDiff < CLICK_THRESHOLD_PX_SQ) {
            ...
            if (time - lastClickEventTime < DBLCLICK_THRESHOLD_MS) {
              ...
              if (e.originalEvent && e.originalEvent.handled)
  ...
  var _raiseSyntheticEvent = function (
    elementId,
    eventName,
    skipShowDescription,
    eventInfo,
    ...
    if (
      $ax.style.IsWidgetDisabled(elementId) &&
      _shouldStopOnDisabledWidget(eventName)
    )
      return;
    ...
    var dObj =
      elementId === ""
        ...
        : $ax.getObjectFromElementId(elementId);
    var axEventObject =
      dObj && dObj.interactionMap && dObj.interactionMap[eventName];
    if (!axEventObject) return;

    eventInfo =
      eventInfo ||
      $ax.getEventInfoFromEvent(
        $ax.getjBrowserEvent(),
        skipShowDescription,
        elementId
      );
  ...
  var _initialize = function () {
    ...
    _initializeObjectEvents(
      $ax(function (obj, elementId) {
  ...
  $ax.event.HasTextChanged = function (diagramObject) {
    if (
      !$ax.public.fn.IsTextBox(diagramObject.type) &&
      !$ax.public.fn.IsTextArea(diagramObject.type)
    )
      return false;
  ...
  $ax.event.HasSelectionChanged = function (diagramObject) {
    if (
      !$ax.public.fn.IsListBox(diagramObject.type) &&
      !$ax.public.fn.IsComboBox(diagramObject.type)
    )
      return false;
    var map = diagramObject.interactionMap;
  ...
  $ax.event.HasCheckedChanged = function (diagramObject) {
    if (
      !$ax.public.fn.IsCheckBox(diagramObject.type) &&
      !$ax.public.fn.IsRadioButton(diagramObject.type)
    )
      return false;
    var map = diagramObject.interactionMap;
  ...
  var _postAdaptiveViewChanged = function (forceSwitchTo) {
    ...
    try {
      if (
        ...
        (!CHROME_5_LOCAL &&
          window.parent.$ &&
          window.parent.$("#mainFrame").length > 0)
      ) {
        var data = {
  ...
  var _postResize = ($ax.postResize = function (e) {
    $ax.setjBrowserEvent(e);
    return fireEventThroughContainers(
      "onResize",
      undefined,
      false,
      [
  ...
  var _fireEventsForTableMenuAndTree = function (
    object,
    event,
    skipShowDescription,
    eventInfo,
    ...
    if (object.objects) {
      for (var index = 0; index < object.objects.length; index++) {
        ...
        if ($ax.public.fn.IsTableCell(subObj.type)) {
          ...
          $ax.public.fn.IsTable(object.type) ||
          $ax.public.fn.IsMenuObject(object.type) ||
          $ax.public.fn.IsTreeNodeObject(object.type)
        ) {
          _fireEventsForTableMenuAndTree(
            subObj,
            event,
            skipShowDescription,
            eventInfo,
  ...
  var fireEventForPageOrMaster = function (
    elementId,
    eventName,
    interactionMap,
    isPage,
    ...
    if (scrolling && !axEvent)
      axEvent = interactionMap.onScrollUp || interactionMap.onScrollDown;
    ...
    if (axEvent) {
      ...
      if (scrolling)
        _handleScrollEvent(
          ...
          eventInfo,
          currentEvent.originalEvent,
          _event.windowScrollingUp,
          _event.windowScrollingDown,
          interactionMap,
          skipShowDescription,
          synthetic
        );
  ...
  var _callFilterCheck = function (callFilter, type) {
    for (var index = 0; index < callFilter.length; index++) {
      ...
      if (currentType === $ax.constants.ALL_TYPE || currentType === type)
        return true;
    }
    return false;
  };

  var fireEventThroughContainers = function (
    eventName,
    objects,
    synthetic,
    searchFilter,
      ...
      if (_callFilterCheck(callFilter, $ax.constants.PAGE_TYPE)) {
        ...
        if (eventName === "onScroll") {
          var currentScrollTop =
            (SAFARI && IOS) || SHARE_APP
              ? $("#ios-safari-html").scrollTop()
              : $(window).scrollTop();
          _event.windowScrollingUp = currentScrollTop < lastScrollTop;
          _event.windowScrollingDown = currentScrollTop > lastScrollTop;
        }

        fireEventForPageOrMaster(
          "",
          eventName,
          $ax.pageData.page.interactionMap,
          true,
          skipShowDescription,
          synthetic
        );
      }
      if (searchFilter.indexOf($ax.constants.PAGE_TYPE) != -1)
        fireEventThroughContainers(
          eventName,
          $ax.pageData.page.diagram.objects,
          synthetic,
          searchFilter,
          callFilter
        );
    ...
    for (var i = 0; i < objects.length; i++) {
      ...
      if (
        ...
        if (_callFilterCheck(callFilter)) {
          //fire current event for all types
          if (
            $ax.public.fn.IsTable(obj.type) ||
            $ax.public.fn.IsMenuObject(obj.type) ||
            $ax.public.fn.IsTreeNodeObject(obj.type)
          ) {
            _fireEventsForTableMenuAndTree(
              obj,
              eventName,
            ...
            var scriptId = $ax.getScriptIdFromPath(pathCopy);
            if (scriptId && itemId)
              scriptId = $ax.repeater.createElementId(scriptId, itemId);
            $ax.event.raiseSyntheticEvent(
              scriptId,
              eventName,
              skipShowDescription,
      ...
      if ($ax.public.fn.IsReferenceDiagramObject(obj.type)) {
        if (
          ...
          fireEventForPageOrMaster(
            objId,
            eventName,
            $ax.pageData.masters[obj.masterId].interactionMap,
            false,
            skipShowDescription,
            synthetic
          );
        ...
        if (
          searchFilter.indexOf($ax.constants.REFERENCE_DIAGRAM_OBJECT_TYPE) !=
          -1
        )
          fireEventThroughContainers(
            eventName,
            $ax.pageData.masters[obj.masterId].diagram.objects,
            synthetic,
            searchFilter,
            callFilter,
            pathCopy,
        ...
        if (_callFilterCheck(callFilter, $ax.constants.DYNAMIC_PANEL_TYPE))
          $ax.event.raiseSyntheticEvent(
            objId,
            eventName,
            skipShowDescription,
        ...
        if (_callFilterCheck(callFilter, $ax.constants.REPEATER))
          $ax.event.raiseSyntheticEvent(
            objId,
            eventName,
            skipShowDescription,
        ...
        if (_callFilterCheck(callFilter, $ax.constants.LAYER_TYPE))
          $ax.event.raiseSyntheticEvent(
            objId,
            eventName,
            skipShowDescription,
