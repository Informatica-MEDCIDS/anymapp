$axure.internal(function ($ax) {
  ...
  $ax.visibility.SetVisible = function (element, visible) {
    ...
    if (visible) {
      ...
      if ($(element).hasClass(UNPLACED_CLASS))
        $(element).removeClass(UNPLACED_CLASS);
  ...
  var _setWidgetVisibility = ($ax.visibility.SetWidgetVisibility = function (
    elementId,
    options
  ) {
    ...
    if (!options.noContainer && (isPanel || isLayer)) {
      ...
      options.onComplete = function () {
        ...
        if (
          ...
          (FIREFOX || $axure.browser.isEdge || IE)
        ) {
          ...
          if (mouseOveredElement && !mouseOveredElement.is(":visible")) {
            ...
            if (
              ($ax.public.fn.IsDynamicPanel(axObj.type) ||
                $ax.public.fn.IsLayer(axObj.type) ||
                $ax.public.fn.IsRepeater(axObj.type)) &&
              axObj.propagate
            ) {
    ...
    if (options.value && !MOBILE_DEVICE && $ax.adaptive.isDeviceMode())
      _updateMobileScrollForWidgetShown(axObj);
  });
  ...
  var _setVisibility = function (parentId, childId, options, preserveScroll) {
    ...
    var size =
      options.size ||
      (options.containerExists ? $(child.children()[0]) : child);
    ...
    if (needContainer) {
      ...
      if (options.cull) {
        ...
        if (childObj && $ax.public.fn.IsLayer(childObj.type)) {
          // || childObj.generateCompound)) {
      ...
      if (options.containInner) {
        wrapped = _wrappedChildren(
          containerExists ? $(child.children()[0]) : child
        );

        // Filter for visibile wrapped children
        visibleWrapped = [];
        for (var i = 0; i < wrapped.length; i++)
          if ($ax.visibility.IsVisible(wrapped[i]))
            visibleWrapped.push(wrapped[i]);
        ...
        if (
          !containerExists &&
          (wrappedOffset.left != 0 || wrappedOffset.top != 0)
        ) {
          for (var i = 0; i < wrapped.length; i++) {
            var inner = $(wrapped[i]);
            inner.css(
              "left",
              $ax.getNumFromPx(inner.css("left")) - wrappedOffset.left
            );
            inner.css(
              "top",
              $ax.getNumFromPx(inner.css("top")) - wrappedOffset.top
            );
    ...
    var onComplete = function () {
      ...
      if (needContainer && completeCount == completeTotal) {
        ...
        if (
          ...
          window.modifiedDynamicPanleParentOverflowProp
        ) {
    ...
    if (!options.easing || options.easing == "none") {
      ...
      innerContainer.data(
        "flip",
        options.direction == "left" || options.direction == "right" ? "y" : "x"
      );
      innerContainer.css({
        position: "relative",
      ...
      var originForFlip =
        containerWidth / 2 + "px " + containerHeight / 2 + "px";
      if (options.value) {
        innerContainer.css({
        ...
        switch (options.direction) {
          case "right":
          case "left":
            _setRotateTransformation(
              innerContainer,
              _getRotateString(
                true,
                options.direction === "right",
                options.showFlipBack
              )
            );
  ...
  var _setRotateTransformation = function (
    elementsToSet,
    transformValue,
    elementParent,
    flipCompleteCallback,
    flipDurationMs,
    useAnimationFrame
  ) {
  ...
  $ax.visibility.GetPanelState = function (id) {
    ...
    for (var i = 0; i < children.length; i++) {
      if (children[i].style && $ax.visibility.IsVisible(children[i]))
  ...
  var containerCount = {};
  $ax.visibility.SetPanelState = function (
    id,
    stateId,
    easingOut,
    directionOut,
    ...
    //pin to browser
    if (isFixed)
      $ax.dynamicPanelManager.adjustFixed(
        id,
        oldState.width(),
        oldState.height(),
        state.width(),
    ...
    _bringPanelStateToFront(
      id,
      stateId,
      oldStateId,
      easingIn == "none" || durationIn == "0"
    );
    ...
    if (fitToContent) {
      ...
      $ax.visibility.setResizedSize(
        id,
        $obj(id).percentWidth ? oldWidth : width,
        height
      );
    }
    ...
    _setVisibility(id, oldStateId, {
      ...
      //cull for
      cull:
        easingOut == "none" || state.children().length == 0 ? oldState : state,
      showFlipBack: true,
    });
    ...
    if (count) containerCount[id] = count + 1;
      ...
      if (panel) {
        for (var i = 0; i < children.length; i++) {
          ...
          childContainer.attr(
            "id",
            $ax.visibility.applyWidgetContainer(child.attr("id"))
          );
        ...
        if (focus)
          $ax.event.addSuppressedEvent(
            $ax.repeater.removeSuffixFromElementId(focus),
            "OnLostFocus"
          );
      ...
      for (var i = 0; i < childIds.length; i++) {
        ...
        if ($ax.getTypeFromElementId(childId) == $ax.constants.LAYER_TYPE) {
          ...
          if (fixedInfo) {
            ...
            var deltaX =
              $ax.getNumFromPx(childObj.css("left")) - fixedInfo.left;
            var deltaY = $ax.getNumFromPx(childObj.css("top")) - fixedInfo.top;

            fixedInfo = fixedInfo.fixed;
            if (fixedInfo.horizontal == "left")
              childObj.css("left", fixedInfo.x + deltaX);
            ...
            if (fixedInfo.vertical == "top")
              childObj.css("top", fixedInfo.y + deltaY);
  ...
  var _setCurrFocus = function (id) {
    if (id) {
      ...
      var trap = $ax.event.blockEvent(
        $ax.repeater.removeSuffixFromElementId(id),
        "OnFocus"
      );
      window.setTimeout(function () {
        $jobj(id).focus();
  ...
  _visibility.getWidgetFromContainer = function (id) {
    ...
    return (
      id.substr(0, containerIndex) +
      id.substr(containerIndex + CONTAINER_SUFFIX.length)
    );
  };
  ...
  _visibility.applyWidgetContainer = function (
    id,
    returnJobj,
    skipCheck,
    checkInner
    ...
    var containerId = $ax.repeater.applySuffixToElementId(
      id,
      checkInner ? CONTAINER_INNER : CONTAINER_SUFFIX
    );
    ...
    container = $jobj(
      $ax.repeater.applySuffixToElementId(id, CONTAINER_SUFFIX)
    );
  ...
  _visibility.getRealChildren = function (query) {
    while (
      query.length &&
      $(query[0]).attr("id").indexOf(CONTAINER_SUFFIX) != -1
    )
  ...
  $ax.visibility.GetPanelStateId = function (dpId, index) {
    ...
    var panelStateId =
      $ax.repeater.getScriptIdFromElementId(dpId) + "_state" + index;
    return $ax.repeater.createElementId(panelStateId, itemNum);
  };

  $ax.visibility.GetPanelStateCount = function (id) {
    return $ax.visibility
      .getRealChildren($jobj(id).children())
      .filter("[id*='_state']").length;
  };

  var _bringPanelStateToFront = function (
    dpId,
    stateId,
    oldStateId,
    oldInFront
    ...
    if (
      (IE || FIREFOX) &&
      window.lastFocusedClickable &&
      $ax.event.getFocusableWidgetOrChildId(window.lastFocusedControl) ==
        window.lastFocusedClickable.id
    ) {
      // Only need to do this if the currently focused widget is in the panel state that is being hidden.
      if (
        $jobj(oldStateId).find(
          "#" + window.lastFocusedClickable.id.split("_")[0]
        ).length
      )
  ...
  var _addLimboAndHiddenIds = ($ax.visibility.addLimboAndHiddenIds = function (
    newLimboIds,
    newHiddenIds,
    query,
    skipRepeater
    ...
    for (var key in newLimboIds) {
      if (
        !$ax.public.fn.IsReferenceDiagramObject(
          $ax.getObjectFromElementId(key).type
        )
      )
        continue;
    ...
    query.each(function (diagramObject, elementId) {
      ...
      if (
        ...
        $ax.public.fn.IsReferenceDiagramObject(diagramObject.type) ||
        $ax.public.fn.IsTableCell(diagramObject.type) ||
        $jobj(elementId).hasClass("sub_menu")
      )
        return;
      if (
        diagramObject.type == "table" &&
        $jobj(elementId).parent().hasClass("ax_menu")
      )
        return;
      if (skipRepeater) {
      ...
      var shouldBeVisible = Boolean(
        !newLimboIds[scriptId] && !newHiddenIds[scriptId]
      );
      var isVisible = Boolean(_isIdVisible(elementId));
      if (shouldBeVisible != isVisible) {
        _setWidgetVisibility(elementId, {
  ...
  var _clearLimboAndHidden = ($ax.visibility.clearLimboAndHidden = function (
    ids
  ) {
  ...
  $ax.visibility.moveMovedLocation = function (scriptId, deltaLeft, deltaTop) {
    ...
    $ax.visibility.setMovedLocation(
      scriptId,
      offsetLocation.x + deltaLeft,
      offsetLocation.y + deltaTop
    );
  ...
  $ax.visibility.initialize = function () {
    ...
    $("." + HIDDEN_CLASS).each(function (index, diagramObject) {
      _defaultHidden[
        $ax.repeater.getScriptIdFromElementId(diagramObject.id)
      ] = true;
    });
