$axure.internal(function ($ax) {
  ...
  $ax.move.PrepareForMove = function (
    id,
    x,
    y,
    to,
    ...
    if (fixedInfo.horizontal == "right") {
      ...
      horzX = to
        ? $(window).width() -
          x -
          $ax.getNumFromPx(jobj.css("right")) -
          query.width()
        : -x;
    ...
    if (fixedInfo.vertical == "bottom") {
      ...
      vertY = to
        ? $(window).height() -
          y -
          $ax.getNumFromPx(jobj.css("bottom")) -
          query.height()
        : -y;
  ...
  $ax.move.MoveWidget = function (
    ...
    if (childrenIds) {
      for (var i = 0; i < childrenIds.length; i++)
        $ax.event.raiseSyntheticEvent(childrenIds[i], "onMove");
    }
  };
  ...
  var _moveElement = function (
    ...
    if (!$ax.dynamicPanelManager.isPercentWidthPanel($obj(id)))
      cssStyles[moveInfo.horzProp] = "+=" + moveInfo.horzX;
    ...
    if (options.easing === "none") {
      ...
      var ellipseArcFunctionY = function (param) {
        return {
          x:
            initialHorzProp +
            (1.0 - Math.cos(param * Math.PI * 0.5)) * moveInfo.horzX,
      ...
      var ellipseArcFunctionX = function (param) {
        return {
          ...
          y:
            initialVertProp +
            (1.0 - Math.cos(param * Math.PI * 0.5)) * moveInfo.vertY,
        };
      };
      var ellipseArcFunction =
        (moveInfo.horzX > 0) ^
        (moveInfo.vertY > 0) ^
        (options.trajectory === "arcClockwise")
          ? ellipseArcFunctionX
          : ellipseArcFunctionY;
      var inverseFunction =
        $ax.public.fn.inversePathLengthFunction(ellipseArcFunction);
      $(state).animate(
        { parameter: 1.0 },
        {
          duration: options.duration,
  ...
  var _noRotateOnlyMove = function (
    id,
    moveDelta,
    rotatableMove,
    fireAnimationQueue,
    ...
    if (moveDelta.x == 0 && moveDelta.y == 0) {
      ...
      $jobj(id).animate(
        { top: "+=" + moveDelta.y, left: "+=" + moveDelta.x },
        {
          duration: duration,
          easing: easing,
          queue: false,
  ...
  _move.circularMove = function (
    ...
    var stepFunc = function (newDegree) {
      ...
      var widgetNewCenter = $axure.fn.getPointAfterRotate(
        deg,
        widgetCenter,
        centerPoint
      );
      ...
      if (resizeOffset) {
        ...
        $axure.fn.getPointAfterRotate(rotation.degree, resizeShift, {
          x: 0,
          y: 0,
        });
      ...
      rotatableMove = $axure.fn.getPointAfterRotate(deg, rotatableMove, {
        x: 0,
        y: 0,
      });
    ...
    var onComplete = function () {
      ...
      var isPercentWidthPanel = $ax.dynamicPanelManager.isPercentWidthPanel(
        $obj(id)
      );
  ...
  _move.rotate = function (
    ...
    var stepFunc = function (now) {
      ...
      query.css(
        $ax.public.fn.setTransformHowever("rotate(" + newDegree + "deg)")
      );
    ...
    var onComplete = function () {
      if (shouldFire) {
        $ax.action.fireAnimationFromQueue(
          $ax.public.fn.compoundIdFromComponent(id),
          $ax.action.queueTypes.rotate
        );
      }
      if (completionCallback) completionCallback();
  ...
  _move.compoundRotateAround = function (
    id,
    degreeDelta,
    centerPoint,
    moveDelta,
    ...
    $(rotation).animate(
      { degree: degreeDelta },
      {
        ...
        step: function (newDegree) {
          ...
          var originalCenter = {
            x: originalLeft + 0.5 * originalWidth,
            y: originalTop + 0.5 * originalHeight,
          };
          var componentCenter = {
            x: originalCenter.x + transform[4],
            y: originalCenter.y + transform[5],
          };
          var deg = newDegree - rotation.degree;
          var ratio = deg / degreeDelta;
          var xdelta = (moveDelta.x + rotatableMove.x) * ratio;
          ...
          var compositionTransform = $ax.public.fn.matrixMultiplyMatrix(
            rotationMatrix,
            {
              m11: transform[0],
              m21: transform[1],
          ...
          var newMatrix = $ax.public.fn.matrixString(
            compositionTransform.m11,
            compositionTransform.m21,
            compositionTransform.m12,
            compositionTransform.m22,
            widgetNewCenter.x - originalCenter.x + xdelta,
            widgetNewCenter.y - originalCenter.y + ydelta
          );
  ...
  _move.getRotationDegreeFromElement = function (element) {
    ...
    if (window.getComputedStyle) {
      ...
      console.log(
        "rotation is not supported for ie 8 and below in this version of axure rp"
      );
      return 0;
    }
  ...
  _move.getRotationDegree = function (elementId) {
    ...
    return _move.getRotationDegreeFromElement(
