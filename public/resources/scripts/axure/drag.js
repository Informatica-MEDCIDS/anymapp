$axure.internal(function ($ax) {
  ...
  $ax.drag.StartDragWidget = function (event, id) {
    ...
    var movedownName =
      IE_10_AND_BELOW && $ax.features.supports.windowsMobile
        ? $ax.features.eventNames.mouseDownName
        : $ax.features.eventNames.mouseMoveName;
  ...
  var _dragWidget = function (event) {
    ...
    if (IE_10_AND_BELOW) {
      ...
      if (event.changedTouches) {
        ...
        if (
          $ax.event.hasSyntheticEvent(widgetDragInfo.widgetId, "onDrag") ||
          $ax.event.hasSyntheticEvent(widgetDragInfo.widgetId, "onSwipeUp") ||
          $ax.event.hasSyntheticEvent(widgetDragInfo.widgetId, "onSwipeDown") ||
          deltaX * deltaX > 25 ||
          ($ax.document.configuration.preventScroll &&
            $ax.legacy.GetScrollable(target) == window.document.body)
        ) {
          event.preventDefault();
        }
      } else {
    ...
    if (!widgetDragInfo.hasDragged) {
      ...
      if ($ax.event.hasSyntheticEvent(widgetDragInfo.widgetId, "onDrag")) {
        ...
        if (niceScrollContainer) {
          widgetDragInfo.oldNiceScrollContainerCursor =
            niceScrollContainer.style.cursor;
  ...
  var _stopDragWidget = function (event) {
    ...
    if (widgetDragInfo.hasDragged) {
      ...
      if (
        $ax.globalVariableProvider.getVariableValue("totaldragx") < -30 &&
        $ax.globalVariableProvider.getVariableValue("dragtime") < 1000
      ) {
      ...
      var totalDragY =
        $ax.globalVariableProvider.getVariableValue("totaldragy");
      if (
        totalDragY < -30 &&
        $ax.globalVariableProvider.getVariableValue("dragtime") < 1000
      ) {
  ...
  $ax.drag.GetTotalDragX = function () {
    if (widgetDragInfo.hasDragged)
      return widgetDragInfo.currentX - widgetDragInfo.cursorStartX;
    return 0;
  };

  $ax.drag.GetTotalDragY = function () {
    if (widgetDragInfo.hasDragged)
      return widgetDragInfo.currentY - widgetDragInfo.cursorStartY;
    return 0;
  };

  $ax.drag.GetDragTime = function () {
    if (widgetDragInfo.hasDragged)
      return widgetDragInfo.currentTime - widgetDragInfo.startTime;
    return 600000;
  };
  ...
  Rectangle.prototype.IntersectsWith = function (rect) {
    ...
    if (rect.length) {
      for (var i = 0; i < rect.length; i++)
        if (!rect[i].Invalid && this.IntersectsWith(rect[i])) return true;
      return false;
    }
  ...
  Rectangle.prototype.Invalid = function () {
    return (
      this.x == -1 && this.y == -1 && this.width == -1 && this.height == -1
    );
  };
