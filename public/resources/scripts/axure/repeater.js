...
$axure.internal(function ($ax) {
  ...
  var _loadRepeaters = function () {
    $ax(function (obj) {
      ...
      for (var i = 0; i < initialItemIds.length; i++)
        $ax.addItemIdToRepeater(initialItemIds[i], repeaterId);
  ...
  var _refreshRepeater = function (
    ...
    if (refreshType == _refreshType.reset)
      $ax.style.clearStateForRepeater(repeaterId);
    ...
    if (eventInfo && eventInfo.repeaterDeleteMap)
      delete eventInfo.repeaterDeleteMap[repeaterId];
    ...
    if (itemsPregen) {
      ...
      for (var i = 1; i < repeaterChildren.length; i++) {
        orderedIds.push(
          _getItemIdFromElementId($(repeaterChildren[i]).attr("id"))
        );
      }
    } else orderedIds = getOrderedIds(repeaterId, eventInfo);
    var ids = [];
    var background = _getAdaptiveProp(
      propMap,
      "backColor",
      viewId,
      repeaterId,
    ...
    var hasAltColor = _getAdaptiveProp(
      propMap,
      "hasAltColor",
      viewId,
      repeaterId,
    ...
    var altColor = hasAltColor
      ? _getAdaptiveProp(propMap, "altColor", viewId, repeaterId, obj)
      : undefined;
    var useAlt = false;
    ...
    if (itemsPregen) {
      ...
      var bounds = _getVisibleDataBounds(
        repeaterToPageInfo[repeaterId],
        itemsPregen ? obj.data.length : orderedIds.length
      );
      start = bounds[0];
      end = bounds[1];
    }
    ...
    var paddingTop = _getAdaptiveProp(
      propMap,
      "paddingTop",
      viewId,
      repeaterId,
    ...
    var paddingLeft = _getAdaptiveProp(
      propMap,
      "paddingLeft",
      viewId,
      repeaterId,
    ...
    var paddingY =
      paddingTop +
      _getAdaptiveProp(propMap, "paddingBottom", viewId, repeaterId, obj);
    var paddingX =
      paddingLeft +
      _getAdaptiveProp(propMap, "paddingRight", viewId, repeaterId, obj);

    var spacingX = _getAdaptiveProp(
      propMap,
      "horizontalSpacing",
      viewId,
      repeaterId,
    ...
    if (itemsPregen) {
      ...
      var processScriptIds = function (full, prop, id) {
        if (id.indexOf("_") <= 0 && id.indexOf("p") == -1)
          templateIds.push("u" + id);
      };
      $("#" + repeaterId + "_script")
        .html()
        .replace(
          /(id|for)="?u([0-9]+(p([0-9]){3})?(_[_a-z0-9]*)?)"?/g,
          processScriptIds
        );
      ...
      for (var pos = start; pos < end; pos++) {
        ...
        for (var i = 0; i < templateIds.length; i++) {
          ...
          if (refreshType != _refreshType.preEval || elementId == itemElementId)
            continue;
          const elJobj = $jobj(elementId);
          $ax.visibility.setResizedSize(
            elementId,
            $ax.getNumFromPx(elJobj.css("width")),
            $ax.getNumFromPx(elJobj.css("height"))
          );
          $ax.visibility.setMovedLocation(
            elementId,
            $ax.getNumFromPx(elJobj.css("left")),
            $ax.getNumFromPx(elJobj.css("top"))
          );
        }
        ...
        $ax.visibility.setResizedSize(
          itemElementId,
          $ax.getNumFromPx(jobj.css("width")),
          $ax.getNumFromPx(jobj.css("height"))
        );
        $ax.visibility.setMovedLocation(
          itemElementId,
          $ax.getNumFromPx(jobj.css("left")),
          $ax.getNumFromPx(jobj.css("top"))
        );
      }
    } else {
      ...
      div.html(html);
      div
        .find("." + $ax.visibility.HIDDEN_CLASS)
        .removeClass($ax.visibility.HIDDEN_CLASS);
      div
        .find("." + $ax.visibility.UNPLACED_CLASS)
        .removeClass($ax.visibility.UNPLACED_CLASS);
      div.css({
        width: offset.width,
      ...
      if (shown)
        document.getElementById(repeaterId).style.visibility = "hidden";
      ...
      if (repeaterObj.css("box-sizing") == "border-box") {
        ...
        startLeft -=
          $ax.getNumFromPx(repeaterObj.css("border-left-width")) || 0;
      }
      var top = startTop;
      var left = startLeft;
      for (pos = start; pos < end; pos++) {
        ...
        if (obj.repeaterPropMap.isolateRadio) {
          ...
          for (
            var radioIndex = 0;
            radioIndex < radioButtons.length;
            radioIndex++
          ) {
      ...
      for (var i = 0; i < ids.length; i++) {
        ...
        if (
          obj.repeaterPropMap.isolateSelection &&
          childJobj.attr("selectiongroup")
        ) {
          childJobj.attr(
            "selectiongroup",
            _createElementId(
              childJobj.attr("selectiongroup"),
              _getItemIdFromElementId(id)
            )
          );
        }
    ...
    if (viewId) $ax.adaptive.applyView(viewId, query);
      ...
      $ax.visibility.resetLimboAndHiddenToDefaults(
        _getItemQuery(repeaterId, preevalMap)
      );
    ...
    if (shown && !itemsPregen)
      document.getElementById(repeaterId).style.visibility = "inherit";
    ...
    if (refreshType != _refreshType.preEval)
      $ax.style.updateStateClass(repeaterId);
  ...
  _getRepeaterElementOffset = _repeaterManager.getRepeaterElementOffset =
    function (repeaterId, elementId) {
      var viewId = $ax.adaptive.currentViewId || "";
      ...
      var repeaterObj = $jobj(repeaterId);
      var repeaterOffset = {
        x: $ax.getNumFromPx(repeaterObj.css("left")),
        y: $ax.getNumFromPx(repeaterObj.css("top")),
      };
      var paddingTop = _getAdaptiveProp(
        propMap,
        "paddingTop",
      ...
      var firstTopLeftOffset = {
        x: paddingLeft + repeaterOffset.x,
        y: paddingTop + repeaterOffset.y,
      };

      var fitToContentOffset = { x: 0, y: 0 };

      var elementContainerId = repeaterId + "-" + repeaterNumber;
      var elementContainerDomElement =
        document.getElementById(elementContainerId);
      if (elementContainerDomElement.style.top) {
        fitToContentOffset.y =
          paddingTop +
          multiplier.y * yOffset -
          $ax.getNumFromPx(elementContainerDomElement.style.top);
      }
      if (elementContainerDomElement.style.left) {
        fitToContentOffset.x =
          paddingLeft +
          multiplier.x * xOffset -
          $ax.getNumFromPx(elementContainerDomElement.style.left);
      }
  ...
  var _getItemQuery = function (repeaterId, preevalMap) {
    var query = $ax(function (diagramObject, elementId) {
      ...
      return (
        $ax.getParentRepeaterFromScriptId(scriptId) == repeaterId &&
        scriptId != repeaterId
      );
    });
  ...
  _repeaterManager.refreshAllRepeaters = function () {
    $ax("*").each(function (diagramObject, elementId) {
      ...
      _refreshRepeater(
        elementId,
        $ax.getEventInfoFromEvent($ax.getjBrowserEvent()),
        _refreshType.persist
      );
    });
  };
  ...
  var _initPageInfo = function (obj, elementId) {
    ...
    var itemsPerPage = _getAdaptiveProp(
      map,
      "itemsPerPage",
      currentViewId,
      elementId,
    ...
    if (itemsPerPage == -1) pageInfo.noLimit = true;
      ...
      pageInfo.currPage = _getAdaptiveProp(
        map,
        "currPage",
        currentViewId,
        elementId,
  ...
  var _initPregen = function (repeaterId) {
    ...
    var unprocessedBaseIds = $jobj(
      $ax.repeater.createElementId(repeaterId, start + 1)
    )
      .html()
      .match(/(id|for)="?u([0-9]+)/g);
    var baseIds = [];
    ...
    for (var itemNum = start; itemNum < end; itemNum++) {
      ...
      for (i = 0; i < baseIds.length; i++)
        ids.push($ax.repeater.createElementId(baseIds[i], itemNum + 1));
      var itemId = itemNum + 1;
      orderedIds[itemNum] = itemId;

      var itemDiv = $jobj(
        $ax.repeater.createElementId(repeaterId, itemNum + 1)
      );
  ...
  var _applyColorCss = function (json, div) {
    ...
    if ($ax.ieColorManager && json.a != 0 && json.a != 1) {
      var ieColor = $ax.ieColorManager.getColorFromArgb(
        json.a * 255,
        json.r,
        json.g,
        json.b,
  ...
  var _getViewIdFromPageViewId = function (pageViewId, id, diagramObject) {
    if (diagramObject.owner.type != "Axure:Master") {
      ...
      var rdoStyle = $ax.style.computeFullStyle(
        parentRdoId,
        rdoState,
        pageViewId
      );
  ...
  var _getAdaptiveProp = (_repeaterManager.getAdaptiveProp = function (
    map,
    prop,
    viewId,
    repeaterId,
    ...
    var base =
      repeaterObj.owner.type != "Axure:Master"
        ? map[""]
        : map["19e82109f102476f933582835c373474"];
  ...
  _repeaterManager.getVisibleDataCount = function (repeaterId) {
    var bounds = _getVisibleDataBounds(
      repeaterToPageInfo[repeaterId],
      repeaterToActiveDataSet[repeaterId].length
    );
  ...
  var _getFilteredDataCount = (_repeaterManager.getFilteredDataCount =
    function (repeaterId) {
  ...
  _repeaterManager.getPageCount = function (repeaterId) {
    ...
    return info.noLimit
      ? 1
      : Math.ceil(_getFilteredDataCount(repeaterId) / info.itemsPerPage);
  };
      ...
      var mergesort = function (list, start, end, compare) {
        ...
        while (index1 < middle && index2 < end) {
          tempList[tempList.length] =
            list[compare(list[index1], list[index2]) > 0 ? index2++ : index1++];
        }
  ...
  const SMART_FILTER_COMBO = 0x0,
    ALL_FILTER_COMBO = 0x1,
    ANY_FILTER_COMBO = 0x2;
  ...
  var applyFilter = function (repeaterId, data, eventInfo) {
    ...
    const filtersByColumnData =
      SMART_FILTER_COMBO == filterType ? _filtersByColumn(filters) : {};
  ...
  var _evaluateFilter = function (filter, eventInfo) {
    const result = $ax.expr.evaluateExpr(
      filter.filter ? filter.filter : filter.condition,
      eventInfo
    );
  ...
  var _updateEventInfoForFilter = function (
    repeaterId,
    filter,
    eventInfo,
    idToWidgetInfo,
    ...
    if (!idToWidgetInfo[eventInfo.srcElement])
      idToWidgetInfo[eventInfo.srcElement] = $ax.getWidgetInfo(
  ...
  var _removeFilter = function (repeaterId, label) {
    ...
    for (var i = filterList.length - 1; i >= 0; i--) {
      ...
      if (filterObj.label && filterObj.label == label)
        $ax.splice(filterList, i, 1);
    }
  };
  ...
  var _setRepeaterToPage = function (repeaterId, type, value, eventInfo) {
    ...
    var lastPage = Math.max(
      1,
      Math.ceil(dataSet.length / pageInfo.itemsPerPage)
    );
  ...
  var removeItems = function (repeaterId) {
    ...
    for (var i = 0; i < itemId.length; i++)
      $jobj(_createElementId(repeaterId, itemId[i])).remove();
  ...
  _repeaterManager.setItemSize = function (repeaterId, itemId, width, height) {
    ...
    if (deltaX || deltaY)
      $ax.event.raiseSyntheticEvent(
        _createElementId(repeaterId, itemId),
        "onItemResize"
      );
  ...
  var _pushItems = (_repeaterManager.pushItems = function (
    repeaterId,
    itemId,
    delta,
    vertical,
    ...
    $ax.visibility.setResizedSize(
      itemElementId,
      $ax.getNumFromPx(itemObj.css("width")),
      $ax.getNumFromPx(itemObj.css("height"))
    );
    ...
    if (wrap) {
      outer: for (; index < sizes.length; index++) {
        var innerSizes = sizes[index];
        for (index2 = 0; index2 < innerSizes.length; index2++)
          if (innerSizes[index2].itemId == itemId) break outer;
      }
    } else {
      for (; index < sizes.length; index++)
        if (sizes[index].itemId == itemId) break;
    }
    ...
    if (vert == vertical) {
      ...
      if (wrap && sizes.length > 1) {
        ...
        var spacing = _getAdaptiveProp(
          obj.repeaterPropMap,
          (vert ? "vertical" : "horizontal") + "Spacing",
          viewId,
          repeaterId,
          obj
        );
      ...
      for (i = index + 1; i < sizes.length; i++) {
        currSizes = sizes[i];
        for (j = 0; j < currSizes.length; j++)
          itemIdsEffected.push(currSizes[j].itemId);
      }
  ...
  var _updateRepeaterSize = function (prop, jobj, delta, vert) {
    ...
    if (vert)
      border +=
        $ax.getNumFromPx(jobj.css("border-top-width")) +
        $ax.getNumFromPx(jobj.css("border-bottom-width"));
    else
      border +=
        $ax.getNumFromPx(jobj.css("border-left-width")) +
        $ax.getNumFromPx(jobj.css("border-right-width"));
    val += border;
    jobj.css(prop, val);
    $ax.visibility.setResizedSize(
      jobj.attr("id"),
  ...
  var _getDataFromDataSet = function (
    eventInfo,
    repeaterId,
    itemId,
    propName,
    ...
    var deleteMap =
      eventInfo &&
      eventInfo.repeaterDeleteMap &&
      eventInfo.repeaterDeleteMap[repeaterId];
  ...
  _repeaterManager.hasData = function (id, propName) {
    ...
    var repeaterId = $ax.getParentRepeaterFromScriptId(
      _getScriptIdFromElementId(id)
    );
    return Boolean(
      repeaterToCurrentDataSet[repeaterId] &&
        repeaterToCurrentDataSet[repeaterId].props.indexOf(propName) != -1
    );
  };

  var _getEventDeleteData = function (eventInfo, repeaterId) {
    var repeaterDeleteMap = eventInfo.repeaterDeleteMap;
    if (!repeaterDeleteMap)
      repeaterDeleteMap = eventInfo.repeaterDeleteMap = {};
  ...
  var _getRealItemId = function (eventInfo, repeaterId, itemId) {
    var deletedBefore = 0;
    var map =
      eventInfo.repeaterDeleteMap && eventInfo.repeaterDeleteMap[repeaterId];
    var deletedIds = map && map.deletedIds;
    if (!deletedIds) return itemId - 1;

    for (var i = 0; i < deletedIds.length; i++)
      if (deletedIds[i] < itemId) deletedBefore++;
  ...
  var _addItemToDataSet = function (repeaterId, row, itemEventInfo) {
    ...
    for (var propName in row) {
      ...
      if (prop.type == "literal") {
        ...
        if (typeof retval == "string" || retval instanceof Date)
          retval = { type: "text", text: retval };
        row[propName] = retval;
      }
    }
  ...
  var _deleteItemsFromDataSet = function (repeaterId, eventInfo, type, rule) {
    ...
    if (type == "this") items = [_getItemIdFromElementId(eventInfo.srcElement)];
      ...
      items = $ax.deepCopy(repeaterToEditItems[repeaterId]);
    else {
      // This should be rule
      ...
      for (
        var i = 0;
        i < visibleData.length + deleteDataMap.deletedIds.length;
        i++
      ) {
        var index = i + 1;
        ...
        if ($ax.expr.evaluateExpr(rule, eventInfo).toLowerCase() != "true")
          continue;
        items.push(index);
      }
      eventInfo.targetElement = oldTarget;
    ...
    for (i = 0; i < items.length; i++) {
      ...
      var deletedRow = $ax.splice(
        dataSet,
        _getRealItemId(eventInfo, repeaterId, itemId),
        1
      )[0];
  ...
  var _updateEditItemsInDataSet = function (
    repeaterId,
    propMap,
    eventInfo,
    type,
  ...
  var _createElementId = function (scriptId, itemId) {
    ...
    var sections =
      i > -1
        ? [scriptId.substring(0, i), scriptId.substring(i + 1)]
        : [scriptId];
  ...
  var _removeSuffixFromElementId = function (id) {
    ...
    if (partId != -1)
      return _createElementId(
        id.substr(0, partId),
        _getItemIdFromElementId(id)
      ); // item id is after part, but before suffix
...
$axure.internal(function ($ax) {
  ...
  var _isIdFitToContent = (_dynamicPanelManager.isIdFitToContent = function (
    id
  ) {
    var obj = $obj(id);
    if (!obj || !$ax.public.fn.IsDynamicPanel(obj.type) || !obj.fitToContent)
      return false;
  ...
  var _fitParentPanel = function (widgetId) {
    ...
    if (parentPanelInfo) {
      ...
      if (_updateFitPanel(parentId, parentPanelInfo.state))
        _fitParentPanel(parentId);
      return;
    }
    ...
    if (
      ...
      repeaterObj.repeaterPropMap.fitToContent
    ) {
  ...
  var _updateMobileScroll = (_dynamicPanelManager.updateMobileScroll =
    function (panelId, stateId, blockResetScroll) {
      ...
      stateQuery.css(
        "cursor",
        "url(resources/css/images/touch.svg) 32 32, auto"
      );
    });
  ...
  _dynamicPanelManager.initMobileScroll = function () {
    ...
    $ax("*").each(function (obj, elementId) {
      ...
      if (
        $ax.public.fn.IsDynamicPanel(obj.type) &&
        obj.scrollbars != "None" &&
        obj.scrollbars != "none" &&
        !$ax.visibility.isElementIdLimboOrInLimboContainer(scriptId)
      ) {
  ...
  var _isPercentWidthPanel = (_dynamicPanelManager.isPercentWidthPanel =
    function (obj) {
  ...
  _dynamicPanelManager.updatePanelContentPercentWidth = function (elementId) {
    ...
    stateChildrenQuery.children(".panel_state_content").each(function () {
      $(this)
        .children(".ax_dynamic_panel")
        .each(function () {
    ...
    if (!fixedCenterPanelsInitialized) {
      $axure(function (diagramObject) {
        return (
          ...
          diagramObject.fixedHorizontal == "center" &&
          !diagramObject.percentWidth
        );
  ...
  var _updatePercentPanelToLeftCache = function (obj, elementId, overwrite) {
    ...
    if (percentPanelToLeftCache[elementId] == undefined || overwrite) {
      if (obj.fixedHorizontal == "center")
        percentPanelToLeftCache[elementId] = $ax.getNumFromPx(
          jObj.css("margin-left")
        );
      else if (obj.fixedHorizontal == "right")
        percentPanelToLeftCache[elementId] =
          axObj.width() + $ax.getNumFromPx(jObj.css("right"));
      else
        percentPanelToLeftCache[elementId] = $ax.getNumFromPx(jObj.css("left"));
      wasUpdated = true;
    }
    ...
    if (obj.fixedHorizontal == "right" && _isIdFitToContent(elementId)) {
      ...
      percentPanelToLeftCache[elementId] =
        fitWidth + $ax.getNumFromPx(jObj.css("right"));
      wasUpdated = true;
    }
    return wasUpdated;
  };

  var _updatePanelPercentWidth = (_dynamicPanelManager.updatePanelPercentWidth =
    function (elementId) {
      ...
      if (obj.fixedHorizontal) {
        ...
        if (parentPanelInfo) {
          ...
          if (parentObj.percentWidth) {
            var stateId = $ax.repeater.applySuffixToElementId(
              parentId,
              "_state" + parentPanelInfo.state
            );
            var stateContentId = stateId + "_content";
            x = -$ax.getNumFromPx($jobj(stateContentId).css("margin-left"));
          } else x = 0;
        } else {
          var parentRepeater = $ax.getParentRepeaterFromScriptId(
            $ax.repeater.getScriptIdFromElementId(elementId)
          );
          if (parentRepeater) {
            var itemId = $ax.repeater.getItemIdFromElementId(elementId);
            var itemContainerId = $ax.repeater.createElementId(
              parentRepeater,
              itemId
            );
            x = 0;
            ...
            var isCenter =
              $ax.adaptive.getPageStyle().pageAlignment == "center";
      ...
      while (
        stateParent.children()[0].id.indexOf($ax.visibility.CONTAINER_SUFFIX) !=
        -1
      )
        stateParent = stateParent.children();
      ...
      if (obj.fixedHorizontal == "center")
        stateChildrenQuery
          ...
          .css("left", "50%")
          .css("margin-left", panelLeft + "px");
      else if (obj.fixedHorizontal == "right")
        stateChildrenQuery
          .children(".panel_state_content")
          .css("left", width - panelLeft + "px");
      else
        stateChildrenQuery
          .children(".panel_state_content")
          .css("margin-left", panelLeft - x + "px");
    });

  _dynamicPanelManager.updateParentsOfNonDefaultFitPanels = function () {
    $ax("*").each(function (diagramObject, elementId) {
      if (
        !$ax.public.fn.IsDynamicPanel(diagramObject.type) ||
        !diagramObject.fitToContent
      )
        return;
      if ($ax.visibility.isElementIdLimboOrInLimboContainer(elementId)) return;

      var stateId = $ax.visibility.GetPanelState(elementId);
      if (stateId != $ax.repeater.applySuffixToElementId(elementId, "_state0"))
        _fitParentPanel(elementId);
    });
  };
  ...
  var _updateLayerRectCache = function (elementId) {
    ...
    var sizeChange =
      oldRect.width != size.width || oldRect.height != size.height;
  ...
  _dynamicPanelManager.setFitToContentCss = function (
    elementId,
    fitToContent,
    oldWidth,
    oldHeight
  ) {
    if ($ax.dynamicPanelManager.isIdFitToContent(elementId) == fitToContent)
      return;
  ...
  var _getShownStateId = function (id) {
    ...
    var children = $ax.visibility
      .applyWidgetContainer(id, true, false, true)
      .children();
    for (var i = 0; i < children.length; i++) {
      var child = children[i];
      while ($ax.visibility.isContainer(child.id))
  ...
  var _updateFitPanel = function (panelId, stateIndex) {
    ...
    $ax.visibility.setResizedSize(
      panelId,
      isPercentWidth ? $ax("#" + panelId).width() : size.width,
      size.height
    );
  ...
  var getParentPanel = function (widgetId, path, targetId) {
    path =
      path ||
      $ax.getPathFromScriptId($ax.repeater.getScriptIdFromElementId(widgetId));
  ...
  var getParentLayer = function (widgetId, path) {
    ...
    if (widgetParentPanel) {
      ...
      if (
        !parentMasterParentPanel ||
        widgetParentPanel.parent != parentMasterParentPanel.parent
      )
        return undefined;
    }
  ...
  var _adjustFixed = (_dynamicPanelManager.adjustFixed = function (
    panelId,
    oldWidth,
    oldHeight,
    width,
  ...
  var _getFixedPosition = (_dynamicPanelManager.getFixedPosition = function (
    panelId,
    oldWidth,
    oldHeight,
    width,
  ...
  _dynamicPanelManager.getFixedInfo = function (panelId) {
    ...
    else if (info.horizontal == "center")
      info.x = $ax.getNumFromPx(jobj.css("margin-left"));
    else if (info.horizontal == "right")
      info.x = $ax.getNumFromPx(jobj.css("right"));

    if (info.vertical == "top") info.y = $ax.getNumFromPx(jobj.css("top"));
    else if (info.vertical == "middle")
      info.y = $ax.getNumFromPx(jobj.css("margin-top"));
    else if (info.vertical == "bottom")
      info.y = $ax.getNumFromPx(jobj.css("bottom"));
  ...
  var getPanelStateSizeDelta = function (oldState, newState) {
    ...
    return {
      width: Math.abs(newQuery.width() - oldQuery.width()),
      height: Math.abs(newQuery.height() - oldQuery.height()),
    };
  };
  ...
  var _compressMove = function (
    ...
    var magnitude = isResize
      ? newQuery[thresholdOffset]() - oldThresholdOffset
      : 1;
    ...
    var clampWidth = Math.max(
      vert ? targetRect.width : targetRect.height,
      newQuery[vert ? "width" : "height"]()
    );
  ...
  var _compress = function (
    ...
    if ($ax.getTypeFromElementId(id) == $ax.constants.LAYER_TYPE) {
      ...
      if (typeof clampWidth == "undefined")
        clampWidth = _getClamp(id)[clamp.offset]();

      var clampPropValueCss = clampLoc.css(clamp.prop);
      var clampPropValue = clampPropValueCss.endsWith("%")
        ? (vert ? clampLoc.width() : clampLoc.height()) *
          (Number(clampPropValueCss.replace("%", "")) / 100)
      ...
      var clampPropMargin = clampPropMarginCss.endsWith("%")
        ? (vert ? clampLoc.width() : clampLoc.height()) *
          (Number(clampPropMarginCss.replace("%", "")) / 100)
    ...
    if (
      $jobj(id).css("position") == "fixed" &&
      $jobj(id).css("display") != "none"
    ) {
      var clampDelta = document
        .getElementById("base")
    ...
    while (
      obj &&
      $ax.public.fn.IsLayer(obj.type) &&
      $ax.visibility.IsIdVisible(parent)
    ) {
      ...
      parent = $ax("#" + parent).getParents(false, [
        "item",
        "state",
        "layer",
      ])[0];
    ...
    if (layer) {
      ...
      var needsOffset =
        !$jobj(layer + "_container").length &&
        !$ax.visibility.IsIdVisible(layer);
    ...
    if (parent && $jobj(parent + "_content").length > 0)
      parent = parent + "_content";
    if (parent && $jobj(parent + "_container").length > 0)
    ...
    if (!obj.repeaterPropMap.fitToContent)
      $ax.repeater.pushItems(repeaterId, itemId, delta, vert);
  };

  var _layerMayNeedCompress = function (
    layerId,
    vert,
    threshold,
    clamp,
    ...
    marker =
      (vert ? boundingRect.top : boundingRect.left) +
      (vert ? boundingRect.height : boundingRect.width);
    layerClamp =
      clamp.prop == "left" ? [boundingRect.left] : [boundingRect.top];
    layerClamp[1] =
      layerClamp[0] +
      (clamp.offset == "width" ? boundingRect.width : boundingRect.height);
    ...
    if (
      ...
      isNaN(layerClamp[1]) ||
      marker < threshold ||
      layerClamp[1] <= clamp.start ||
      layerClamp[0] >= clamp.end
    ) {
      return false;
    }
    return true;
  ...
  var _compressChildrenHelper = function (
    id,
    children,
    vert,
    threshold,
    ...
    for (var i = 0; i < children.length; i++) {
      ...
      if (child[0] && child[0].tagName == "A" && child.hasClass("basiclink"))
      ...
      if ($ax.getTypeFromElementId(childId) == $ax.constants.LAYER_TYPE) {
        ...
        if (
          !_layerMayNeedCompress(childId, vert, threshold, clamp, parentLayer)
        ) {
          allMove = false;
          continue;
        }
        ...
        var addSelf;
        var container = $ax.visibility.applyWidgetContainer(
          childId,
          true,
          true
        );
        ...
        if (
          $ax.getLayerParentFromElementId(id) != childId &&
          _objectNeedsCompress(childId, vert, threshold, clamp, parentLayer)
        )
          addSelf = true;
        else
          addSelf = _compressChildrenHelper(
            id,
            layerChildren,
            vert,
            threshold + threshProp,
      ...
      if (
        ...
        (numbers.length == 2 && isNaN(Number(numbers[1]))) ||
        numbers.length > 2
      )
        continue;

      if (_objectNeedsCompress(childId, vert, threshold, clamp, parentLayer))
        toMove.push(childId);
      else allMove = false;
    }
    ...
    if (allMove && parentLayer) {
      ...
      for (var i = 0; i < toMove.length; i++) {
        $ax("#" + toMove[i]).moveBy(
          vert ? complimentaryDelta : delta,
          vert ? delta : complimentaryDelta,
          easing == "none" ? {} : { duration: duration, easing: easing }
        );
      }
    }
    return false;
  ...
  var _objectNeedsCompress = function (
    id,
    vert,
    threshold,
    clamp,
    ...
    if (
      ...
      isNaN(childClamp[1]) ||
      marker < threshold ||
      childClamp[1] <= clamp.start ||
      childClamp[0] >= clamp.end
    ) {
      return false;
    }
    return true;
  ...
  var _parentHandlesStyles = function (id) {
    var parents = $ax("#" + id).getParents(true, [
      "dynamicPanel",
      "layer",
      "item",
    ])[0];
    ...
    var hoverChildren = function (children) {
      ...
      for (var i = 0; i < children.length; i++) {
        ...
        if (
          ($ax.public.fn.IsDynamicPanel(obj.type) ||
            $ax.public.fn.IsLayer(obj.type) ||
            $ax.public.fn.IsRepeater(obj.type)) &&
          !obj.propagate
        )
          continue;
