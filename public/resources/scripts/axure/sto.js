$axure.internal(function ($ax) {
  ...
  _stoHandlers["var"] = function (sto, scope, eventInfo) {
    ...
    var retVal = scope.hasOwnProperty(sto.name)
      ? scope[sto.name]
      : $ax.globalVariableProvider.getVariableValue(sto.name, eventInfo);
  ...
  _stoHandlers.item = function (sto, scope, eventInfo, prop) {
    prop =
      ...
      (eventInfo.data
        ...
        : "text");
    var id =
      sto.isTarget || !$ax.repeater.hasData(eventInfo.srcElement, sto.name)
  ...
  _stoHandlers.fCall = function (sto, scope, eventInfo) {
    ...
    if (
      sto.thisSTO.desiredType == "string" &&
      sto.thisSTO.computedType != "string"
    )
    ...
    var fn =
      (funcs.hasOwnProperty(sto.func) && funcs[sto.func]) || thisObj[sto.func];
    return fn.apply(thisObj, args);
  };

  _stoHandlers.propCall = function (sto, scope, eventInfo) {
    //TODO: [mas] handle required type
    if ((sto.prop == "url" || sto.prop == "img") && sto.thisSTO.sto == "item")
      return _stoHandlers.item(sto.thisSTO, scope, eventInfo, sto.prop);
    var thisObj = _evaluateSTO(sto.thisSTO, scope, eventInfo);
    ...
    var prop =
      thisObj[sto.prop] instanceof Function
        ? thisObj[sto.prop]()
        : thisObj[sto.prop];
    return prop;
  };
  ...
  var _evaluateSTO = function (sto, scope, eventInfo) {
    ...
    return _tryEscapeRichText(
      castSto(_stoHandlers[sto.sto](sto, scope, eventInfo), sto),
      eventInfo
    );
  };
