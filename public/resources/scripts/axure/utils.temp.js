...
$axure.internal(function ($ax) {
  ...
  var _deepCopy = function (original, trackCopies, filter) {
    ...
    isArray
      ? deepCopyArray(original, trackCopies, copy, filter)
      : deepCopyObject(original, trackCopies, copy, filter);
    return copy;
  };
  ...
  var _getCopyIndex = function (original) {
    for (var i = 0; i < _originalToCopy.length; i++)
      if (original === _originalToCopy[i][0]) return i;
    return -1;
  };
  ...
  var deepCopyObject = function (original, trackCopies, copy, filter) {
    for (var key in original) {
      ...
      if (filter && filter.indexOf[key] != -1) copy[key] = original[key];
      // If that key is filtered out, skip recursion on it.
  ...
  $ax.splice = function (array, startIndex, count) {
    var retval = [];
    if (startIndex >= array.length || startIndex < 0 || count == 0)
      return retval;
    if (!count || startIndex + count > array.length)
      count = array.length - startIndex;
    for (var i = 0; i < count; i++) retval[i] = array[startIndex + i];
    for (i = startIndex + count; i < array.length; i++)
...
$axure.internal(function ($ax) {
  $(window.document).ready(function () {
    if (
      !$ax.document.configuration.linkFlowsToPages &&
      !$ax.document.configuration.linkFlowsToPagesNewWindow
    )
      return;
    ...
    $ax(function (dObj) {
      return (
        ($ax.public.fn.IsVector(dObj.type) ||
          $ax.public.fn.IsSnapshot(dObj.type)) &&
        dObj.referencePageUrl
      );
      ...
      if ($ax.document.configuration.linkFlowsToPagesNewWindow) {
        $("#" + elementId + "_ref").append(
          "<div id='" + elementId + "PagePopup' class='refpageimage'></div>"
        );
