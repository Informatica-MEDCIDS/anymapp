  ...
  $(window.document).ready(function () {
    ...
    function populateNotes(pageForNotes) {
      for (var master in pageForNotes.masters) {
        ...
        var hasWidgetNotes =
          masterData.annotations && masterData.annotations.length > 0;
        if (
          (master.notes && !$.isEmptyObject(masterData.notes)) ||
          hasWidgetNotes
        ) {
  ...
  var _processTempInit = function () {
    ...
    $axure.messageCenter.addMessageListener(function (message, data) {
      if (message == "setGlobalVar") {
        $ax.globalVariableProvider.setVariableValue(
          data.globalVarName,
          data.globalVarValue,
          true
        );
    ...
    $ax(function (dObj) {
      ...
      if ($ax.public.fn.IsLayer(dObj.type))
        $ax.event.layerMapFocus(dObj, elementId);
    ...
    function attachFocusAndBlur($query) {
      $query
        ...
        .blur(function () {
          if (window.lastFocusedClickable == this)
            window.lastFocusedClickable = null;
        });
    }
    ...
    $(window.document).bind("keyup", function (e) {
      switch (e.which) {
        case 13:
        case 32:
          if (window.lastFocusedClickable)
            $(window.lastFocusedClickable).click();
          break;
        default:
...
(function () {
  ...
  if (
    (gv_hasCanvas =
      !!(_canvas.getContext && _canvas.getContext("2d")) &&
