...
$axure.internal(function ($ax) {
  ...
  var isEqual = function (left, right, isFunction) {
    ...
    if (left instanceof Object && right instanceof Object) {
      ...
      for (prop in right) {
        ...
        if (left.hasOwnProperty(prop) != right.hasOwnProperty(prop))
          return false;
      }
  ...
  _exprHandlers.panelDiagramLiteral = function (expr, eventInfo) {
    ...
    for (var i = 0; i < elementIds.length; i++) {
      elementIdsWithSuffix[i] = $ax.repeater.applySuffixToElementId(
        elementIds[i],
        suffix
      );
    }
  ...
  _exprHandlers.fcall = function (expr, eventInfo) {
    ...
    return targets.length
      ? retval
      : _exprFunctions[expr.functionName].apply(this, fcallArgs);
  };
  ...
  var _substituteSTOs = function (expr, eventInfo) {
    ...
    for (var varName in expr.localVariables) {
      scope[varName] = $ax.expr.evaluateExpr(
        expr.localVariables[varName],
        eventInfo
      );
    }
    ...
    var i = 0;
    var retval;
    var retvalString = expr.value.replace(
      /\[\[(?!\[)(.*?)\]\](?=\]*)/g,
      function (match) {
        var sto = expr.stos[i++];
        ...
        return (
          (result instanceof Object && (result.label || result.text)) || result
        );
      }
    );
  ...
  _exprFunctions.SetSelectedOption = function (elementIds, value) {
    for (var i = 0; i < elementIds.length; i++) {
      ...
      if ($ax.event.HasSelectionChanged($ax.getObjectFromElementId(elementId)))
        $ax.event.raiseSyntheticEvent(elementId, "onSelectionChange");
    }
  };
  ...
  _exprFunctions.SetWidgetFormText = function (elementIds, value) {
    for (var i = 0; i < elementIds.length; i++) {
      ...
      if (
        obj.val() == value ||
        (value == "" && $ax.placeholderManager.isActive(elementId))
      )
        return;
      obj.val(value);
      $ax.placeholderManager.updatePlaceholder(elementId, !value);
      if ($ax.event.HasTextChanged($ax.getObjectFromElementId(elementId)))
  ...
  _exprFunctions.SetFocusedWidgetText = function (elementId, value) {
    if (window.lastFocusedControl) {
      ...
      if ($ax.public.fn.IsTextBox(type) || $ax.public.fn.IsTextArea(type))
        _exprFunctions.SetWidgetFormText([elementId], value);
  ...
  _exprFunctions.SetWidgetRichText = function (ids, value, plain) {
    ...
    for (var i = 0; i < ids.length; i++) {
      ...
      if ($obj(id).type !== $ax.constants.LINK_TYPE)
        id = $ax.GetTextPanelId(id, true);
  ...
  var _displayHackEnd = function (displayChangedList) {
    for (var i = 0; i < displayChangedList.length; i++)
      displayChangedList[i].style.display = "none";
  };
  ...
  var autofitWidget = function (id) {
    ...
    if (obj.autoFitHeight || obj.autoFitWidth) {
      ...
      var style = $ax.style.computeFullStyle(
        id,
  ...
  _exprFunctions.GetCursorRectangles = function () {
    ...
    rects.lastRect = new $ax.drag.Rectangle(
      $ax.lastMouseLocation.x,
      $ax.lastMouseLocation.y,
      1,
      1
    );
    rects.currentRect = new $ax.drag.Rectangle(
      $ax.mouseLocation.x,
      $ax.mouseLocation.y,
      1,
      1
  ...
  _exprFunctions.GetWidgetRectangles = function (elementIds, eventInfo) {
    ...
    if (invalid) {
      rects.lastRect = rects.currentRect = new $ax.drag.Rectangle(
        -1,
        -1,
        -1,
        -1
  ...
  _exprFunctions.GetAdaptiveView = function (eventInfo) {
    if (eventInfo && eventInfo.srcElement) {
      ...
      if (diagramObject.owner.type == "Axure:Master") {
        var viewIdChain = $ax.style.getViewIdChain(
          $ax.adaptive.currentViewId || "",
          id,
          diagramObject
        );
  ...
  _exprFunctions.IsEntering = function (movingRects, targetRects) {
    return (
      !movingRects.lastRect.IntersectsWith(targetRects.currentRect) &&
      movingRects.currentRect.IntersectsWith(targetRects.currentRect)
    );
  };
  ...
  var _evaluateExpr = ($ax.expr.evaluateExpr = function (
    expr,
    eventInfo,
    toString
  ) {
