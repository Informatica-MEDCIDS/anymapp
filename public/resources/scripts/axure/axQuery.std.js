...
$axure.internal(function ($ax) {
  ...
  $ax.public.fn.IsContainer = function (type) {
    return (
      type == $ax.constants.VECTOR_SHAPE_TYPE ||
      type == $ax.constants.TABLE_TYPE ||
      type == $ax.constants.MENU_OBJECT_TYPE ||
      type == $ax.constants.TREE_NODE_OBJECT_TYPE
    );
  };

  var SET_OPACITY_TYES = [
    $ax.constants.CHECK_BOX_TYPE,
    $ax.constants.RADIO_BUTTON_TYPE,
    $ax.constants.TEXT_BOX_TYPE,
    $ax.constants.TEXT_AREA_TYPE,
    $ax.constants.LIST_BOX_TYPE,
    $ax.constants.COMBO_BOX_TYPE,
    $ax.constants.BUTTON_TYPE,
    $ax.constants.IMAGE_BOX_TYPE,
    $ax.constants.IMAGE_MAP_REGION_TYPE,
    $ax.constants.VECTOR_SHAPE_TYPE,
  ];
  $ax.public.fn.SupportSetOpacity = function (type) {
    return $.inArray(type, SET_OPACITY_TYES) !== -1;
  };
  ...
  var RESIZABLE_TYPES = [
    ...
    $ax.constants.IMAGE_BOX_TYPE,
    $ax.constants.IMAGE_MAP_REGION_TYPE,
    $ax.constants.INLINE_FRAME_TYPE,
    ...
    $ax.constants.TEXT_AREA_TYPE,
    $ax.constants.TEXT_BOX_TYPE,
    $ax.constants.SNAPSHOT_TYPE,
  ];

  $ax.public.fn.IsSelectionButton = function (type) {
    return (
      type == $ax.constants.RADIO_BUTTON_TYPE ||
      type == $ax.constants.CHECK_BOX_TYPE
    );
  };

  $ax.public.fn.SupportsErrorStyle = function (widgetType) {
    return (
      $ax.public.fn.IsImageBox(widgetType) ||
      $ax.public.fn.IsVector(widgetType) ||
      $ax.public.fn.IsDynamicPanel(widgetType) ||
      $ax.public.fn.IsLayer(widgetType) ||
      $ax.public.fn.IsTextArea(widgetType) ||
      $ax.public.fn.IsTextBox(widgetType) ||
      $ax.public.fn.IsListBox(widgetType) ||
      $ax.public.fn.IsComboBox(widgetType) ||
      $ax.public.fn.IsCheckBox(widgetType) ||
      $ax.public.fn.IsRadioButton(widgetType)
    );
  };
  ...
  $ax.public.fn.SupportsRichText = function () {
    ...
    if (obj.type == $ax.constants.TREE_NODE_OBJECT_TYPE)
      return obj.friendlyType == "Tree Node";
    // Do the same for tree node icons maybe?

    return $.inArray(obj.type, SUPPORTS_RICH_TEXT_TYPES) != -1;
  };
  var SUPPORTS_RICH_TEXT_TYPES = [
    $ax.constants.CHECK_BOX_TYPE,
    $ax.constants.RADIO_BUTTON_TYPE,
    $ax.constants.IMAGE_BOX_TYPE,
    $ax.constants.VECTOR_SHAPE_TYPE,
    $ax.constants.TABLE_CELL_TYPE,
    $ax.constants.CONNECTOR_TYPE,
  ];
  ...
  for (
    var jqueryFunctionIndex = 0;
    jqueryFunctionIndex < _jQueryFunctionsToAdd.length;
    jqueryFunctionIndex++
  )
    _addJQueryFunction(_jQueryFunctionsToAdd[jqueryFunctionIndex]);
  ...
  var _jQueryEventFunctionsToAdd = [
    "click",
    "mouseenter",
    "mouseleave",
    "bind",
  ...
  for (
    var jqueryEventIndex = 0;
    jqueryEventIndex < _jQueryEventFunctionsToAdd.length;
    jqueryEventIndex++
  )
    _addJQueryEventFunction(_jQueryEventFunctionsToAdd[jqueryEventIndex]);
  ...
  $ax.public.fn.SetPanelState = function (
    stateNumber,
    options,
    eventInfo,
    showWhenSet
    ...
    for (var index = 0; index < elementIds.length; index++) {
      ...
      if ($ax.public.fn.IsDynamicPanel($ax.getTypeFromElementId(elementId))) {
        ...
        var newStateName = $ax.visibility.GetPanelStateId(
          elementId,
          Number(stateNumber) - 1
        );
        var wasVisible = $ax.visibility.IsIdVisible(elementId);

        $ax.dynamicPanelManager.setPanelSizeChange(
          elementId,
          $ax.dynamicPanelManager.getPanelStateSizeDelta(
            currentStateName,
            newStateName
          )
        );
        ...
        if (options.compress && options.compressDistanceType == "custom") {
          delta = Number(
            $ax.expr.evaluateExpr(options.compressValue, eventInfo)
          );
        }
        ...
        if (
          options.compress &&
          $ax.dynamicPanelManager.isIdFitToContent(elementId) &&
          wasVisible
        ) {
          $ax.dynamicPanelManager.compressDelta(
            elementId,
            currentStateName,
            newStateName,
            options.vertical,
            options.compressEasing,
            options.compressDuration,
            delta
          );
        }
        $ax.visibility.SetPanelState(
          elementId,
          newStateName,
          animateOutInfo.easingType,
          animateOutInfo.direction,
          animateOutInfo.duration,
          animateInInfo.easingType,
          animateInInfo.direction,
          animateInInfo.duration,
          showWhenSet
        );
        // If compressing because of a show, must be after state is set.
        if (options.compress && !wasVisible && showWhenSet) {
          $ax.dynamicPanelManager.compressToggle(
            elementId,
            options.vertical,
            true,
            options.compressEasing,
  ...
  $ax.public.fn.show = function (options, eventInfo) {
    ...
    for (var index = 0; index < elementIds.length; index++) {
      ...
      var lightboxId = $ax.repeater.applySuffixToElementId(
        elementId,
        "_lightbox"
      );
      ...
      if (options && options.showType == "lightbox") {
        ...
        if (lightbox.length == 0) {
          ...
          for (var j = 0; j < parents.length; j++) {
            ...
            if (
              $ax.visibility.IsIdVisible(parentId) &&
              ($jobj(parentId).css("z-index") != "auto" ||
                $ax.features.supports.mobile)
            ) {
          ...
          (function (lightbox, query) {
            $ax.event.attachClick(lightbox, function () {
              $ax.action.addAnimation(
                elementId,
                $ax.action.queueTypes.fade,
                function () {
        ...
        $ax.flyoutManager.registerFlyout(
          rects,
          elementId,
  ...
  var _getAnimateInfo = function (options, defaultDuration, useHide) {
    ...
    if (options) {
      ...
      (easingOption = "none"), (animationOption = "swing");
    }
  ...
  var _setVisibility = function (elementId, value, options, useHide) {
    ...
    var wasShown = $ax.visibility.IsIdVisible(elementId);
    var compress =
      options && _shouldCompress(options.showType) && wasShown != value;
    if (compress)
      $ax.dynamicPanelManager.compressToggle(
        elementId,
        options.vertical,
    ...
    function setOpacity(ids) {
      for (var index = 0; index < ids.length; index++) {
        ...
        if ($ax.public.fn.IsLayer(obj.type)) {
          ...
          $ax.action.removeAnimationFromQueue(
            elementId,
            $ax.action.queueTypes.fade
          );
  ...
  $ax.public.fn.circularMoveAndRotate = function (
    degreeChange,
    options,
    centerPointLeft,
    centerPointTop,
    ...
    for (var index = 0; index < elementIds.length; index++) {
      ...
      $ax.move.circularMove(
        elementId,
        degreeChange,
        { x: centerPointLeft, y: centerPointTop },
        moveDelta,
        rotatableMove,
        resizeOffset,
        options,
  ...
  $ax.public.fn.resize = function (
    newLocationAndSizeCss,
    resizeInfo,
    axShouldFire,
    moves,
    onCompletedFunc
    ...
    for (var index = 0; index < elementIds.length; index++) {
      ...
      if (isDynamicPanel) {
        ...
        $ax.dynamicPanelManager.setFitToContentCss(
          elementId,
          false,
          oldWidth,
          oldHeight
        ...
        if (
          ...
          ((obj.fixedHorizontal && obj.fixedHorizontal == "center") ||
            (obj.fixedVertical && obj.fixedVertical == "middle"))
        ) {
          moves = true;
          var loc = $ax.dynamicPanelManager.getFixedPosition(
            elementId,
            oldWidth,
            oldHeight,
            newLocationAndSizeCss.width,
            newLocationAndSizeCss.height
          );
          if (loc) {
            if (
              loc[0] != 0 &&
              !$ax.dynamicPanelManager.isPercentWidthPanel(obj)
            )
              newLocationAndSizeCss["margin-left"] =
                "+=" +
                (Number(newLocationAndSizeCss["margin-left"].substr(2)) +
                  loc[0]);
            if (loc[1] != 0)
              newLocationAndSizeCss["margin-top"] =
                "+=" +
                (Number(newLocationAndSizeCss["margin-top"].substr(2)) +
                  loc[1]);
          }
        }
      ...
      while (
        children &&
        children.length &&
        $(children[0]).attr("id").indexOf("container") != -1
      ) {
      ...
      if (children && children.length !== 0) {
        ...
        children.each(function (i, child) {
          ...
          var differentSizedImage =
            childSizingObj.width - oldWidth != 0 ||
            childSizingObj.height - oldHeight != 0;
          if ((differentSizedImage || isConnector) && child.tagName == "IMG") {
            //oldwidth is zero for connectors
            var widthOffset = oldWidth
              ? ((childSizingObj.width - oldWidth) *
                  newLocationAndSizeCss.width) /
                oldWidth
              : childSizingObj.width;
            var heightOffset = oldHeight
              ? ((childSizingObj.height - oldHeight) *
                  newLocationAndSizeCss.height) /
                oldHeight
              : childSizingObj.height;
          ...
          if ($(child).css("position") === "absolute") {
            if (child.offsetLeft) {
              ...
              childCss.left = oldWidth
                ? (child.offsetLeft * newLocationAndSizeCss.width) / oldWidth
                : child.offsetLeft; //- transformedShift.x;
            }
            if (child.offsetTop) {
              childSizingObj.top = child.offsetTop;
              childCss.top = oldHeight
                ? (child.offsetTop * newLocationAndSizeCss.height) / oldHeight
                : child.offsetTop; //- transformedShift.y;
            }
          }
          childAnimationArray.push({
            obj: child,
            sizingObj: childSizingObj,
            sizingCss: childCss,
          });
        });
      }
        ...
        if (childAnimationArray) {
          $(childAnimationArray).each(function (i, animationObj) {
            if (animationObj.resizeMatrixFunction) {
              $(animationObj.obj).css(
                $ax.public.fn.setTransformHowever(
                  animationObj.resizeMatrixFunction(
                    animationObj.width,
                    animationObj.height
                  )
                )
              ...
              $(animationObj.obj).animate(animationObj.sizingCss, {
                queue: false,
                duration: 0,
              });
            }
            ...
            if (animationObj.resizeMatrixFunction) {
              $(animationObj.sizingObj).animate(animationObj.sizingCss, {
                ...
                step: function (now) {
                  ...
                  $(animationObj.obj).css(
                    $ax.public.fn.setTransformHowever(
                      animationObj.resizeMatrixFunction(widthRatio, heightRatio)
                    )
                  );
                },
              });
    ...
    if (arguments[0] == undefined) {
      ...
      for (var index = 0; index < elementIds.length; index++) {
        ...
        if (
          ...
          if ($(idRtf).length == 0)
            idRtf = "#u" + (Number(currentItem.substring(1)) + 1);
          ...
          if ($(idRtf).length != 0) {
            ...
            if ($(idRtf).find("p").find("span").length > 0) {
              ...
              $(idRtf)
                .find("span")
                .html(textHtml.replace(/--NEWLINE--/g, "<br>"));
            }
          }
        }
      }
  ...
  var getWidgetText = function (id) {
    ...
    if (
      idQuery.is("input") &&
      ($ax.public.fn.IsCheckBox(idQuery.attr("type")) ||
        idQuery.attr("type") == "radio")
    ) {
    ...
    if (arguments[0] == undefined) {
      ...
      for (var index = 0; index < elementIds.length; index++) {
        ...
        if (
          ...
          //Do nothing for non rtf
          continue;
        } else {
          //TODO -- [mas] fix this!
          var idRtf = "#" + currentItem;
          if ($(idRtf).length == 0)
            idRtf = "#u" + (parseInt(currentItem.substring(1)) + 1);
  ...
  $ax.public.fn.value = function () {
    if (arguments[0] == undefined) {
      ...
      if (
        ...
        //for select lists and drop lists
      ...
      } else if ($ax.public.fn.IsTextBox(widgetType)) {
        //for text box
        return $("#" + firstId + "_input").val();
      } else {
        //for text based form elements
      ...
      for (var index = 0; index < elementIds.length; index++) {
        ...
        if (
          ...
          //For select lists, drop lists, text based form elements
  ...
  var _scrollHelper = function (id, scrollX, scrollY, easing, duration) {
    ...
    if ($scrollable.is("body"))
      viewportLocation = $ax("#" + id).viewportLocation();
    else
      viewportLocation = $ax("#" + id).pageBoundingRect(
        true,
        $scrollable.attr("id"),
        true
      ).location;
    ...
    if (easing == "none") {
      ...
      if (!scrollX) {
        ...
        $scrollable.animate(
          { scrollTop: targetTop, scrollLeft: targetLeft },
          duration,
          easing
        );
      }
  ...
  $ax.public.fn.enabled = function () {
    if (arguments[0] == undefined) {
      ...
      if (
        ...
        return !$ax.style.IsWidgetDisabled(firstId);
      else
        return (
          this.jQuery().children(":disabled").length <= 0 &&
          this.jQuery().children(".disabled").length <= 0
        );
    } else {
      var elementIds = this.getElementIds();
      ...
      for (var index = 0; index < elementIds.length; index++) {
        ...
        if (PLAIN_TEXT_TYPES.indexOf(widgetType) != -1 && images) {
          var img = $jobj(
            $ax.repeater.applySuffixToElementId(elementId, "_image_sketch")
          );
          var key =
  ...
  $ax.public.fn.visible = function () {
    ...
    for (var index = 0; index < ids.length; index++)
      $ax.visibility.SetIdVisible(ids[index], arguments[0]);
    return this;
  };
  ...
  $ax.public.fn.selected = function () {
    if (arguments[0] == undefined) {
      ...
      if ($ax.public.fn.IsTreeNodeObject(widgetType)) {
        ...
        for (var i = 0; i < allElementIds.length; i++) {
          ...
          if (
            $ax.public.fn.IsVector(currObj.type) &&
            currObj.parent &&
            currObj.parent.scriptIds &&
            currObj.parent.scriptIds[0] == firstId
          ) {
        ...
        $ax.public.fn.IsImageBox(widgetType) ||
        $ax.public.fn.IsVector(widgetType) ||
        $ax.public.fn.IsTableCell(widgetType) ||
        $ax.public.fn.IsDynamicPanel(widgetType) ||
        $ax.public.fn.IsLayer(widgetType) ||
        $ax.public.fn.IsTextArea(widgetType) ||
        $ax.public.fn.IsTextBox(widgetType) ||
    ...
    for (var index = 0; index < elementIds.length; index++) {
      ...
      if ($ax.public.fn.IsTreeNodeObject(widgetType)) {
        //for tree node
        var treeRootId = $("#" + elementIds[index])
          .parents(".treeroot")
          .attr("id");
        ...
        for (var i = 0; i < childElementIds.length; i++) {
          ...
          if (
            currObj &&
            currObj.type == $ax.constants.VECTOR_SHAPE_TYPE &&
            currObj.parent &&
            currObj.parent.scriptIds &&
            currObj.parent.scriptIds[0] == elementIds[index]
          ) {
            treeNodeButtonShapeId = elementId;
            break;
          }
  ...
  $ax.public.fn.expanded = function () {
    if (arguments[0] == undefined) {
      ...
      return (
        firstId &&
        !$ax.public.fn.IsTreeNodeObject($ax.getTypeFromElementId(firstId)) &&
        $ax.visibility.IsIdVisible(firstId + "_children")
      );
    } else {
      var elementIds = this.getElementIds();
      ...
      for (var index = 0; index < elementIds.length; index++) {
        if (
          $ax.public.fn.IsTreeNodeObject(
            $ax.getTypeFromElementId(elementIds[index])
          )
        ) {
          ...
          if (
            $("#" + childContainerId).length == 0 ||
            !$jobj(plusMinusId).children().first().is("img")
          )
            plusMinusId = "";
  ...
  var _boundingRectForIds = function (childIds) {
    ...
    var childrenBoundingRect = {
      left: childIds.length > 0 ? 99999 : 0,
      top: childIds.length > 0 ? 99999 : 0,
      right: 0,
      bottom: 0,
    };
    for (var i = 0; i < childIds.length; i++) {
      ...
      if (
        $ax.visibility.limboIds[childId] ||
        !$ax.visibility.IsIdVisible(childId) ||
        ($ax.public.fn.IsDynamicPanel(childObj.type) &&
          childObj.fixedHorizontal)
      )
        continue;
      ...
      // Ignore empty groups
      if (
        $ax.public.fn.IsLayer(childObj.type) &&
        boundingRect.width == 0 &&
        boundingRect.height == 0
      )
        continue;

      childrenBoundingRect.left = Math.min(
        childrenBoundingRect.left,
        boundingRect.left
      );
      childrenBoundingRect.top = Math.min(
        childrenBoundingRect.top,
        boundingRect.top
      );
      childrenBoundingRect.right = Math.max(
        childrenBoundingRect.right,
        boundingRect.right
      );
      childrenBoundingRect.bottom = Math.max(
        childrenBoundingRect.bottom,
        boundingRect.bottom
      );
    }
    childrenBoundingRect.width =
      childrenBoundingRect.right - childrenBoundingRect.left;
    childrenBoundingRect.height =
      childrenBoundingRect.bottom - childrenBoundingRect.top;
  ...
  var _fixedLocation = function (elementId, size) {
    ...
    if (horz == "left") {
      newLeft =
        ...
        (axObj.percentWidth
          ? 0
          : $ax.getNumFromPx($jobj(elementId).css("left")));
    } else if (horz == "center") {
      newLeft =
        windowScrollLeft +
        (windowWidth - width) / 2 +
        axObj.fixedMarginHorizontal;
    } else if (horz == "right") {
      newLeft =
        windowScrollLeft + windowWidth - width - axObj.fixedMarginHorizontal;
    }
    ...
    if (vert == "top") {
      ...
      newTop =
        ...
        axObj.fixedMarginVertical;
    } else if (vert == "bottom") {
      newTop =
        windowScrollTop + windowHeight - height - axObj.fixedMarginVertical;
    }
    ...
    return {
      valid: true,
      top: newTop,
      left: axObj.isPercentWidthPanel ? 0 : newLeft,
    };
  };

  //relative to the parent
  $ax.public.fn.offsetBoundingRect = function (
    ignoreRotation,
    ignoreOuterShadow
  ) {
    ...
    if (movedLoc) {
      ...
      style = $ax.style.computeFullStyle(
        elementId,
        state,
        $ax.adaptive.currentViewId
      );
    ...
    if (!ignoreRotation) {
      ...
      if (rotation && rotation != 0)
        boundingRect = $ax.public.fn.getBoundingRectForRotate(
          _populateBoundingRect(boundingRect),
          rotation
        );
    }
  ...
  $ax.public.fn.pageBoundingRect = function (
    ignoreRotation,
    scrollableId,
    ignoreOuterShadow
  ) {
    ...
    var otherParents = $ax("#" + elementId).getParents(true, [
      "item",
      "repeater",
      "dynamicPanel",
    ])[0];
    ...
    var parentScrollableId = scrollableId
      ? scrollableId.split("_")[0]
      : scrollableId;
  ...
  $ax.public.fn.viewportBoundingRect = function (scrollableId) {
    ...
    if (!boundingRect.isFixed)
      boundingRect.left = _bodyToWorld(boundingRect.left, false);
    return _populateBoundingRect(boundingRect);
  };

  $ax.public.fn.size = function ({
    ignoreRotation = true,
    ignoreOuterShadow = true,
  } = {}) {
    var boundingRect = this.offsetBoundingRect(
      ignoreRotation,
      ignoreOuterShadow
  ...
  var _bodyToWorld = ($axure.fn.bodyToWorld = function (x, from) {
    ...
    var offset =
      $ax.getNumFromPx(body.css("left")) +
      Math.max(0, ($(window).width() - body.width()) / 2);
    if (from) offset *= -1;
    return x + offset;
  });
