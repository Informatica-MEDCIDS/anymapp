$axure.internal(function ($ax) {
  ...
  var _switchView = function (viewId, forceSwitchTo) {
    ...
    $axure("*").each(function (diagramObject, elementId) {
      ...
      if (element) {
        ...
        if (!isPanel || diagramObject.fitToContent) {
          //keeps size on the panel states when switching adaptive views to optimize fit to panel
          if (diagramObject.fitToContent)
            $ax.dynamicPanelManager.setFitToContentCss(elementId, true);
          var children = query.children();
          if (children.length) children.css(resetCss);
        }

        $ax.dynamicPanelManager.resetFixedPanel(diagramObject, element);
        $ax.dynamicPanelManager.resetAdaptivePercentPanel(
          diagramObject,
          element
        );
      }
    ...
    if (!viewId) {
      $axure("*").each(function (diagramObject, elementId) {
        ...
        if (
          diagramObject.type == "horizontalLine" ||
          diagramObject.type == "verticalLine"
        ) {
        ...
        if (child.length)
          $ax.style.transformTextWithVerticalAlignment(
            child[0].id,
            function () {}
          );
      });
  ...
  var _getAdaptiveIdChain = ($ax.adaptive.getAdaptiveIdChain = function (
    viewId
  ) {
    if (!viewId) return [];
  ...
  var _getMasterAdaptiveIdChain = ($ax.adaptive.getMasterAdaptiveIdChain =
    function (masterId, viewId) {
      if (!viewId) return [];
  ...
  var _applyAdaptiveViewOnObject = function (
    diagramObject,
    elementId,
    viewIdChain,
    viewId,
    ...
    for (var i = 0; i < viewIdChain.length; i++) {
      ...
      if (viewStyle) {
        ...
        if (viewStyle.size)
          $ax.public.fn.convertToSingleImage($jobj(elementId));
      }
    }
    ...
    if (images) {
      if (
        ...
        var compoundUrl = _matchImageCompound(
          diagramObject,
          elementId,
          viewIdChain,
          state
    ...
    var adaptiveStyle = $ax.style.computeAllOverrides(
      elementId,
      undefined,
      state,
      viewId
    ...
    if (compoundStyle.limbo && !diagramObject.isContained)
      limboIds[scriptId] = true;
  ...
  var _matchImage = function (
    ...
    const defaultStateImage =
      findImage(images, scriptId, state) ||
      findImage(images, scriptId, overrideState);
  ...
  var _matchImageCompound = function (diagramObject, id, viewIdChain, state) {
    ...
    for (var i = 0; i < diagramObject.compoundChildren.length; i++) {
      ...
      images[component] = _matchImage(
        id,
        diagramObject.images[component],
        viewIdChain,
        state
      );
    }
  ...
  $ax.adaptive.getImageForStateAndView = function (id, state) {
    ...
    if (diagramObject.generateCompound)
      return _matchImageCompound(diagramObject, id, viewIdChain, state);
  ...
  var _getAdaptiveView = function (winWidth, winHeight) {
    var _isViewOneGreaterThanTwo = function (view1, view2, winHeight) {
      ...
      if (view1.size.width == view2.size.width) {
        if (view2.size.height <= winHeight)
          return (
            view1.size.height > view2.size.height &&
            view1.size.height <= winHeight
          );
    ...
    if (_isWindowWidthGreaterThanViewWidth(defaultView, winWidth, winHeight))
      greater = defaultView;
    less = defaultView;
    for (var i = 0; i < _enabledViews.length; i++) {
      ...
      if (_isWindowWidthGreaterThanViewWidth(view, winWidth, winHeight)) {
        if (!greater || _isViewOneGreaterThanTwo(view, greater, winHeight))
          greater = view;
      }
    ...
    if (message == "switchAdaptiveView") {
      ...
      if (data.scale != 0 && data.scale != 3) {
        ...
        var view = $ax.adaptive.currentViewId
          ? _idToView[$ax.adaptive.currentViewId]
          : $ax.pageData.defaultAdaptiveView;
        ...
        var mobileWidth =
          (IOS
            ? isLandscape
              ...
              : window.screen.width
            : window.screen.width) - data.panelWidthOffset;
        var scaleN = (newScaleN =
          (MOBILE_DEVICE ? mobileWidth : data.mainFrameWidth) / bodyWidth);

        if (data.scale == 2) {
          var pageSize = $ax.public.fn.getPageSize();
          var hScaleN =
            (MOBILE_DEVICE ? data.viewportHeight : data.mainFrameHeight) /
            Math.max(1, pageSize.bottom);
          if (hScaleN < scaleN) {
        ...
        if (data.scale == 3) {
          const zoom = $(document).find("html").attr("zoom")
            ? parseInt($(document).find("html").attr("zoom"), 10)
            : 100;
  ...
  var _isHtmlQuery = function ($container) {
    return $container.length > 0 && $container[0] == $("html")[0];
  };

  var _removeNiceScroll = ($ax.adaptive.removeNiceScroll = function (
    $container,
    blockResetScroll
  ) {
    if (!blockResetScroll) {
  ...
  var _addNiceScrollExitDetector = function ($container) {
    if (_isHtmlQuery($container)) {
      ...
      var $scrollContainer = isScrollContainerExist
        ? $(".scrollContainer")
      ...
      $base.on("mouseleave.ax", function (e) {
        var nS = $container.getNiceScroll();
        for (var i = 0; i < nS.length; ++i) nS[i].ontouchend(e);
      });
  ...
  var _addNiceScroll = ($ax.adaptive.addNiceScroll = function (
    $container,
    options,
    blockResetScroll
  ) {
  ...
  var _setTrackpadHorizontalScroll = function (active) {
    ...
    if (!active) {
      ...
      document
        .getElementById("html")
        .addEventListener("mousewheel", preventScroll, { passive: false });
    } else {
      ...
      document
        .getElementById("html")
        .removeEventListener("mousewheel", preventScroll, { passive: false });
    }
  };
  ...
  $ax.adaptive.setAdaptiveView = function (view) {
    var viewIdForSitemapToUnderstand =
      view == "auto" ? undefined : view == "default" ? "" : view;
  ...
  $ax.adaptive.initialize = function () {
    ...
    if (_views && _views.length > 0) {
      ...
      if (_autoIsHandledBySidebar && _initialViewSizeToLoad)
        _handleSetViewForSize(
          _initialViewSizeToLoad.width,
          _initialViewSizeToLoad.height
        );
