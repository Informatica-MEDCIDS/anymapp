...
$axure.internal(function ($ax) {
  ...
  var _scrollToSelectedNote = function ($elmt, view) {
    ...
    var bodyLeft =
      $("body").css("left") !== undefined && $("body").css("left") !== "auto"
        ? Number($("body").css("left").replace("px", ""))
        : 0;
    ...
    var _isInteractive = function (diagramObject) {
      if (diagramObject && diagramObject.interactionMap) {
        for (var index in userTriggeredEventNames) {
          if (diagramObject.interactionMap[userTriggeredEventNames[index]])
            return true;
        }
      }
      return false;
  ...
  var _getElementIdFromTarget = function (target) {
    ...
    var jTarget = $(target);
    while (
      (!targetId || targetId.indexOf("cache") > -1) &&
      jTarget[0].tagName != "HTML"
    ) {
