$axure.internal(function ($ax) {
  ...
  var _setLinkStyle = function (id, styleName) {
    ...
    var style = _computeAllOverrides(
      id,
      parentId,
      styleName,
  ...
  var _widgetHasState = function (id, state) {
    ...
    for (var i = 0; i < adaptiveIdChain.length; i++) {
      ...
      if (
        adaptiveStyle &&
        adaptiveStyle.stateStyles &&
        adaptiveStyle.stateStyles[state]
      )
        return true;
    }
  ...
  $ax.style.SetWidgetHover = function (id, value) {
    ...
    const state = _generateFullState(
      id,
      undefined,
      undefined,
      undefined,
  ...
  $ax.style.AddRolloverOverride = function (id) {
    ...
    if ($ax.event.mouseOverIds.indexOf(id) == -1)
      $ax.style.SetWidgetHover(id, true);
  };
  ...
  var _generateFullState = function (
    id,
    forceFocused,
    forceSelected,
    forceDisabled,
    ...
    if (
      forceError === true ||
      (forceError !== false && _style.IsWidgetError(id))
    ) {
      state = _compoundStates(state, ERROR);
    }
    if (
      forceSelected === true ||
      (forceSelected !== false && _style.IsWidgetSelected(id))
    ) {
      state = _compoundStates(state, SELECTED);
    }
    if (
      forceDisabled === true ||
      (forceDisabled !== false && _isWidgetDisabled(id))
    ) {
      return _compoundStates(DISABLED, state);
    }
    if (
      forceFocused === true ||
      (forceFocused !== false && _hasAnyFocusedClass(id))
    ) {
      return _compoundStates(state, FOCUSED);
    }
    if (
      forceMouseDown === true ||
      (forceMouseDown !== false && $ax.event.mouseDownObjectId == id)
    ) {
      state = _compoundStates(state, MOUSE_DOWN);
    }
    if (
      forceMouseOver === true ||
      (forceMouseOver !== false &&
        (($ax.event.mouseOverIds.indexOf(id) !== -1 &&
          _widgetHasState(id, MOUSE_OVER)) ||
          _isRolloverOverride(id)))
    ) {
  ...
  $ax.style.SetWidgetSelected = function (id, value, alwaysApply) {
    ...
    if (value) {
      ...
      if (group) {
        $("[selectiongroup='" + group + "']").each(function () {
          ...
          if (
            $ax.visibility.isScriptIdLimbo(
              $ax.repeater.getScriptIdFromElementId(otherId)
            )
          )
            return;
    ...
    if (obj) {
      ...
      if (
        ...
        var path = $ax.getPathFromScriptId(
          $ax.repeater.getScriptIdFromElementId(id)
        );
        ...
        while (
          obj.isContained &&
          !$ax.getObjectFromElementId(id).interactionMap
        )
          obj = obj.parent;
        path = $ax.getPathFromScriptId(
          $ax.repeater.getScriptIdFromElementId(id)
        );
        path[path.length - 1] = obj.id;
        actionId = $ax.getElementIdFromPath(path, { itemNum: itemId });
  ...
  $ax.style.SetWidgetEnabled = function (id, value) {
    ...
    $("#" + id)
      .find("a")
      .css("cursor", value ? "pointer" : "default");
    ...
    if (!value) {
      ...
      $ax.style.SetWidgetSelected(id, $ax.style.IsWidgetSelected(id), true);
  };
  ...
  $ax.style.SetWidgetPlaceholder = function (id, active, text, password) {
    ...
    const style = _computeAllOverrides(
      id,
      undefined,
      state,
      $ax.adaptive.currentViewId
    ...
    _updateStateClasses(
      [id, $ax.repeater.applySuffixToElementId(id, "_div"), inputId],
      state,
      true
    );
  };
  ...
  var _addImageOverride = function (elementId, state, val) {
    if (!_elementIdsToImageOverrides[elementId])
      _elementIdsToImageOverrides[elementId] = {};
  ...
  $ax.style.getElementImageOverride = function (elementId, state) {
    var url =
      _elementIdsToImageOverrides[elementId] &&
      _elementIdsToImageOverrides[elementId][state];
    return url;
  };
  ...
  var MOUSE_OVER_MOUSE_DOWN_SELECTED_ERROR_HINT =
  ...
  var _highestPriorityBaseState = (_style.highestPriorityBaseState = function (
    state
  ) {
  ...
  var _decomposeState = (_style.decomposeState = function (state) {
    ...
    if (state == MOUSE_OVER_SELECTED_ERROR_HINT)
      return MOUSE_OVER_SELECTED_ERROR;
    ...
    if (state == MOUSE_DOWN_SELECTED_ERROR_HINT)
      return MOUSE_DOWN_SELECTED_ERROR;
    ...
    if (state == MOUSE_OVER_MOUSE_DOWN_SELECTED_ERROR)
      return MOUSE_OVER_MOUSE_DOWN_SELECTED;
    if (state == MOUSE_OVER_MOUSE_DOWN_SELECTED_HINT)
      return MOUSE_OVER_MOUSE_DOWN_SELECTED;
    if (state == MOUSE_OVER_MOUSE_DOWN_SELECTED_ERROR_HINT)
      return MOUSE_OVER_MOUSE_DOWN_SELECTED_ERROR;
    if (state == MOUSE_OVER_MOUSE_DOWN_ERROR_HINT)
      return MOUSE_OVER_MOUSE_DOWN_ERROR;
  ...
  var _updateElementIdImageStyle = (_style.updateElementIdImageStyle =
    function (elementId, state) {
      if (!_style.elementHasAnyImageOverride(elementId)) return;

      if (!state) state = _generateState(elementId);

      var style = _computeFullStyle(
        elementId,
        state,
        $ax.adaptive.currentViewId
      );
      ...
      if (hasBorderWidth && linePattern)
        borderQueryCss["border-style"] = linePattern;

      var borderFill = style.borderFill;
      if (hasBorderWidth && borderFill) {
        var color =
          borderFill.fillType == "solid"
            ? borderFill.color
            : borderFill.fillType == "linearGradient"
            ? borderFill.colors[0].color
            : 0;
      ...
      if (outerShadow && outerShadow.on) {
        var arg = "";
        arg +=
          outerShadow.offsetX + "px" + " " + outerShadow.offsetY + "px" + " ";
        var rgba = outerShadow.color;
        arg +=
          outerShadow.blurRadius +
          "px" +
          " 0px " +
          _rgbaToFunc(rgba.r, rgba.g, rgba.b, rgba.a);
  ...
  var _applyImageAndTextJson = function (id, event) {
    ...
    if (textId) {
      const overridedStyle = _computeAllOverrides(
        id,
        undefined,
        event,
        $ax.adaptive.currentViewId
      ...
      if (!$.isEmptyObject(overridedStyle)) {
        ...
        if (
          borderElement &&
          textElement &&
          (diagramObject.autoFitHeight || diagramObject.autoFitWidth)
        ) {
          const fullStyle = _computeFullStyle(
            id,
            event,
            $ax.adaptive.currentViewId,
            overridedStyle
  ...
  let _updateStateClassesHelper = function (
    id,
    event,
    addMouseOverOnMouseDown
  ) {
    const jobj = $jobj(id);

    const isMouseDownEvent = _stateHasMouseDown(event);

    for (let i = 0; i < ALL_STATES_WITH_CSS_CLASS.length; i++)
      jobj.removeClass(ALL_STATES_WITH_CSS_CLASS[i]);

    if (
      (addMouseOverOnMouseDown && isMouseDownEvent) ||
      _stateHasMouseOver(event)
    )
      jobj.addClass(MOUSE_OVER);
  ...
  var _getViewIdChain = ($ax.style.getViewIdChain = function (
    currentViewId,
    id,
    diagramObject
  ) {
    ...
    if (diagramObject.owner.type != "Axure:Master") {
      ...
      viewIdChain = $ax.adaptive.getMasterAdaptiveIdChain(
        diagramObject.owner.packageId,
        viewOverride
      );
    }
    return viewIdChain;
  ...
  var _computeAllOverrides = ($ax.style.computeAllOverrides = function (
    id,
    parentId,
    state,
    currentViewId
    ...
    var computedStyle = {};
    if (parentId)
      computedStyle = _computeAllOverrides(
        parentId,
        null,
        state,
        currentViewId
    ...
    for (var i = 0; i < viewIdChain.length; i++) {
      ...
      if (style) {
        ...
        if (style) {
          var customStyle =
            style.baseStyle &&
            $ax.document.stylesheet.stylesById[style.baseStyle];
    ...
    $.extend(
      computedStyle,
      _computeStateStyleForViewChain(diagramObject, NORMAL, viewIdChain, true)
    );
    if (_stateHasMouseOver(state)) {
      $.extend(
        computedStyle,
        _computeStateStyleForViewChain(
          diagramObject,
          MOUSE_OVER,
          viewIdChain,
          true
    ...
    if (_stateHasError(state)) {
      $.extend(
        computedStyle,
        _computeStateStyleForViewChain(diagramObject, ERROR, viewIdChain, true)
      );
    }
    if (_stateHasHint(state)) {
      $.extend(
        computedStyle,
        _computeStateStyleForViewChain(diagramObject, HINT, viewIdChain, true)
      );
    }
  ...
  var _computeStateStyleForViewChain = function (
    diagramObject,
    state,
    viewIdChain,
    excludeNormal
    ...
    for (var i = viewIdChain.length - 1; i >= 0; i--) {
      ...
      var stateStyle =
        viewStyle && _getFullStateStyle(viewStyle, state, excludeNormal);
    ...
    var stateStyleFromDefault = _getFullStateStyle(
      styleObject.style,
      state,
      true
    );
  ...
  var _computeFullStyle = ($ax.style.computeFullStyle = function (
    id,
    state,
    currentViewId,
    overrides
    ...
    var obj = $obj(id);
    if (!overrides)
      overrides = _computeAllOverrides(id, undefined, state, currentViewId);
    ...
    var customStyle =
      objStyle.baseStyle &&
      $ax.document.stylesheet.stylesById[objStyle.baseStyle];
    var returnVal = $.extend(
      {},
      $ax.document.stylesheet.defaultStyle,
      customStyle,
      objStyle,
      dynamicPanelStyle,
      overrides
  ...
  var _removeUnsupportedProperties = function (style, object) {
    ...
    if (
      $ax.public.fn.IsRadioButton(object.type) ||
      $ax.public.fn.IsCheckBox(object.type)
    ) {
      style.paddingTop = 0;
      style.paddingLeft = 0;
    ...
    if (
      $ax.public.fn.IsTextBox(object.type) ||
      $ax.public.fn.IsTextArea(object.type) ||
      $ax.public.fn.IsButton(object.type) ||
      $ax.public.fn.IsListBox(object.type) ||
      $ax.public.fn.IsComboBox(object.type)
    ) {
  ...
  var _getFullStateStyle = function (style, state, excludeNormal) {
    ...
    var stateStyle =
      state == NORMAL && !excludeNormal
        ? style
        : style && style.stateStyles && style.stateStyles[state];
    if (stateStyle) {
      var customStyle =
        stateStyle.baseStyle &&
        $ax.document.stylesheet.stylesById[stateStyle.baseStyle];
      //make sure not to extend the customStyle this can mutate it for future use
  ...
  var _applyOpacityFromStyle = ($ax.style.applyOpacityFromStyle = function (
    id,
    style
  ) {
    return;
  ...
  var _applyImage = ($ax.style.applyImage = function (id, imgUrl, state) {
    ...
    if (object.generateCompound) {
      for (var i = 0; i < object.compoundChildren.length; i++) {
        ...
        _updateStateClasses([childId + "_img", childId], state, false);
      }
    } else {
      ...
      _updateStateClasses([id, $ax.GetImageIdFromShape(id)], state, false);
      if (imgQuery.parents("a.basiclink").length > 0)
        imgQuery.css("border", "none");
    }
  });
  ...
  var _transformTextWithVerticalAlignment =
    ($ax.style.transformTextWithVerticalAlignment = function (
      textId,
      transformFn
    ) {
      if (!_originalTextCache[textId]) {
  ...
  $ax.style.reselectElements = function () {
    // TODO
    console.log(
      "reselect elements -- need to make sure selected/error/disabled are all correct"
    );
  ...
  _style.updateStateClass = function (repeaterId) {
    ...
    for (var i = 0; i < subElementIds.length; i++) {
      _applyImageAndTextJson(
        subElementIds[i],
        $ax.style.generateState(subElementIds[i])
      );
    }
  };
  ...
  $ax.style.setAdaptiveStyle = function (shapeId, style) {
    _adaptiveStyledWidgets[$ax.repeater.getScriptIdFromElementId(shapeId)] =
      style;
  ...
  var _getCssStyleProperties = function (style) {
    ...
    if (style.fontName)
      toApply.allProps.fontFamily = toApply.runProps.fontFamily =
        style.fontName;
    ...
    if (style.fontSize)
      toApply.allProps.fontSize =
        toApply.runProps.fontSize =
        toApply.parProps.fontSize =
          style.fontSize;
    if (style.fontWeight !== undefined)
      toApply.allProps.fontWeight = toApply.runProps.fontWeight =
        style.fontWeight;
    if (style.fontStyle !== undefined)
      toApply.allProps.fontStyle = toApply.runProps.fontStyle = style.fontStyle;

    var textDecoration = [];
    if (style.underline !== undefined)
      textDecoration[0] = style.underline ? "underline " : "none";
    ...
    if (textDecoration.length > 0) {
      ...
      toApply.allProps.textDecoration = toApply.runProps.textDecoration =
        decorationLineUp;
    }
    if (style.foreGroundFill) {
      toApply.allProps.color = toApply.runProps.color = _getColorFromFill(
    ...
    if (style.horizontalAlignment)
      toApply.allProps.textAlign =
        toApply.parProps.textAlign =
        toApply.runProps.textAlign =
          style.horizontalAlignment;
    if (style.lineSpacing)
      toApply.allProps.lineHeight = toApply.parProps.lineHeight =
        style.lineSpacing;
    if (style.textShadow)
      toApply.allProps.textShadow = toApply.parProps.textShadow = _getCssShadow(
        style.textShadow
      );
    if (style.letterCase)
      toApply.allProps.textTransform = toApply.parProps.textTransform =
        style.letterCase;
    if (style.characterSpacing)
      toApply.allProps.letterSpacing = toApply.runProps.letterSpacing =
  ...
  $ax.style.ClearCacheForRepeater = function (repeaterId) {
    for (var elementId in _originalTextCache) {
      ...
      if ($ax.getParentRepeaterFromScriptId(scriptId) == repeaterId)
        delete _originalTextCache[elementId];
    }
  };
