...
$axure.internal(function ($ax) {
  ...
  var _updateLinkLocations = ($ax.annotation.updateLinkLocations = function (
    elementId
  ) {
    ...
    if (WEBKIT && rotation) {
      ...
      var oldShapeTransform =
        document.getElementById(elementId).style["-webkit-transform"];
      var oldTextTransform =
        document.getElementById(textId).style["-webkit-transform"];
  ...
  var _createFootnotes = ($ax.annotation.createFootnotes = function (
    query,
    create
  ) {
  ...
  $ax.annotation.jQueryAnn = function (query) {
    ...
    var jQuerySelectorText =
      elementIdSelectors.length > 0 ? elementIdSelectors.join(", ") : "";
  ...
  var _adjustIconLocation = ($ax.annotation.adjustIconLocation = function (
    id,
    dObj
  ) {
    ...
    if (ann) {
      ...
      var width = $(ann).width();
      var newTopRight = $ax.public.fn.vectorPlus(
        corners.relativeTopRight,
        corners.centerPoint
      );
    ...
    if (ref) {
      ...
      var newBottomRight = $ax.public.fn.vectorPlus(
        corners.relativeBottomRight,
        corners.centerPoint
      );
