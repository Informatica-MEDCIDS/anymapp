$axure.internal(function ($ax) {
  ...
  var getAnimation = function (id, type) {
    return (
      animationQueue[id] &&
      animationQueue[id][type] &&
      animationQueue[id][type][0]
    );
  };

  var _addAnimation = (_action.addAnimation = function (
    id,
    type,
    func,
    suppressFire
  ...
  var _addAnimations = function (animations) {
    ...
    for (var i = 0; i < animations.length; i++) {
      ...
      if (thisReady) readyCount++;
        ...
        if (!typeToGroups)
          actionToActionGroups[animation.id] = typeToGroups = {};
  ...
  var _fireAnimationFromQueue = (_action.fireAnimationFromQueue = function (
    id,
    type
  ) {
  ...
  var _removeAnimationFromQueue = (_action.removeAnimationFromQueue = function (
    id,
    type
  ) {
    if (animationQueue[id] && animationQueue[id][type])
      $ax.splice(animationQueue[id][type], 0, 1);
  });
  ...
  var _ignoreAction = function (repeaterId) {
    for (var i = 0; i < _refreshing.length; i++)
      if (_refreshing[i] == repeaterId) return true;
    return false;
  };

  var _addRefresh = function (repeaterId) {
    if (_repeatersToRefresh.indexOf(repeaterId) == -1)
      _repeatersToRefresh.push(repeaterId);
  };
  ...
  var _queueResizeMove = function (id, type, eventInfo, actionInfo) {
    if (type == queueTypes.resize || type == queueTypes.rotate)
      $ax.public.fn.convertToSingleImage($jobj(id));
    ...
    if (
      !state[type].queue.length &&
      (!myOptions.easing || myOptions.easing == "none" || !myOptions.duration)
    ) {
      var func =
    ...
    var otherType1 =
      type == queueTypes.move ? queueTypes.resize : queueTypes.move;
    var otherType2 =
      type == queueTypes.rotate ? queueTypes.resize : queueTypes.rotate;
    if (!state[otherType1].queue.length || !state[otherType2].queue.length) {
      ...
      while (
        state[otherType1].queue.length &&
        state[otherType2].queue.length &&
        duration != 0
      ) {
        ...
        var otherOptions1 =
          otherType1 == queueTypes.resize
            ? other1.actionInfo
            : other1.actionInfo.options;
        // If queue up action is a non animation, then don't combo it, just queue it and move on
        if (
          !otherOptions1.easing ||
          otherOptions1.easing == "none" ||
          !otherOptions1.duration
        ) {
          func =
            otherType1 == queueTypes.resize
              ? _addResize
        ...
        var otherOptions2 =
          otherType2 == queueTypes.resize
            ? other2.actionInfo
            : other2.actionInfo.options;
        // If queue up action is a non animation, then don't combo it, just queue it and move on
        if (
          !otherOptions2.easing ||
          otherOptions2.easing == "none" ||
          !otherOptions2.duration
        ) {
          func =
            otherType2 == queueTypes.resize
              ? _addResize
        ...
        var stop = {
          start: myOptions.duration - duration,
          len: myOptions.duration,
        };
        ...
        var otherStop1 = {
          start: used1,
          end: options.duration + used1,
          len: otherOptions1.duration,
        };
        var otherStop2 = {
          start: used2,
          end: options.duration + used2,
          len: otherOptions2.duration,
        };
        options.stop =
          type == queueTypes.resize
            ? stop
  ...
  _action.flushAllResizeMoveActions = function (eventInfo) {
    var idToResizeMoveState = _getIdToResizeMoveState(eventInfo);
    for (var id in idToResizeMoveState)
      _flushResizeMoveActions(id, idToResizeMoveState);
  };
  ...
  var _flushResizeMoveActions = function (id, idToResizeMoveState) {
    ...
    while (moveInfo || resizeInfo || rotateInfo) {
      ...
      if (resizeInfo) {
        ...
        if (moveInfo)
          duration = Math.min(duration, moveInfo.options.duration - move.used);
        if (rotateInfo)
          duration = Math.min(
            duration,
            rotateInfo.options.duration - rotate.used
          );
        ...
        options.stop = {
          start: resize.used,
          end: resize.used + duration,
          len: resizeInfo.duration,
        };
        if (moveInfo)
          options.moveStop = {
            start: move.used,
        ...
        if (
          !rotateInfo.options.duration ||
          rotateInfo.options.easing == "none"
        ) {
        ...
        if (!moveInfo.options.duration || moveInfo.options.easing == "none") {
          _addMove(id, eventInfo, moveInfo, {
            easing: "none",
            duration: 0,
            stop: { instant: true },
          });
  ...
  var _updateResizeMoveUsed = function (
    id,
    type,
    duration,
    idToResizeMoveState
    ...
    var optionDur =
      (options.easing && options.easing != "none" && options.duration) || 0;
  ...
  var _dispatchAction = ($ax.action.dispatchAction = function (
    eventInfo,
    actions,
    currentIndex
  ) {
    ...
    _actionHandlers[actions[currentIndex].action](
      eventInfo,
      actions,
      currentIndex
    );
  ...
  var shouldUseStartHtml = function (linkAction) {
    return (
      linkAction.target.targetType == "page" && //only adding player for page, not external links
    ...
    ); //share app use special handling on its link, add start.html breaks the handling
  };

  var urlWithStartHtml = function (url) {
    var pageName = url.substring(0, url.lastIndexOf(".html"));
    var pageHash = $axure.utils.setHashStringVar(
      START_URL_NAME,
      PAGE_URL_NAME,
      pageName
    );
  ...
  var _includeVars = function (target, eventInfo) {
    ...
    if (target.urlLiteral) {
      ...
      if (
        literal.exprType == "stringLiteral" &&
        literal.value.indexOf("[[") == 0 &&
        literal.value.indexOf("]]" == literal.value.length - 2) &&
        literal.stos.length == 1 &&
        sto.sto == "item" &&
        eventInfo.item
      ) {
        var data = $ax.repeater.getData(
          eventInfo,
          eventInfo.item.repeater.elementId,
          eventInfo.item.index,
          sto.name,
          "data"
        );
  ...
  _actionHandlers.setPanelState = function (eventInfo, actions, index) {
    ...
    for (var i = 0; i < action.panelsToStates.length; i++) {
      ...
      var elementIds = $ax.getElementIdsFromPath(
        panelToState.panelPath,
        eventInfo
      );
        ...
        (function (elementId, stateInfo) {
          _addAnimation(elementId, queueTypes.setState, function () {
            ...
            if (stateInfo.setStateType == "value") {
              var stateName = $ax.expr.evaluateExpr(
                stateInfo.stateValue,
                eventInfo
              );
              ...
              if (!stateNameFound) {
                ...
                if (
                  isNaN(stateNumber) ||
                  stateNumber <= 0 ||
                  stateNumber > panelCount ||
                  Math.round(stateNumber) != stateNumber
                )
                  return _fireAnimationFromQueue(
                    elementId,
                    queueTypes.setState
                  );
              }
            } else if (
              stateInfo.setStateType == "next" ||
              stateInfo.setStateType == "previous"
            ) {
              ...
              return _progessPanelState(
                elementId,
                info,
                eventInfo,
                info.repeatSkipFirst
              );
            }
            delete _repeatPanelMap[elementId];

            // If setting to current (to stop repeat) break here
            if (stateInfo.setStateType == "current")
              return _fireAnimationFromQueue(elementId, queueTypes.setState);

            $ax("#" + elementId).SetPanelState(
              stateNumber,
              stateInfo.options,
              eventInfo,
              stateInfo.showWhenSet
            );
          });
        })(elementId, stateInfo);
  ...
  var _progessPanelState = function (id, stateInfo, eventInfo, skipFirst) {
    ...
    if (currentStateId != "") {
      ...
      var currentStateNumber = Number(
        currentStateId.substr(currentStateId.indexOf("state") + 5)
      );
      if (direction == "next") {
      ...
      if (hasRepeat && _repeatPanelMap[id] != stateInfo)
        return _fireAnimationFromQueue(id, queueTypes.setState);
  ...
  _actionHandlers.fadeWidget = function (eventInfo, actions, index) {
    ...
    for (var i = 0; i < action.objectsToFades.length; i++) {
      ...
      var elementIds = $ax.getElementIdsFromPath(
        action.objectsToFades[i].objectPath,
        eventInfo
      );
      ...
      for (var j = 0; j < elementIds.length; j++) {
        ...
        (function (elementId, fadeInfo) {
          _addAnimation(elementId, queueTypes.fade, function () {
            if (
              fadeInfo.options.compress &&
              fadeInfo.options.compressDistanceType == "custom"
            ) {
              var oldTarget = eventInfo.targetElement;
              eventInfo.targetElement = elementId;
              fadeInfo.options.compressDelta = Number(
                $ax.expr.evaluateExpr(fadeInfo.options.compressValue, eventInfo)
              );
              eventInfo.targetElement = oldTarget;
            }
  ...
  _actionHandlers.setOpacity = function (eventInfo, actions, index) {
    ...
    for (var i = 0; i < action.objectsToSetOpacity.length; i++) {
      ...
      var elementIds = $ax.getElementIdsFromPath(
        action.objectsToSetOpacity[i].objectPath,
        eventInfo
      );
      ...
      for (var j = 0; j < elementIds.length; j++) {
        ...
        (function (elementId, opacityInfo) {
          _addAnimation(elementId, queueTypes.fade, function () {
            ...
            $ax("#" + elementId).setOpacity(
              opacity / 100,
              opacityInfo.easing,
              opacityInfo.duration
            );
          });
  ...
  _actionHandlers.moveWidget = function (eventInfo, actions, index) {
    ...
    for (var i = 0; i < action.objectsToMoves.length; i++) {
      ...
      if (right || below) {
        ...
        var xDelta = isDelta
          ? Number($ax.expr.evaluateExpr(moveInfo.xValue, eventInfo))
          : NaN;
        var yDelta = isDelta
          ? Number($ax.expr.evaluateExpr(moveInfo.yValue, eventInfo))
          : NaN;
        ...
        var clampRect = isResize
          ? $ax.visibility.getResizingRect(srcId)
          : $ax("#" + srcId).offsetBoundingRect(true);

        $ax.dynamicPanelManager.compressMove(
          srcId,
          below,
          isResize,
          clampRect,
          moveInfo.options.easing,
          moveInfo.options.duration,
          below ? yDelta : xDelta,
          below ? xDelta : yDelta
        );
        continue;
    ...
    if ($ax.public.fn.IsLayer($obj(elementId).type)) {
      ...
      animations.push({
        ...
        func: function () {
          ...
          var deltaLoc = _getMoveLoc(
            elementId,
            moveInfo,
            eventInfoCopy,
            optionsOverride.stop,
            idToResizeMoveState[elementId],
            options,
            layerInfo
          );
  ...
  var _moveSingleWidget = function (elementId, delta, options, onComplete) {
    ...
    var moveInfo = $ax.move.PrepareForMove(
      elementId,
      delta.x,
      delta.y,
      false,
    ...
    $jobjAll(elementId).animate(css, {
      ...
      complete: function () {
        ...
        if (moveInfo.rootLayer)
          $ax.visibility.popContainer(moveInfo.rootLayer, false);
        $ax.dynamicPanelManager.fitParentPanel(elementId);
        $ax.action.fireAnimationFromQueue(
          elementId,
          $ax.action.queueTypes.move
        );
      },
    });
  };

  var _getMoveLoc = function (
    elementId,
    moveInfo,
    eventInfoCopy,
    stopInfo,
    ...
    switch (moveInfo.moveType) {
      case "location":
        ...
        var toRatio = stopInfo.instant
          ? 1
          : (stopInfo.end - stopInfo.start) / (stopInfo.len - stopInfo.start);
        ...
        if (stopInfo.instant || stopInfo.end == stopInfo.len)
          comboState.moveResult = undefined;
        ...
        break;
      case "delta":
        var ratio = stopInfo.instant
          ? 1
          : (stopInfo.end - stopInfo.start) / stopInfo.len;
      ...
      case "withThis":
        ...
        var srcElementId = $ax.getElementIdsFromEventAndScriptId(
          eventInfoCopy,
          eventInfoCopy.srcElement
        )[0];
    ...
    if (options && options.boundaryExpr) {
      ...
      if (moveWithThis && (xValue || yValue)) {
        _updateLeftExprVariable(
          options.boundaryExpr,
          xValue.toString(),
          yValue.toString()
        );
      }
      ...
      if (!$ax.expr.evaluateExpr(options.boundaryExpr, eventInfoCopy)) {
        ...
        if (boundaryStoInfo) {
          if (moveWithThis) {
            ...
            if (stoScopes) {
              for (var s in stoScopes) {
                ...
                if (boundaryScope.localVariables.withx)
                  boundaryScope.localVariables.withx.value = xValue.toString();
                if (boundaryScope.localVariables.withy)
                  boundaryScope.localVariables.withy.value = yValue.toString();
              }
            }
          }
          ...
          if (boundaryStoInfo.ySto) {
            ...
            var newTop = $ax.evaluateSTO(
              boundaryStoInfo.ySto,
              boundaryStoInfo.boundaryScope,
              eventInfoCopy
            );
          ...
          if (boundaryStoInfo.xSto) {
            ...
            var newLeft = $ax.evaluateSTO(
              boundaryStoInfo.xSto,
              boundaryStoInfo.boundaryScope,
              eventInfoCopy
            );
  ...
  var _updateLeftExprVariable = function (exprTree, xValue, yValue) {
    ...
    //traversal
    if (exprTree.op) {
      if (exprTree.leftExpr)
        _updateLeftExprVariable(exprTree.leftExpr, xValue, yValue);
      if (exprTree.rightExpr)
        _updateLeftExprVariable(exprTree.rightExpr, xValue, yValue);
    }
  };

  var widgetRotationFilter = [
    $ax.constants.IMAGE_BOX_TYPE,
    $ax.constants.IMAGE_MAP_REGION_TYPE,
    $ax.constants.DYNAMIC_PANEL_TYPE,
    $ax.constants.VECTOR_SHAPE_TYPE,
    $ax.constants.VERTICAL_LINE_TYPE,
    $ax.constants.HORIZONTAL_LINE_TYPE,
  ];
  _actionHandlers.rotateWidget = function (eventInfo, actions, index) {
    ...
    for (var i = 0; i < action.objectsToRotate.length; i++) {
      ...
      var elementIds = $ax.getElementIdsFromPath(
        action.objectsToRotate[i].objectPath,
        eventInfo
      );
  ...
  var _addRotate = function (
    ...
    var eval = function (boundingRect) {
      rotateDegree = parseFloat(
        $ax.expr.evaluateExpr(rotateInfo.degree, eventInfoCopy)
      );
      offset.x = Number(
        $ax.expr.evaluateExpr(rotateInfo.offsetX, eventInfoCopy)
      );
      offset.y = Number(
        $ax.expr.evaluateExpr(rotateInfo.offsetY, eventInfoCopy)
      );
    ...
    if ($ax.public.fn.IsLayer(obj.type)) {
      var childrenIds = $ax.public.fn.getLayerChildrenDeep(
        elementId,
        true,
        true
      );
        ...
        func: function () {
          ...
          if (offset.x == 0 && offset.y == 0)
            _rotateSingle(
              elementId,
              rotateDegree,
              rotateInfo.rotateType == "location",
              delta,
              options,
              options.stop,
              true
  ...
  var _rotateSingle = function (
    ...
    var degreeDelta = _applyRotateStop(
      rotateDegree,
      $ax.move.getRotationDegree(elementId),
      rotateTo,
      stop
    );
    $ax("#" + elementId).rotate(
  ...
  var _rotateSingleOffset = function (
    elementId,
    rotateDegree,
    rotateTo,
    delta,
    ...
    if (currRotation) {
      offset = $axure.fn.getPointAfterRotate(currRotation, offset, {
        x: 0,
        y: 0,
      });
    }
    ...
    $ax("#" + elementId).circularMoveAndRotate(
      degreeDelta,
      options,
      widgetCenter.x + offset.x,
      widgetCenter.y + offset.y,
      rotate,
      delta,
      resizeOffset
    );
  ...
  var _applyRotateStop = function (rotateDegree, currRotation, to, stop) {
    ...
    if (to) {
      ...
      ratio = stop.instant
        ? 1
        : (stop.end - stop.start) / (stop.len - stop.start);
    } else {
  ...
  var _addResize = function (
    ...
    var moves =
      moveInfo ||
      resizeInfo.anchor != "top left" ||
      ($ax.public.fn.IsDynamicPanel(axObject.type) &&
        ((axObject.fixedHorizontal && axObject.fixedHorizontal == "center") ||
          (axObject.fixedVertical && axObject.fixedVertical == "middle"))) ||
    ...
    if ($ax.public.fn.IsLayer(axObject.type)) {
      ...
      animations.push({
        ...
        func: function () {
          ...
          var ratio = stop.instant
            ...
            : (stop.end - stop.start) / (stop.len - stop.start);
          widthChangedPercent =
            (Math.round(size.width - boundingRect.width) / boundingRect.width) *
            ratio;
          heightChangedPercent =
            (Math.round(size.height - boundingRect.height) /
              boundingRect.height) *
            ratio;
          resizeOffset = _applyAnchorToResizeOffset(
            widthChangedPercent * boundingRect.width,
            heightChangedPercent * boundingRect.height,
            resizeInfo.anchor
          );
      ...
      for (var idIndex = 0; idIndex < childrenIds.length; idIndex++) {
        ...
        (function (childId) {
          ...
          var rotate =
            $.inArray($obj(childId).type, widgetRotationFilter) != -1;
          animations.push({
            id: childId,
            type: queueTypes.resize,
            func: function () {
              ...
              if (isLayer) {
                ...
                var css = _getCssForResizingLayerChild(
                  childId,
                  resizeInfo.anchor,
                  boundingRect,
                  widthChangedPercent,
                ...
                if (rotateInfo) {
                  var offset = {
                    ...
                    y: Number(
                      $ax.expr.evaluateExpr(rotateInfo.offsetY, eventInfo)
                    ),
                  };
                  ...
                  $ax("#" + childId).circularMoveAndRotate(
                    degreeDelta,
                    options,
                    centerPoint.x,
                    centerPoint.y,
      ...
      animations.push({
        ...
        func: function () {
          ...
          var sizeId = $ax.public.fn.IsTextArea(axObject.type)
            ? $jobj(elementId).children("textarea").attr("id")
            : elementId;
          ...
          if (rotateInfo) {
            ...
            _updateOffset(
              offset,
              rotateInfo.anchor,
              $ax("#" + elementId).offsetBoundingRect()
            );
          ...
          if (rotateInfo) {
            ...
            if (rotateHandlesMove) {
              var resizeOffset = _applyAnchorToResizeOffset(
                deltaWidth,
                deltaHeight,
                rotateInfo.anchor
              );
            ...
            _moveSingleWidget(elementId, delta, options);
  ...
  var _getCssForResizingWidget = function (
    elementId,
    eventInfo,
    anchor,
    newWidth,
    ...
    if (stop.instant || stop.end == stop.len) {
      var idToResizeMoveState = _getIdToResizeMoveState(eventInfo);
      if (idToResizeMoveState[elementId])
        idToResizeMoveState[elementId].resizeResult = undefined;
    }
    ...
    if (jobj.css("position") == "fixed") {
      if (obj.fixedHorizontal && obj.fixedHorizontal == "center")
        css["margin-left"] = "+=" + delta.x;
      if (obj.fixedVertical && obj.fixedVertical == "middle")
        css["margin-top"] = "+=" + delta.y;
      return css;
    }
    ...
    if (obj.fixedVertical) {
      if (obj.fixedVertical == "middle") anchor = obj.fixedHorizontal;
        ...
        anchor =
          obj.fixedVertical +
          (obj.fixedHorizontal == "center" ? "" : " " + obj.fixedHorizontal);
    }
    ...
    if (handleMove) {
      if (jobj.css("position") === "absolute") {
        css.left =
          $ax.getNumFromPx(jobj.css("left")) + locationShift.x + delta.x;
  ...
  var _getCssForResizingLayerChild = function (
    elementId,
    anchor,
    layerBoundingRect,
    widthChangedPercent,
    ...
    var newHeight =
      currentSize.height + currentSize.height * heightChangedPercent;
    ...
    if (anchor.indexOf("top") > -1) {
      var topDelta =
        (currentTop - layerBoundingRect.top) * heightChangedPercent;
      if (!resizable && Math.round(topDelta))
        topDelta += currentSize.height * heightChangedPercent;
    } else if (anchor.indexOf("bottom") > -1) {
      if (resizable)
        topDelta =
          (currentTop - layerBoundingRect.bottom) * heightChangedPercent;
      else {
        var bottomDelta =
          Math.round(
            currentTop + currentSize.height - layerBoundingRect.bottom
          ) * heightChangedPercent;
        if (bottomDelta)
          topDelta = bottomDelta - currentSize.height * heightChangedPercent;
        else topDelta = 0;
      }
    } else {
      //center vertical
      if (resizable)
        ...
        var centerTopChange =
          Math.round(childCenterPoint.y - layerBoundingRect.centerPoint.y) *
          heightChangedPercent;
        if (centerTopChange > 0)
          topDelta =
            centerTopChange +
            Math.abs((currentSize.height * heightChangedPercent) / 2);
        else if (centerTopChange < 0)
          topDelta =
            centerTopChange -
            Math.abs((currentSize.height * heightChangedPercent) / 2);
        else topDelta = 0;
    ...
    if (changeLeft) {
      if (anchor.indexOf("left") > -1) {
        var leftDelta =
          (currentLeft - layerBoundingRect.left) * widthChangedPercent;
        if (!resizable && Math.round(leftDelta))
          leftDelta += currentSize.width * widthChangedPercent;
      } else if (anchor.indexOf("right") > -1) {
        if (resizable)
          leftDelta =
            (currentLeft - layerBoundingRect.right) * widthChangedPercent;
        else {
          var rightDelta =
            Math.round(
              currentLeft + currentSize.width - layerBoundingRect.right
            ) * widthChangedPercent;
          if (rightDelta)
            leftDelta = rightDelta - currentSize.width * widthChangedPercent;
          else leftDelta = 0;
        }
      } else {
        //center horizontal
        if (resizable)
          ...
          var centerLeftChange =
            Math.round(childCenterPoint.x - layerBoundingRect.centerPoint.x) *
            widthChangedPercent;
          if (centerLeftChange > 0)
            leftDelta =
              centerLeftChange +
              Math.abs((currentSize.width * widthChangedPercent) / 2);
          else if (centerLeftChange < 0)
            leftDelta =
              centerLeftChange -
              Math.abs((currentSize.width * widthChangedPercent) / 2);
          else leftDelta = 0;
  ...
  _actionHandlers.setPanelOrder = function (eventInfo, actions, index) {
    ...
    for (var i = 0; i < action.panelPaths.length; i++) {
      var func = action.panelPaths[i].setOrderInfo.bringToFront
        ? "bringToFront"
        : "sendToBack";
      var elementIds = $ax.getElementIdsFromPath(
        action.panelPaths[i].panelPath,
        eventInfo
      );
      for (var j = 0; j < elementIds.length; j++)
        $ax("#" + elementIds[j])[func]();
    }

    _dispatchAction(eventInfo, actions, index + 1);
  };

  _actionHandlers.modifyDataSetEditItems = function (
    eventInfo,
    actions,
    index
  ) {
    ...
    for (var i = 0; i < repeaters.length; i++) {
      ...
      if (data.type == "this") {
        var scriptId = $ax.repeater.getScriptIdFromElementId(
  ...
  _actionHandlers.addItemsToDataSet = function (eventInfo, actions, index) {
    ...
    for (var i = 0; i < action.dataSetsToAddTo.length; i++) {
      ...
      for (var j = 0; j < dataset.length; j++)
        $ax.repeater.addItem(id, $ax.deepCopy(dataset[j]), eventInfo);
  ...
  _actionHandlers.deleteItemsFromDataSet = function (
    eventInfo,
    actions,
    index
  ) {
  ...
  _actionHandlers.removeFilterFromRepeater = function (
    eventInfo,
    actions,
    index
  ) {
  ...
  _actionHandlers.addSortToRepeater = function (eventInfo, actions, index) {
    ...
    for (var i = 0; i < action.repeatersToAddSort.length; i++) {
      ...
      $ax.repeater.addSort(
        id,
        addSortInfo.label,
        addSortInfo.columnName,
        addSortInfo.ascending,
        addSortInfo.toggle,
        addSortInfo.sortType,
        addSortInfo.removeAllSorts
      );
      _addRefresh(id);
    }
  ...
  _actionHandlers.removeSortFromRepeater = function (
    eventInfo,
    actions,
    index
  ) {
    ...
    for (var i = 0; i < action.repeatersToRemoveSort.length; i++) {
      ...
      else if (removeSortInfo.sortName != "")
        $ax.repeater.removeSort(id, removeSortInfo.sortName);
      _addRefresh(id);
    }
  ...
  _actionHandlers.setRepeaterToPage = function (eventInfo, actions, index) {
    ...
    for (var i = 0; i < action.repeatersToSetPage.length; i++) {
      ...
      $ax.repeater.setRepeaterToPage(
        id,
        setPageInfo.pageType,
        setPageInfo.pageValue,
        eventInfo
      );
      eventInfo.targetElement = oldTarget;
      _addRefresh(id);
  ...
  _actionHandlers.setItemsPerRepeaterPage = function (
    eventInfo,
    actions,
    index
  ) {
    ...
    for (var i = 0; i < action.repeatersToSetItemCount.length; i++) {
      ...
      if (setItemCountInfo.noLimit) $ax.repeater.setNoItemLimit(id);
        ...
        $ax.repeater.setItemLimit(
          id,
          setItemCountInfo.itemCountValue,
          eventInfo
        );
      _addRefresh(id);
    }
  ...
  _actionHandlers.refreshRepeater = function (eventInfo, actions, index) {
    ...
    for (var i = 0; i < action.repeatersToRefresh.length; i++) {
      // Grab the first one because repeaters must have only element id, as they cannot be inside repeaters
      //  or none if unplaced
      var id = $ax.getElementIdsFromPath(
        action.repeatersToRefresh[i],
        eventInfo
      )[0];
  ...
  _actionHandlers.enableDisableWidgets = function (eventInfo, actions, index) {
    ...
    for (let i = 0; i < action.pathToInfo.length; i++) {
      const elementIds = $ax.getElementIdsFromPath(
        action.pathToInfo[i].objectPath,
        eventInfo
      );
  ...
  _actionHandlers.setImage = function (eventInfo, actions, index) {
    ...
    for (var i = 0; i < action.imagesToSet.length; i++) {
      ...
      var elementIds = $ax.getElementIdsFromPath(
        action.imagesToSet[i].objectPath,
        eventInfo
      );
      ...
      for (var j = 0; j < elementIds.length; j++) {
        ...
        if (evaluatedImgs.mouseOver || evaluatedImgs.mouseDown)
          $ax.event.updateIxStyleEvents(elementId);
      }
    }
    eventInfo.targetElement = oldTarget;
  ...
  var _evaluateImages = function (imgInfo, eventInfo) {
    ...
    for (var state in imgInfo) {
      ...
      var img =
        imgInfo[state][$ax.adaptive.getSketchKey()] ||
        $ax.expr.evaluateExpr(imgInfo[state].literal, eventInfo);
  ...
  $ax.clearRepeaterImageOverrides = function (repeaterId) {
    ...
    for (var i = childIds; i < childIds.length; i++)
      $ax.style.deleteElementIdToImageOverride(childIds[i]);
  };
  ...
  _actionHandlers.setFocusOnWidget = function (eventInfo, actions, index) {
    ...
    if (action.objectPaths.length > 0) {
      ...
      if (elementIds.length > 0) {
        ...
        if (
          action.selectText &&
          !$ax.placeholderManager.isActive(elementIds[0])
        ) {
          ...
          for (var i = 0; i < elementChildren.length; i++) {
            ...
            if (
              elementTagName &&
              (elementTagName.toLowerCase() == "input" ||
  ...
  _actionHandlers.expandCollapseTree = function (eventInfo, actions, index) {
    ...
    for (var i = 0; i < action.pathToInfo.length; i++) {
      ...
      for (var j = 0; j < elementIds.length; j++)
        $ax("#" + elementIds[j]).expanded(pair.expandCollapseInfo.expand);
    }
  ...
  _actionHandlers.other = function (eventInfo, actions, index) {
    ...
    $ax.navigate({
      url:
        ...
        "#other=" +
        encodeURI(action.otherDescription),
      target: "popup",
  ...
  _actionHandlers.fireEvents = function (eventInfo, actions, index) {
    ...
    if (
      obj &&
      $ax.public.fn.IsReferenceDiagramObject(obj.type) &&
      eventInfo.isMasterEvent
    ) {
      rdoId = objId;
      rdo = obj;
    ...
    for (var i = 0; i < action.firedEvents.length; i++) {
      ...
      var targetObjIds = isPage
        ? [rdoId]
        : $ax.getElementIdsFromPath(firedEvent.objectPath, eventInfo);
      for (var j = 0; j < targetObjIds.length; j++) {
        ...
        if (raisedEvents) {
          for (var k = 0; k < raisedEvents.length; k++) {
            var event =
              ...
              targetObj.interactionMap.raised &&
              targetObj.interactionMap.raised[raisedEvents[k]];
            if (event)
              $ax.event.handleEvent(targetObjId, eventInfo, event, false, true);
          }
        }
        ...
        if (firedEventNames) {
          for (k = 0; k < firedEventNames.length; k++) {
            event =
              firedTarget.interactionMap &&
              firedTarget.interactionMap[firedEventNames[k]];
            if (event)
              $ax.event.handleEvent(
                isPage ? "" : targetObjId,
                eventInfo,
                event,
                false,
