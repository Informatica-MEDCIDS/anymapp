$axure.internal(function ($ax) {
  ...
  var _initializePageFragment = function (pageFragment, objIdToObject) {
    ...
    var diagramObjectHelper = function (diagramObject, parent) {
      ...
      if (diagramObject.objects)
        objectArrayHelper(diagramObject.objects, diagramObject);
    };
  ...
  (function () {
    ...
    _initializePageData = function () {
      ...
      _pathToScriptIdHelper(
        _pageData.objectPaths,
        [],
        _pathsToScriptIds,
        scriptIdToPath
      );
      ...
      for (var i = 0; i < _pathsToScriptIds.length; i++) {
        ...
        var diagramObject =
          objIdToObject[packageId + "~" + path[path.length - 1]];
      ...
      var mapIdsToRepeaterAndLayer = function (path, objs, repeaterId) {
        ...
        for (var i = 0; i < objs.length; i++) {
          ...
          if ($ax.public.fn.IsDynamicPanel(obj.type)) {
            for (var j = 0; j < obj.diagrams.length; j++)
              mapIdsToRepeaterAndLayer(
                path,
                obj.diagrams[j].objects,
                repeaterId
              );
    ...
    var _getScriptIdFromPath = function (path, relativeTo, includeLimbo) {
      ...
      if (relativeTo) {
        ...
        if (relativeTo.srcElement) {
          //this is eventInfo
          relativeToScriptId = $ax.repeater.getScriptIdFromElementId(
            relativeTo.srcElement
          );
        ...
        if (relativeToScriptId) {
          ...
          if (!includeMasterInPath)
            relativePath = relativePath.slice(0, relativePath.length - 1);
        } else if (relativeTo instanceof Array) {
          //this is a path
      ...
      return (
        (includeLimbo || !$ax.visibility.isScriptIdLimbo(scriptId)) && scriptId
      );
    };
    ...
    var _getElementIdFromPath = function (path, params, includeLimbo) {
      var scriptId = _getScriptIdFromPath(
        path,
        params.relativeTo,
        includeLimbo
      );
      ...
      if (params.relativeTo && typeof params.relativeTo === "string") {
        if ($jobj(params.relativeTo))
          itemNum = $ax.repeater.getItemIdFromElementId(params.relativeTo);
      }
    ...
    var _getElementsIdFromEventAndScriptId = function (eventInfo, scriptId) {
      var itemId =
        eventInfo && $ax.repeater.getItemIdFromElementId(eventInfo.srcElement);
      var target = false;
      ...
      if (parentRepeater && scriptId != parentRepeater) {
        if (
          ...
          (!eventInfo ||
            parentRepeater ==
              $ax.getParentRepeaterFromScriptId(
                $ax.repeater.getScriptIdFromElementId(
                  target ? eventInfo.targetElement : eventInfo.srcElement
                )
              ))
        ) {
        ...
        for (var i = 0; i < itemIds.length; i++)
          elementIds[i] = $ax.repeater.createElementId(scriptId, itemIds[i]);
        return elementIds;
      }
      return [scriptId];
    };
    ...
    var _getSrcElementIdFromEvent = function (event) {
      ...
      while (
        currentQuery &&
        currentQuery.length &&
        (!$obj(currentQuery.attr("id")) ||
          $jobj(currentQuery.attr("id")).hasClass("text"))
      ) {
    ...
    var _getEventInfoFromEvent = function (
      event,
      ...
      if (event != null) {
        ...
        eventInfo.srcElement =
          elementId || elementId == ""
            ? elementId
            : _getSrcElementIdFromEvent(event);
    ...
    var _getItemInfo = function (elementId) {
      ...
      item.repeater = repeaterInfoCache[repeaterId]
        ? repeaterInfoCache[repeaterId]
        : _getWidgetInfo(repeaterId);
    ...
    var _getWidgetInfo = function (elementId) {
      ...
      widget.name = widget.label = elementQuery.data("label")
        ? elementQuery.data("label")
        : "";
      ...
      var repeaterId = $ax.getParentRepeaterFromScriptId(scriptId);
      if (repeaterId)
        widget.repeater = $ax.public.fn.IsRepeater(obj.type)
          ? widget
          : _getWidgetInfo(repeaterId);
      ...
      if ($ax.public.fn.IsRepeater(obj.type)) {
        widget.visibleitemcount = repeaterIdToItemIds[scriptId]
          ? repeaterIdToItemIds[scriptId].length
          : $ax.repeater.getVisibleDataCount(scriptId);
      ...
      widget.heightchange = function () {
        ...
        var panelSizeChange =
          $ax.dynamicPanelManager.getPanelSizeChange(elementId);
      ...
      widget.cacheProp = function (prop) {
        if (prop == "x" || prop == "y" || prop == "width" || prop == "height") {
          ...
          var boundingRect = $ax("#" + this.elementId).offsetBoundingRect(
            true,
            true
          );
      ...
      if (!panelDiv) {
        ...
        var newDiv = $(
          '<div id="' +
            ...
            '" style="visibility: inherit; position: absolute"><p><span></span></p></div>'
        );
        buttonShape.append(newDiv);

        $ax.style.setAdaptiveStyle(
          id,
          $ax.style.computeAllOverrides(
            id,
            undefined,
            $ax.style.generateState(id),
            adaptiveId
          )
        );
    ...
    $ax.GetParentIdFromLink = function (id) {
      return $ax.GetShapeIdFromText(
        $jobj(id).parentsUntil(".text").parent().attr("id")
      );
    };
    ...
    $ax.GetButtonShapeId = function (id) {
      ...
      switch (obj.type) {
        case $ax.constants.TREE_NODE_OBJECT_TYPE:
          return obj.buttonShapeId
            ? $ax.getElementIdFromPath([obj.buttonShapeId], { relativeTo: id })
            : "";
    ...
    $ax.GetShapeIdFromText = function (id) {
      ...
      while (current && current.tagName != "BODY") {
        ...
        if (currentId && currentId != "base")
          return $ax.visibility.getWidgetFromContainer(currentId);
        current = current.parentElement;
      }
    ...
    $ax.addItemIdToRepeater = function (itemId, repeaterId) {
      ...
      for (var i = 0; i < scriptIds.length; i++)
        elementIdToObject[$ax.repeater.createElementId(scriptIds[i], itemId)] =
          $ax.getObjectFromScriptId(scriptIds[i]);
    };
      ...
      for (var i = 0; i < _scriptIds.length; i++) {
        ...
        if (repeaterId && repeaterId != scriptId) {
          ...
          for (var j = 0; j < itemIds.length; j++)
            elementIds[elementIds.length] = $ax.repeater.createElementId(
              scriptId,
              itemIds[j]
            );
    ...
    $ax.getObjectFromElementId = function (elementId) {
      return $ax.getObjectFromScriptId(
        $ax.repeater.getScriptIdFromElementId(elementId)
      );
    };
    ...
    $ax.getParentRepeaterFromElementId = function (elementId) {
      return $ax.getParentRepeaterFromScriptId(
        $ax.repeater.getScriptIdFromElementId(elementId)
      );
    };
    ...
    $ax.getChildElementIdsForRepeater = function (repeaterId) {
      ...
      for (var i = 0; i < scriptIds.length; i++) {
        for (var j = 0; j < itemIds.length; j++) {
          retVal[retVal.length] = $ax.repeater.createElementId(
            scriptIds[i],
            itemIds[j]
          );
        }
    ...
    $ax.getRdoParentFromElementId = function (elementId) {
      ...
      if ($ax.getParentRepeaterFromScriptId(rdoId))
        rdoId = $ax.repeater.createElementId(
          rdoId,
          $ax.repeater.getItemIdFromElementId(elementId)
        );
      return rdoId;
    };
    ...
    $ax.getLayerParentFromElementId = function (elementId) {
      var itemId = $ax.repeater.getItemIdFromElementId(elementId);
      var scriptId =
        scriptIdToParentLayer[$ax.repeater.getScriptIdFromElementId(elementId)];
      return $ax.getParentRepeaterFromElementId(scriptId)
        ? $ax.repeater.createElementId(scriptId, itemId)
        : scriptId;
    };
  ...
  var _pathToScriptIdHelper = function (
    currentPath,
    currentChain,
    fullPathArray,
    scriptIdToPath
    ...
    for (var key in currentPath) {
      if (key != "scriptId") {
        ...
        fullPathArray[fullPathArray.length] = {
          idPath: currentChain,
          scriptId: currentPath.scriptId,
        };
  ...
  /**
    ...
    */
  $ax.public.navigate = $ax.navigate = function (to) {
    //url, includeVariables, type) {
    var targetUrl;
    if (typeof to === "object") {
      targetUrl = !to.includeVariables
        ? to.url
        : $ax.globalVariableProvider.getLinkUrl(
            to.url,
            to.useGlobalVarNameInUrl
          );
      ...
      if (to.target == "new") {
        ...
        if (!_needsReload(targetLocation, to.url)) {
          ...
          targetLocation.href =
            $axure.utils.getReloadPath() + "#" + encodeURI(targetUrl);
        }
      }
    } else {
      $ax.navigate({
  ...
  var _getPopupFeatures = function (options) {
    ...
    optionsList.push(
      "scrollbars=" + (selectedOptions.scrollbars ? "yes" : "no")
    );
    ...
    optionsList.push(
      "directories=" + (selectedOptions.directories ? "yes" : "no")
    );
    ...
    var targetUrl =
      includeVariables === false
        ...
        : $axure.utils.getReloadPath() +
          "#" +
          encodeURI($ax.globalVariableProvider.getLinkUrl($ax.pageData.url));
  ...
  /**
    ...
    */
  $ax.public.setGlobalVariable = $ax.setGlobalVariable = function (
    name,
    value
  ) {
    if (!name || !value) {
  ...
  $ax.getObjectFromElementIdDisregardHex = function (elementId) {
    var elementIdInput =
      elementId.charAt(0) == "#" ? elementId.substring(1) : elementId;
