...
$axure.internal(function ($ax) {
  ...
  var getVariableValue = function (
    variable,
    eventInfo,
    ignoreDefaultsForLinkUrl
  ) {
    variable = variable.toLowerCase();
    if (_globalVariableValues[variable] !== undefined) {
      ...
      if (
        ignoreDefaultsForLinkUrl == true &&
        $ax.document.globalVariables[variable] ==
          _globalVariableValues[variable]
      ) {
        return null;
      }
    ...
    if ($ax.document.globalVariables[variable] !== undefined)
      return ignoreDefaultsForLinkUrl == true
        ? null
        : $ax.document.globalVariables[variable];
    switch (variable) {
      case "pagename":
      ...
      case "this":
        return (
          eventInfo &&
          eventInfo.thiswidget &&
          $ax.getWidgetInfo(eventInfo.thiswidget.elementId)
        );
      case "item":
        return (
          (eventInfo &&
            eventInfo.item &&
            eventInfo.item.valid &&
            eventInfo.item) ||
          getVariableValue("targetitem", eventInfo, ignoreDefaultsForLinkUrl)
        );
      case "targetitem":
        return (
          eventInfo &&
