(function () {
  if ($axure.player.isCloud) return;
  ...
  function processPagesHint(currentIndex) {
    if (
      $axure.document.sitemap.rootNodes.length > 1 &&
      !$axure.document.configuration.isAxshare
    ) {
      $.get("shouldShowSitemapHint", function (shouldShow) {
        if (shouldShow) {
          var pagesHint = $(
            "<div class='pluginHint left'><span>Click to expand the sitemap and view your project pages.</span></div>"
          );
  ...
  function processNotesHint(currentIndex) {
    var hasNotes =
      ...
      (($axure.page.notes && !$.isEmptyObject($axure.page.notes)) ||
        ($axure.page.masterNotes && $axure.page.masterNotes.length > 0) ||
        ($axure.page.widgetNotes && $axure.page.widgetNotes.length > 0));
    if (hasNotes && !$axure.document.configuration.isAxshare) {
      $.get("shouldShowPageNoteHint", function (shouldShow) {
        if (shouldShow) {
          var notesHint = $(
            "<div class='pluginHint'><span>Click to view the Page and Widget Notes on this page.</span></div>"
          );
  ...
  function processConsoleHint() {
    if (
      !$axure.document.configuration.showConsole ||
      $axure.document.configuration.isAxshare
    )
      return;
    ...
    $.get("shouldShowConsoleHint", function (shouldShow) {
      if (shouldShow) {
        var consoleHint = $(
          "<div class='pluginHint right'><span><b>Reminder:</b> Use the console to see when events and actions are performed.</span></div>"
        );
