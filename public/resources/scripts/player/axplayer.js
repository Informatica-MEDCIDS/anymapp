...
(function () {
  ...
  $axure.loadDocument = function (document) {
    ...
    _settings.isAxshare = false;
    _settings.isExpo =
      configuration.isExpo == null ? false : configuration.isExpo;
  ...
  $(window).bind("load", function () {
    if (
      (CHROME && BROWSER_VERSION < 64) || // First 2018 release
    ...
    if (
      FIREFOX &&
      BROWSER_VERSION >= 68 &&
      document.location.href.indexOf("file://") >= 0
    ) {
      //detecting firefox and local
  ...
  function appendOutOfDateNotification() {
    ...
    toAppend +=
      '   <div style="font-size: 24px; text-align: center; color: #FFFFFF;">LOOKS LIKE YOUR BROWSER IS OUT OF DATE</div>';
    toAppend +=
      '   <div style="font-size: 14px; text-align: center; color: #FFFFFF; margin-bottom: 16px;">This prototype may not look or function correctly until you update your browser</div>';
    toAppend += '   <div id="supportedBrowsersListContainer">';
    toAppend += '       <div class="browserContainer">';
    toAppend +=
      '           <div class="browserName">Google Chrome</div><div class="browserSupportedVersion">v64 and later</div>';
    toAppend += "       </div>";
    toAppend += '       <div class="browserContainer">';
    toAppend +=
      '           <div class="browserName">Mozilla Firefox</div><div class="browserSupportedVersion">v57 and later</div>';
    toAppend += "       </div>";
    toAppend += '       <div class="browserContainer">';
    toAppend +=
      '           <div class="browserName">Microsoft Edge</div><div class="browserSupportedVersion">v15 and later</div>';
    toAppend += "       </div>";
    toAppend += '       <div class="browserContainer">';
    toAppend +=
      '           <div class="browserName">Apple Safari</div><div class="browserSupportedVersion">v10 and later</div>';
    toAppend += "       </div>";
    toAppend += "   </div>";
    toAppend += '   <div id="browserOutOfDateNotificationButtons">';
    if (!MOBILE_DEVICE) {
      toAppend +=
        '       <div style="margin-right: 36px"><a href="http://outdatedbrowser.com/en" id="updateBrowserButton">UPDATE BROWSER</a></div>';
      toAppend +=
        '       <div style="flex: 0 1 45%;"><a id="continueToPrototypeButton">Continue viewing prototype anyway</a></div>';
    } else {
      toAppend +=
        '       <div style="width: 100%; text-align:center"><a id="continueToPrototypeButton">Continue viewing prototype anyway</a></div>';
    }
    toAppend += "   </div>";
    toAppend += "</div>";
  ...
  $(document).on("pluginShown", function (event, data) {
    if (!suppressPluginVarUpdate)
      $axure.player.setVarInCurrentUrlHash(PLUGIN_VAR_NAME, data ? data : "");
  });

  $(document).on("pluginCreated", function (event, data) {
    if (
      !$axure.player.isMobileMode() &&
      $axure.player.settings.startPluginGid.indexOf(data) > -1
    ) {
  ...
  function initializeEvents() {
    ...
    $("#interfacePageNameContainer").on(
      $axure.eventNames.mouseDownName,
      toggleSitemap
    );
    $("#interfaceAdaptiveViewsContainer").on(
      $axure.eventNames.mouseDownName,
      toggleAdaptiveViewsPopup
    );
    $("#overflowMenuButton").on(
      $axure.eventNames.mouseDownName,
      toggleOverflowMenuPopup
    );
    $("#scaleMenuButton").on(
      $axure.eventNames.mouseDownName,
      toggleScaleMenuPopup
    );
  ...
  function initializeMainFrame() {
    ...
    if (SAFARI && document.location.href.indexOf("file://") >= 0) {
      $(mainFrame).on("load", function () {
        ...
        try {
          var mainFrameWindow =
            mainFrame.contentWindow || mainFrame.contentDocument;
  ...
  var isMobileMode = ($axure.player.isMobileMode = function () {
    return (
      $axure.utils.isShareApp() || (MOBILE_DEVICE && $(window).width() < 420)
    );
  });
  ...
  $axure.player.updatePlugins = function updatePlugins() {
    if (MOBILE_DEVICE && !$axure.utils.isShareApp()) {
      ...
      $(".rightPanel .leftPanel .mobileOnlyPanel").css(
        "padding",
        hostPanelPadding
      );
    }
  ...
  function deactivateMobileTextEntry() {
    ...
    var newControlHeight =
      newHeight - (!$axure.utils.isShareApp() ? 140 : IOS ? 157 : 138);
  ...
  function activateMobileTextEntry() {
    ...
    var newControlHeight =
      newHeight - (!$axure.utils.isShareApp() ? 140 : IOS ? 157 : 138);
    newControlHeight =
      newControlHeight + (!$axure.utils.isShareApp() ? 61 : IOS ? 72 : 60);
  ...
  function setAdaptiveView() {
    if (typeof noViewport == "undefined") {
      ...
      if (!isViewOverridden() && !isAnimating)
      ...
      $axure.player.refreshViewPort();
      if ($axure.player.updateAdaptiveViewHeader != null)
        $axure.player.updateAdaptiveViewHeader();
    }
  }
  ...
  $axure.player.resizeContent = function (noViewport) {
    ...
    if (isMobile) {
      ...
      if (
        $("#mHideSidebar").is(":visible") &&
        !$("#mobileControlFrameContainer").is(":visible")
      ) {
      ...
      if ($(".rightPanel").is(":visible")) {
        var lastRightPanelWidthDefaultSub =
          $(window).width() - lastRightPanelWidthDefault || 0;
        var rightPanelWidth = $(".rightPanel").width() || 0;
        var leftPanelPanelWidthSub =
          $(window).width() - $(".leftPanel").width() || 0;

        var newWidth = Math.min(
          lastRightPanelWidthDefaultSub,
          rightPanelWidth,
          leftPanelPanelWidthSub
        );
        lastRightPanelWidth = Math.max(lastRightPanelWidthDefault, newWidth);
        $(".rightPanel").width(
          lastRightPanelWidth ? lastRightPanelWidth : lastRightPanelWidthDefault
        );
        $("#rsplitbar").css(
          "left",
          $(window).width() - $(".rightPanel").width()
        );
      }
      if ($(".leftPanel").is(":visible")) {
        var lastLeftPanelWidthSub =
          $(window).width() - lastLeftPanelWidthDefault || 0;
        var leftPanelWidth = $(".leftPanel").width() || 0;
        var rightPanelWidthSub =
          $(window).width() - $(".rightPanel").width() || 0;

        var newWidth = Math.min(
          lastLeftPanelWidthSub,
          leftPanelWidth,
          rightPanelWidthSub
        );
        ...
        $(".leftPanel").width(
          lastLeftPanelWidth ? lastLeftPanelWidth : lastLeftPanelWidthDefault
        );
  ...
  function contentDocument_onload() {
    (function setRepositionWhenReady() {
      ...
      if (
        $($iframe.contentWindow.document.body).length === 0 ||
        $iframe.contentWindow.document.URL === "about:blank"
      ) {
  ...
  function panAndZoomKeydownHandler(e) {
    ...
    if (e.key == " ") {
      ...
      if (
        (e.target !== iframeBody &&
          (iframeBody.contains(e.target) ||
            tabbableElements.lastIndexOf(e.target.tagName.toLowerCase()) !==
              -1)) ||
        isDevice
      ) {
        return;
      ...
      toggleDragMode(true);
      // self-destruction
      dragOverlayTimeout = setTimeout(function () {
        toggleDragMode(false);
      }, selfDestructionTimeoutDuration);
    }
    // if hotkey zooming
    ...
    if (e.key == "Control" || e.key == "Meta") {
      ...
      // self-destruction
      zoomOverlayTimeout = setTimeout(function () {
  ...
  function zoomWheelHandler(event) {
    if (event.ctrlKey || event.metaKey) {
      ...
      if (wheelEvent) {
        ...
        if (
          disableChunkedZoomEvents &&
          timestamp - zoomLastTimestamp < 0.4 &&
          direction == zoomLastDirection
        ) {
        ...
        const zoomStep =
          deltaY < 0 ? zoom * 0.0409 + 0.0415 : zoom * 0.0394 + 0.0367;
  ...
  function zoomGestureChange(e) {
    ...
    gestureRaf = requestAnimationFrame(function () {
      ...
      if (
        disableChunkedZoomEvents &&
        timestamp - zoomLastTimestamp < 0.5 &&
        direction == zoomLastDirection
      ) {
        // ignore chunked events
        return;
    ...
    if (!event.ctrlKey && !event.metaKey) {
      ...
      if (wheelEvent) {
        ...
        let deltaY = wheelEvent.deltaY
          ? -wheelEvent.deltaY * deltaFactor * scrollStep
          : 0;
        let deltaX = wheelEvent.deltaX
          ? -wheelEvent.deltaX * deltaFactor * scrollStep
          : 0;
  ...
  function common_contentDocument_onload() {
    (function bindHandlersWhenReady() {
      ...
      if (
        ...
        mainPanel.removeEventListener("wheel", zoomWheelHandler, {
          passive: false,
        });
        mainPanel.removeEventListener("wheel", scrollWheelHandler, {
          passive: false,
        });
        ...
        mainPanel.addEventListener("wheel", zoomWheelHandler, {
          passive: false,
        });
        mainPanel.addEventListener("wheel", scrollWheelHandler, {
          passive: false,
        });

        iframeWindow.addEventListener("keydown", panAndZoomKeydownHandler);
        iframeWindow.addEventListener("keyup", panAndZoomKeyupHandler);
        iframeWindow.addEventListener("wheel", zoomWheelHandler, {
          passive: false,
          capture: true,
        });
        ...
        if (iframeWindow) {
          ...
          // remove already added listeners
          window.removeEventListener("gesturestart", zoomGestureStart, {
            capture: true,
          });
          window.removeEventListener("gesturechange", zoomGestureChange, {
            capture: true,
          });
          window.removeEventListener("gestureend", zoomGestureEnd, {
            capture: true,
          });
          ...
          window.addEventListener("gesturestart", zoomGestureStart, {
            capture: true,
          });
          window.addEventListener("gesturechange", zoomGestureChange, {
            capture: true,
          });
          window.addEventListener("gestureend", zoomGestureEnd, {
            capture: true,
          });

          iframeWindow.addEventListener("gesturestart", zoomGestureStart, {
            capture: true,
          });
          iframeWindow.addEventListener("gesturechange", zoomGestureChange, {
            capture: true,
          });
          iframeWindow.addEventListener("gestureend", zoomGestureEnd, {
            capture: true,
          });
        }
      }
  ...
  var zoomValues = ($axure.player.zoomValues = [
  ...
  var getPageSize = function () {
    ...
    return {
      ...
      width:
        ($($("#mainFrame")[0].contentDocument).find("body").width() || 0) *
        currentZoom,
    };
  };
  ...
  var getPagePosition = ($axure.player.getPagePosition = function () {
    ...
    var pageX =
      -screenContainerPosition.x * currentZoom -
      (viewerSize.width > pageSize.width
        ? pageSize.width / 2
        : viewerSize.width / 2);
    var pageY =
      -screenContainerPosition.y * currentZoom - viewerSize.height / 2;
  ...
  var setPagePosition = ($axure.player.setPagePosition = function (position) {
    ...
    const screenContainerLeft =
      viewerSize.width / 2 > Math.abs(position.x)
        ? 0
        : -position.x - viewerSize.width / 2;
    const screenContainerTop = -position.y - viewerSize.height / 2;

    setPageScrollPosition({
      x: screenContainerLeft / currentZoom,
      y: screenContainerTop / currentZoom,
    });
  });
  ...
  var getNextStepOfZoom = function (offset) {
    ...
    const currentZoomIndex =
      zoomValues.findIndex(function (value) {
        return value === nearZoom;
      }) || 0;
  ...
  function normalizeZoomWheelEvent(e) {
    ...
    if (isMac()) {
      ...
      if (CHROME || $axure.browser.isEdge) {
        ...
        deltaY = -wheelDeltaY;
        lowResMouse =
          Math.abs(deltaX) >= lowResCutoff || Math.abs(deltaY) >= lowResCutoff;
        if (lowResMouse) {
          deltaX /= 120;
          deltaY /= 120;
        }
  ...
  var toggleDragMode = function (value) {
    ...
    if (value) {
      viewer.on("mousedown.drag", function (event) {
        startDrag(event);
      });
    } else {
  ...
  var startDrag = function (event) {
    ...
    $($("#mainFrame")[0].contentDocument)
      .find("#dragOverlay")
      .toggleClass("dragging__start", true);
    viewer.on("mouseup.drag", function () {
      stopDrag();
    });
    viewer.on("mouseout.drag mousemove.drag", function (e) {
      move(e);
    });
  };
  ...
  var stopDrag = function () {
    ...
    $($("#mainFrame")[0].contentDocument)
      .find("#dragOverlay")
      .toggleClass("dragging__start", false);
  ...
  var determineIframePosition = function () {
    ...
    var bodyWidth =
      $body.offset().left !== 0 ? $body.width() : dimensions.width;
  ...
  function calculateClippingBoundsWidth(panelSize, isLeftPanel) {
    ...
    var leftPanelOffset =
      !isMobileMode() && $leftPanel.length > 0 && !$leftPanel.hasClass("popup")
        ? $leftPanel.width()
        : 0;

    var $rightPanel = $(".rightPanel:visible");
    var rightPanelOffset =
      !isMobileMode() && $rightPanel.length > 0 ? $rightPanel.width() : 0;
  ...
  var updateClippingBoundsWidth = ($axure.player.updateClippingBoundsWidth =
    function () {
      if ($(".leftPanel").is(":visible"))
        $("#clippingBounds").css("left", $(".leftPanel").width());
  ...
  function calculateClippingBoundsScrollPosition() {
    ...
    scale =
      scale == "none"
        ? 1
        : Number(scale.substring(scale.indexOf("(") + 1, scale.indexOf(",")));

    // Iframe and Main Panel Positioning
    var iframeScroll = determineIframeScroll();
    var iframePos = determineIframePosition();
    var viewablePanelLeftMargin = parseInt(
      $("#mainPanelContainer").css("margin-left")
    );
    ...
    if (scaleVal == 2) {
      ...
      viewablePanelLeftMargin =
        ($("#mainPanel").width() - $("#mainPanelContainer").width() * scale) /
        2;
      viewablePanelTop =
        ($("#mainPanel").height() - $("#mainPanelContainer").height() * scale) /
        2;
    }
    ...
    var leftPos =
      viewablePanelLeftMargin +
      (iframePos.left - iframeScroll.scrollLeft) * scale;
    ...
    else if (isCentered && scaleVal == 2)
      leftPos =
        ($("#mainPanelContainer").width() * scale) / 2.0 -
        contentLeftOfOriginOffset;
  ...
  function repositionClippingBoundsScroll() {
    ...
    (function repositionWhenReady() {
      if ($($("#mainFrame")[0].contentWindow.document.body).length === 0) {
        ...
        position.left =
          position.left -
          $("#mainPanel").scrollLeft() -
          $("#clipFrameScroll").scrollLeft();
        position.top =
          position.top -
          $("#mainPanel").scrollTop() -
          $("#clipFrameScroll").scrollTop();
  ...
  function calculateScrollLeftWithOffset(offset, isLeftPanel) {
    ...
    var scaleVal = $(".vpScaleOption")
      .find(".selectedRadioButton")
      .parent()
      .attr("val");
    ...
    var dimStr = $(".currentAdaptiveView").attr("data-dim");
    var hasFrame =
      (!dimStr ? false : dimStr.split("x")[1] != "0") && !$axure.player.noFrame;
    ...
    if (hasFrame) {
      ...
      var viewablePanelRightMargin = parseInt(
        $("#mainPanelContainer").css("margin-right")
      );
    ...
    if (isCentered) {
      ...
      var clippedContentWidth =
        $body.width() -
        calculateClippingBoundsWidth(Math.abs(offset), isLeftPanel);
      ...
      if (!isMobileMode()) {
        ...
        $(".leftPanel").animate(
          { "margin-left": -newWidth + "px" },
          {
            duration: 200,
            complete: function () {
              $(".leftPanel").width(0).hide().css({ marginLeft: "" });
            },
          }
        );
        $("#lsplitbar").animate(
        ...
        $("#clippingBounds").animate(
          { left: "", width: clippingWidth + "px" },
          { duration: 200 }
        );
        $("#clippingBoundsScrollContainer").animate(
          { left: newLeft + "px" },
          {
            duration: 200,
            complete: function () {
      ...
      if (!isMobileMode()) {
        ...
        $(".rightPanel").animate(
          { "margin-right": -newWidth + "px" },
          {
            duration: 200,
            complete: function () {
              $(".rightPanel").width(0).hide().css({ marginRight: "" });
            },
          }
        );
        $("#rsplitbar").animate(
  ...
  $axure.player.expandFromBar = function (
    hostId,
    context,
    isFinalPluginToRestore
  ) {
    if (context === "project") {
      ...
      if (!isMobileMode()) {
        isAnimating = true;
        var newWidth = lastLeftPanelWidth
          ? lastLeftPanelWidth
          : lastLeftPanelWidthDefault;
        ...
        $("#clippingBounds").animate(
          { left: newWidth + "px", width: clippingWidth + "px" },
          { duration: 200 }
        );
        $("#clippingBoundsScrollContainer").animate(
          { left: newLeft + "px" },
      ...
      if ($("#rsplitbar").is(":visible")) {
        ...
        var newWidth = lastRightPanelWidth
          ? lastRightPanelWidth
          : lastRightPanelWidthDefault;
      ...
      if (!isMobileMode()) {
        ...
        $("#rsplitbar").show();
        $("#rsplitbar").animate(
          { left: $(window).width() - $(".rightPanel").width() + "px" },
          { duration: 200 }
        );
  ...
  $axure.player.refreshViewPort = function () {
    ...
    prevScaleN =
      prevScaleN == "none"
        ? 1
        : Number(
            prevScaleN.substring(
              prevScaleN.indexOf("(") + 1,
              prevScaleN.indexOf(",")
            )
          );
    ...
    var $leftPanel = $(".leftPanel:visible");
    var leftPanelOffset =
      !isMobileMode() && $leftPanel.length > 0 ? $leftPanel.width() : 0;
    var $rightPanel = $(".rightPanel:visible");
    var rightPanelOffset =
      !isMobileMode() && $rightPanel.length > 0 ? $rightPanel.width() : 0;
  ...
  function setUserLoggedInStatus(response, safariAuthResponseProfile) {
    if (!response.success) {
      ...
      if (safariAuthResponseProfile)
        userAcct.userName =
          response.username == null || response.username.trim() === ""
            ? response.userEmail
            : response.username.trim();
      else
        userAcct.userName =
          response.nickname == null || response.nickname.trim() === ""
            ? response.userEmail
    ...
    if (typeof feedback !== "undefined")
      feedback.updateUserAccountInfo(
        userAcct,
        authCookieValue,
        userCookieValue
      );
              ...
              success: function (response) {
                if (!response.success) {
                  ...
                  setUserLoggedInStatus(
                    response,
                    response.data.profile[userAcct.userId]
                  );
  ...
  $axure.player.axureLogin = function axureLogin(
    email,
    password,
    success,
    failure,
  ...
  function playerLogout() {
    ...
    // If feedback is loaded, update feedback with new account information
    if (typeof feedback !== "undefined")
      feedback.updateUserAccountInfo(userAcct);
  }
  ...
  function buildAccountLoginPopup() {
    return [
      ...
      '           <span>No account? <a class="axureSignUpLink" href="',
      window.AXSHARE_HOST_SECURE_URL,
      ...
      $axure.player.axureLogin(
        ...
        function (response) {
          ...
          $container
            .find(".errorMessage")
            .append(
              "Please <a class='refreshLink' style='text-decoration: underline;'>refresh</a> this page after logging in via your identity provider."
            );
          $container.find(".errorMessage").show();
  ...
  function initializeSignIn() {
    if (
      typeof PREVIEW_INFO === "undefined" &&
      $axure.player.settings.isAxshare
    ) {
      (function finishInit() {
        if (
          window.$axshare == null ||
          ...
          $.ajax({
            ...
            success: function (response) {
              ...
              if (isSubInstance)
                $("#accountLoginContainer").find("#playerSignUpLink").hide();
              ...
              if (typeof feedback !== "undefined") {
                feedback.setReadOnlyModeAndMessage(
                  response.readOnlyMode,
                  response.readOnlyMessage
                );
  ...
  function overflowProvidesScroll(node) {
    ...
    var overflowIsHidden =
      style.overflow === "hidden" ||
      (style.overflowX === "hidden" && style.overflowY === "hidden");
    var overflowIsVisible =
      style.overflow === "visible" ||
      (style.overflowX === "visible" && style.overflowY === "visible");
    var mozTextareaOverflowVisible =
      FIREFOX && overflowIsVisible && node instanceof HTMLTextAreaElement;
    return (
      (!overflowIsHidden && !overflowIsVisible) || mozTextareaOverflowVisible
    );
  }
  ...
  function findNearestScrollableParent(firstNode) {
    ...
    while (!scrollable && node) {
      if (
        ...
        if (
          overflowProvidesScroll(node) ||
          $(node).css("-webkit-overflow-scrolling") === "touch"
        ) {
          scrollable = node;
        }
      }
  ...
  function getScrollOwner(target) {
    ...
    if (
      ...
      owner === document.body ||
      $(owner).parents("#topPanel").length ||
      owner == document.getElementById("forwardSlash")
    ) {
      return null;
    }
  ...
  function removeElasticScrollFromIframe() {
    var $iframe = $($("#mainFrame")[0].contentWindow);
    $iframe[0].document.body.addEventListener(
      "touchmove",
      function (event) {
  ...
  function appendProjectOptions() {
    ...
    toAppend +=
      '       <div id="projectOptionsShowHotspots" class="mobileText projectOptionsHotspotsRow" style="border-bottom: solid 1px #c7c7c7">';
    toAppend += '           <div id="projectOptionsHotspotsCheckbox"></div>';
    toAppend += "       Show Hotspots</div> ";
    toAppend +=
      '       <div class="mobileSubHeader" style="margin-top: 16px">Scale</div>';
    toAppend +=
      '       <div id="projectOptionsScaleContainer" class="mobileText"></div>';
    toAppend +=
      '       <div id="projectOptionsAdaptiveViewsHeader" class="mobileSubHeader" style="margin-top: 16px">Adaptive Views</div>';
    toAppend +=
      '       <div id="projectOptionsAdaptiveViewsContainer" class="mobileText"></div>';
    toAppend += "    </div>";
    toAppend += "</div>";
  ...
  function appendMobileBrowserControlFrame() {
    ...
    toAppend +=
      '<div id="mobileBrowserControlFrame" class="mobilePrototypeControlFrame">';
    toAppend +=
      '    <div id="return" style="width:100%; position:relative; top:-15px; float:left">';
    toAppend += '        <div id="closeBackground" class="circleBackground">';
    toAppend +=
      '           <div id="forwardSlash" class="closeIconSlash"><div id="backwardSlash" class="closeIconSlash"></div></div>';
    toAppend += "        </div>";
    toAppend += "    </div>";
    toAppend += "</div>";
    ...
    if (IOS)
      $("#mobileControlFrameContainer").on(
        $axure.eventNames.mouseDownName,
        function (e) {
          e.stopPropagation();
        }
  ...
  function appendNativePrototypeControlFrame() {
    var toAppend = "";
    toAppend +=
      '<div id="nativeAppControlFrame" class="mobilePrototypeControlFrame">';
    ...
    toAppend +=
      '                   <div id="exitText" class="nativeMenuText">Exit</div>';
    toAppend += "               </div>";
    toAppend += "           </div>";
    toAppend += "        </li>";
    toAppend +=
      '        <li id="return" style="width:40%; position:relative; top:-15px; float:left">';
    toAppend +=
      '            <div id="returnBackground" class="circleBackground">';
    toAppend += '                <div id="returnIcon"></div>';
    toAppend += "            </div>";
    toAppend +=
      '            <div id="returnText" class="nativeMenuText">Return to Prototype</div>';
    toAppend += "        </li>";
    ...
    toAppend +=
      '                   <div id="refreshText" class="nativeMenuText">Refresh</div>';
    toAppend += "               </div>";
    toAppend += "           </div>";
    toAppend += "        </li>";
    toAppend += "    </ul>";
    ...
    document.getElementById("nativeMenuBlueBackground").style.height =
      barHeight;
  ...
  function addAppButtonClickListener(id) {
    var func = function () {
      IOS
        ? window.webkit.messageHandlers.prototypeMenuButtonClick.postMessage(id)
        : ShareApp.PrototypeMenuButtonClick(id);
    };
  ...
  function startLeftSplit() {
    ...
    $(document)
      .bind($axure.eventNames.mouseMoveName, doLeftSplitMove)
      .bind($axure.eventNames.mouseUpName, endLeftSplitMove);
  }
  ...
  function startRightSplit() {
    ...
    $('<div class="splitterMask"></div>').insertAfter($left);
    $(document)
      .bind($axure.eventNames.mouseMoveName, doRightSplitMove)
      .bind($axure.eventNames.mouseUpName, endRightSplitMove);
  }

  function doLeftSplitMove() {
    var currentX = window.event.pageX;
    var newWidth = Math.min(
      startSplitWidth + currentX - startSplitX,
      $(window).width() - $(".rightPanel").width(),
      $(window).width() - lastRightPanelWidthDefault
    );
    lastLeftPanelWidth = Math.max(lastLeftPanelWidthDefault, newWidth);
    $(".leftPanel").width(
      lastLeftPanelWidth ? lastLeftPanelWidth : lastLeftPanelWidthDefault
  ...
  function doRightSplitMove() {
    ...
    var newWidth = Math.min(
      ...
      $(window).width() - lastLeftPanelWidthDefault
    );
    lastRightPanelWidth = Math.max(lastRightPanelWidthDefault, newWidth);
    $(".rightPanel").width(
      lastRightPanelWidth ? lastRightPanelWidth : lastRightPanelWidthDefault
  ...
  function endLeftSplitMove() {
    ...
    $(document)
      .unbind($axure.eventNames.mouseMoveName, doLeftSplitMove)
      .unbind($axure.eventNames.mouseUpName, endLeftSplitMove);
    setAdaptiveView();
  }
  ...
  function endRightSplitMove() {
    ...
    $left.removeClass("active");
    $(document)
      .unbind($axure.eventNames.mouseMoveName, doRightSplitMove)
      .unbind($axure.eventNames.mouseUpName, endRightSplitMove);
    setAdaptiveView();
  }
  ...
  var getMaxMLeft = function () {
    if ($(".rightPanel.mobileMode").length == 0)
      return $(".leftPanel.mobileMode").last().position().left + 100;
  ...
  function startM(e) {
    ...
    $(document)
      .bind($axure.eventNames.mouseMoveName, doMMove)
      .bind($axure.eventNames.mouseUpName, endMMove);
  ...
  function endMMove(e) {
    $("div.splitterMask").remove();
    $(document)
      .unbind($axure.eventNames.mouseMoveName, doMMove)
      .unbind($axure.eventNames.mouseUpName, endMMove);
    e.stopPropagation();
  ...
  function adjustM(direction, animate) {
    ...
    if (!$m.is(":visible") || newLeft > -100) {
      ...
      if (!handled) {
        $m.animate(
          { left: -$panels.last().position().left + 25 + "px" },
          duration
        );
      }
  ...
  function repositionPinsOnScaleChange(data) {
    ...
    if (typeof data.contentOriginOffset !== "undefined")
      contentLeftOfOriginOffset = data.contentOriginOffset;
  }

  function messageCenter_message(message, data) {
    if (message == "expandFrame") expand();
    else if (
      message == "getCollapseFrameOnLoad" &&
      $axure.player.settings.startCollapsed &&
      !MOBILE_DEVICE
    )
      $axure.messageCenter.postMessage("collapseFrameOnLoad");
      ...
      if (data.clipToView) {
        ...
        if (scaleVal == "2" || (!MOBILE_DEVICE && scaleVal == "0")) {
          ...
          $("#mainPanelContainer").css({
            transform: scale,
            transformOrigin:
              Number($("#mainPanelContainer").css("width").replace("px", "")) /
                2 +
              "px 0px",
          });
          var mainPanelHeight = Number(
            $("#mainPanel").css("height").replace("px", "")
          );
          var mainPanelContainerHeight = Number(
            $("#mainPanelContainer").css("height").replace("px", "")
          );
          var height =
            mainPanelHeight / data.scaleN > mainPanelContainerHeight
              ? mainPanelContainerHeight
              : mainPanelHeight / data.scaleN;
          var y =
            (mainPanelHeight -
              (mainPanelHeight > mainPanelContainerHeight * data.scaleN
                ? mainPanelContainerHeight * data.scaleN
                : mainPanelHeight)) /
            2;
          ...
          var mainPanelWidth = Number(
            $("#mainPanel").css("width").replace("px", "")
          );
          var mainPanelContainerWidth = Number(
            $("#mainPanelContainer").css("width").replace("px", "")
          );
          var width =
            mainPanelWidth / data.scaleN > mainPanelContainerWidth
              ? mainPanelContainerWidth
              : mainPanelWidth / data.scaleN;
          var x =
            (mainPanelContainerWidth -
              (mainPanelWidth / data.scaleN > mainPanelContainerWidth
                ? mainPanelContainerWidth
        ...
        if (data.scaleN != 1) {
          ...
          var height =
            Number($("#mainPanelContainer").css("height").replace("px", "")) /
              data.scaleN +
            "px";
          $("#mainPanelContainer").css({
            transform: scale,
      ...
      if (forcePanZoomDisabled) {
        ...
        $axure.messageCenter.postMessage("cloud_SetPageScale", {
          scale: resultScale,
        });
      }
      ...
      if ($axure.browser.isEdge) {
        newHeight =
          window.innerHeight -
          (!isMobileMode() && $("#topPanel").is(":visible")
            ? $("#topPanel").height()
            : 0);
        newWidth = $(window).width();
        $("#outerContainer").height(newHeight).width(newWidth);
  ...
  function loadVariablesFromUrl(removeVarFromUrl) {
    ...
    if (query.length > 0) {
      ...
      if (removeVarFromUrl) {
        originalHashValues = originalHashValues
  ...
  var getPageUrlsById = ($axure.player.getPageUrlsById = function (
    packageId,
    foundById,
    nodes
  ) {
  ...
  function closePlayer() {
    if ($axure.page.location) window.location.href = $axure.page.location;
      ...
      window.location.href =
        currentLocation.substr(0, currentLocation.lastIndexOf("/") + 1) +
        pageFile;
    }
  }
  ...
  $axure.player.setVarInCurrentUrlHash = function (varName, varVal) {
    var newHash = $axure.utils.setHashStringVar(
  ...
  function deleteHashStringVar(currentHash, varName) {
    ...
    if (pageIndex != -1) {
      ...
      if (newHash == "") {
        //beginning of string
        newHash =
          ampIndex != -1 ? "#" + currentHash.substring(ampIndex + 1) : "";
      } else {
        //somewhere in the middle
        newHash =
          newHash + (ampIndex != -1 ? currentHash.substring(ampIndex) : "");
      }
  ...
  function setUpController() {
    ...
    _player.createPluginHost = function (settings) {
      ...
      if (settings.id == "feedbackHost")
        $("#overflowMenuContainer").prepend(
          '<div id="showCommentsOption" class="showOption" style="order: 2"><div class="overflowOptionCheckbox"></div>Show Comments</div>'
        );
      ...
      hostContainerId = _player.isMobileMode()
        ? "mHideSidebar"
        : "outerContainer";
      ...
      if (settings.context == "project") {
        ...
        if (
          _player.isMobileMode() &&
          $("#" + hostContainerId).find("#projectOptionsHost").length > 0
        ) {
          host = $(
            '<div id="' + settings.id + '" class="' + panelClass + '"></div>'
        ...
        if (!$("#separatorContainer").hasClass("hasLeft"))
          $("#separatorContainer").addClass("hasLeft");
        var closeButtonContainer = $(
          '<div class="closeButtonContainer"></div>'
        );
      ...
      var style =
        IE || $axure.browser.isEdge ? '" style="border-radius: 0' : "";
      var headerLink = $(
        '<a pluginId="' +
          settings.id +
          '" title="' +
          ...
          style +
          '" >' +
          (settings.context === "inspect" ? "<span>" + "</span>" : "&nbsp;") +
          "</a>"
      );
      headerLink
        .mousedown(
          $axure.utils.curry(interfaceControlHeaderButton_click, settings.id)
        )
        .wrap(
          '<li id="' +
            settings.id +
            'Btn"' +
            (settings.id == "handoffHost" ? ' style="display: none"' : "") +
            ">"
        );
    ...
    var interfaceControlHeaderButton_click = function (id) {
      ...
      var clickedPlugin = $(
        "#" + controlContainerId + " a[pluginId=" + id + "]"
      );
      if (currentVisibleHostId[context] == id) {
        ...
        $axure.player.pluginVisibleChanged(
          currentVisibleHostId[context],
          false
        );
    ...
    _player.restorePlugins = function () {
      ...
      for (var id in plugins) {
        var clickedPlugin = $(
          "#" + getControlContainerId(id) + " a[pluginId=" + id + "]"
        );
        if (clickedPlugin.hasClass("selected")) selectedPluginsCount++;
      }
      if ($axure.player.settings.isAxshare && selectedPluginsCount != 0)
        $("#clippingBoundsScrollContainer").hide();
      ...
      for (var id in plugins) {
        ...
        if (clickedPlugin.hasClass("selected")) {
          ...
          _player.expandFromBar(
            id,
            context,
            selectedPluginsCount == selectedPluginsSeen
          );
        } else {
          $("#" + id).hide();
        }
  ...
  $axure.player.addDeviceFraming = function (project, isEdit) {
    ...
    var $overlayParent = $(window.parent.parent.document).find(
      "#previewPlayerDiv"
    );
    $overlayParent =
      isEdit && $overlayParent.length !== 0
        ? $overlayParent
        : $("#mainPanelContainer");
