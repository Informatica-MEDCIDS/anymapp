...
(function () {
  ...
  $axure.utils.loadCSS = function (url) {
    $("head").append(
      '<link text="text/css" href="' + url + '" rel="Stylesheet" />'
    );
  };

  $axure.utils.loadJS = function (url) {
    $("head").append(
      '<script text="text/javascript" language="JavaScript" src="' +
        url +
        '"></script>'
    );
  };
  ...
  $axure.utils.isInPlayer = function () {
  ...
  $axure.utils.isShareApp = function () {
  ...
  $axure.utils.setHashStringVar = function (currentHash, varName, varVal) {
    ...
    if (pageIndex != -1) {
      ...
      if (ampIndex != -1) {
        newHash =
          newHash == ""
            ? "#" + currentHash.substring(ampIndex + 1)
            : newHash + currentHash.substring(ampIndex);
      }
      hashToSet = newHash;
