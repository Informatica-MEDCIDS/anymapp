...
(function () {
  ...
  if (
    !$axure.document.configuration.showPageNotes &&
    !$axure.document.configuration.showAnnotationsSidebar &&
    !$axure.document.configuration.showAnnotations
  ) {
    return;
  }
  ...
  $(window.document).ready(function () {
    ...
    if ($axure.document.configuration.showAnnotations) {
      $("#overflowMenuContainer").prepend(
        '<div id="showNotesOption" class="showOption" style="order: 3"><div class="overflowOptionCheckbox"></div>Show Note Markers</div>'
      );
    }
    ...
    function populateNotes(pageForNotes) {
      ...
      if ($axure.document.configuration.showPageNotes) {
        ...
        function populatePageNotes(pageOrMaster) {
          ...
          if (notes && !$.isEmptyObject(notes)) {
            ...
            for (var noteName in notes) {
              ...
              if (showNames) {
                pageNoteUi +=
                  "<div class='pageNoteName'>" + noteName + "</div>";
              }
              pageNoteUi +=
                "<div class='pageNote'>" + linkify(notes[noteName]) + "</div>";
              pageNoteUi += "</div>";
        ...
        if (pageNoteUi.length > 0) {
          ...
          var pageNotesHeader =
            "<div id='pageNotesSectionHeader' class='notesSectionHeader pluginNameHeader'>Page Notes</div>";
      ...
      if ($axure.document.configuration.showAnnotationsSidebar) {
        ...
        function populateWidgetNotes(widgetNotes) {
          if (widgetNotes) {
            for (var i = 0; i < widgetNotes.length; i++) {
              ...
              widgetNoteUi +=
                "<div class='widgetNoteContainer' data-id='" +
                widgetNote["ownerId"] +
                "'>";
              widgetNoteUi +=
                "<div class='widgetNoteFootnote'>" +
                widgetNote["fn"] +
                "</div>";
              widgetNoteUi +=
                "<div class='widgetNoteLabel'>" +
                widgetNote["label"] +
                "</div>";

              for (var widgetNoteName in widgetNote) {
                if (
                  widgetNoteName != "label" &&
                  widgetNoteName != "fn" &&
                  widgetNoteName != "ownerId"
                ) {
                  widgetNoteUi +=
                    "<div class='pageNoteName'>" + widgetNoteName + "</div>";
                  widgetNoteUi +=
                    "<div class='pageNote'>" +
        ...
        if (widgetNoteUi.length > 0) {
          var widgetNotesHeader =
            "<div id='widgetNotesSectionHeader' class='notesSectionHeader pluginNameHeader'>Widget Notes</div>";
          ...
          $(".widgetNoteContainer").click(function () {
            ...
            $(".widgetNoteContainerSelected").removeClass(
            ...
            var leftPanelOffset =
              !$axure.player.isMobileMode() && $leftPanel.length > 0
                ? $leftPanel.width()
                : 0;
            var $rightPanel = $(".rightPanel:visible");
            var rightPanelOffset =
              !$axure.player.isMobileMode() && $rightPanel.length > 0
                ? $rightPanel.width()
                : 0;
            var viewDimensions = {
              h: h != "0" ? h : "",
              scaleVal: $(".vpScaleOption")
  ...
  function getWidgetNotesHtml(ownerId, page) {
    ...
    widgetNoteUi +=
      "<div data-ownerid='" + ownerId + "' class='closeNotesDialog'></div>";
      ...
      for (var i = 0; i < widgetNotes.length; i++) {
        ...
        if (widgetNote["ownerId"] == ownerId) {
          widgetNoteUi +=
            ...
            "'>";
          widgetNoteUi +=
            "<div class='widgetNoteFootnote'>" + widgetNote["fn"] + "</div>";
          widgetNoteUi +=
            "<div class='widgetNoteLabel'>" + widgetNote["label"] + "</div>";
  ...
  var _toggleAnnDialog = function (id, srcLeft, srcTop, page) {
    ...
    $dialog
      .find(".notesDialogScroll")
      .niceScroll({ cursorcolor: "#8c8c8c", cursorborder: "0px solid #fff" });

    $dialog
      .find(".notesDialogScroll")
      .on($axure.eventNames.mouseDownName, function (event) {
        event.stopPropagation();
      });

    $dialog
      .find(".closeNotesDialog")
      .on($axure.eventNames.mouseDownName, function (event) {
        event.stopPropagation();
      });
    ...
    function startDialogMove() {
      ...
      $(document)
        .bind($axure.eventNames.mouseMoveName, doDialogMove)
        .bind($axure.eventNames.mouseUpName, endDialogMove);
    ...
    function endDialogMove() {
      ...
      $(document)
        .unbind($axure.eventNames.mouseMoveName, doDialogMove)
        .unbind($axure.eventNames.mouseUpName, endDialogMove);
    ...
    $dialog
      .find(".resizeNotesDialog")
      .on($axure.eventNames.mouseDownName, startDialogResize);
    ...
    function startDialogResize() {
      ...
      $('<div class="splitterMask"></div>').insertAfter($("#notesOverlay"));
      $(document)
        .bind($axure.eventNames.mouseMoveName, doDialogResize)
        .bind($axure.eventNames.mouseUpName, endDialogResize);
    ...
    function endDialogResize() {
      ...
      $dialog.removeClass("active");
      $(document)
        .unbind($axure.eventNames.mouseMoveName, doDialogResize)
        .unbind($axure.eventNames.mouseUpName, endDialogResize);
  ...
  function clearSelection() {
    var selectedNote = $("#pageNotesContainer").find(
      ".widgetNoteContainerSelected"
    );
  ...
  function generatePageNotes() {
    ...
    pageNotesUi +=
      "<div id='pageNotesEmptyState' class='emptyStateContainer'><div class='emptyStateTitle'>No notes for this page.</div><div class='emptyStateContent'>Notes added in Axure RP will appear here.</div><div class='dottedDivider'></div></div>";
