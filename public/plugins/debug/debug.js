...
(function () {
  ...
  $(document).ready(function () {
    ...
    var processMessages = function (message, data) {
      if (message == "openPlugin") {
        ...
        for (var key in data) {
          ...
          $("#variablesDiv").append(
            '<div class="variableList"><div class="variableName">' +
              key +
              '</div><div class="variableValue">' +
              value +
              "</div></div>"
          );
        }
        ...
        addToStack +=
          ...
          ")</div>";
        addToStack +=
          "    <div class='axEvent'>" + data.event.description + "</div>";
        addToStack += "</div></div>";
        ...
        if (currentStack.length == 0) {
          for (var i = finishedStack.length - 1; i >= 0; i--) {
            if ($("#traceDiv").children().length > 99)
              $("#traceDiv").children().last().remove();
        ...
        if (data.description) {
          addToStack +=
            "    <div class='axCaseDescription' title='" +
            data.description +
            "'>" +
            data.description +
            "</div>";
        ...
        if (data.description) {
          addToStack +=
            "    <div class='axInfoDescription' title='" +
    ...
    function handleNoCondition() {
      ...
      if (action.length == 0) {
        event.append(
          $(
            "<div class='axActionContainer'><span>No condition met</span></div></div>"
          )
        );
      }
    }
    ...
    function stoptrace_click(event) {
      ...
      $("#traceDiv").prepend(
        '<div class="tracePausedNotification">Trace Paused<div>'
      );
  ...
  function generateDebug() {
    ...
    pageNotesUi +=
      "<div id='consoleTitle' class='pluginNameHeader'>Console</div>";

    pageNotesUi += "</div>";
    pageNotesUi += "</div>";

    pageNotesUi +=
      "<div id='variablesContainer' style='max-height:300px; overflow-y:auto'>";
    pageNotesUi +=
      "<div id='variablesTitle' class='sectionTitle'>Variables</div>";
    pageNotesUi +=
      "<a id='variablesClearLink' class='traceOption'>Reset Variables</a>";
    ...
    pageNotesUi +=
      "<span class='sectionTitle'>Trace</span><a id='traceClear' class='traceOption'>Clear Trace</a><a id='traceToggle' class='traceOption'>Stop Trace</a>";
    pageNotesUi += "</div>";
    pageNotesUi += "</div>";
    ...
    pageNotesUi +=
      "<div id='traceEmptyState'>Interactions will be recorded here as you click through the prototype.</div>";
