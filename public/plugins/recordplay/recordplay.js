...
(function () {
  ...
  var leadingZeros = function (number, digits) {
    // because this thing doesn't have string.format (or does it?)
  ...
  var isBulkMouse = function (eventType) {
    return eventType === "OnMouseHover" || eventType === "OnMouseMove";
  };
    ...
    //   $axure.recording.stopRecord();
      ...
      for (var eventListIdx in eventList) {
        ...
        if (eventListItem.eventType === "OnDrag") {
          var lastDrag =
            eventListItem.eventInfo.mousePositions[
              eventListItem.eventInfo.mousePositions.length - 1
            ].dragInfo;
          ...
          eventListItem.eventInfo.dragInfo.xDelta =
            eventListItem.eventInfo.dragInfo.currentX -
            eventListItem.eventInfo.dragInfo.lastX;
          eventListItem.eventInfo.dragInfo.yDelta =
            eventListItem.eventInfo.dragInfo.currentY -
            eventListItem.eventInfo.dragInfo.lastY;
          transmissionFields = {};
          transmissionFields = tackItOn(transmissionFields, eventListItem, [
            "eventType",
            "elementID",
            "path",
          ]);
          transmissionFields = tackItOn(
            transmissionFields,
  ...
  var _attachEventTriggers = function (axRecording) {
    for (var eventIdx in axRecording.eventList) {
      ...
      $("#" + eventID).click(
        _triggerEvent(axRecording["recordingId"], eventObject.timeStamp)
      );
  ...
  var _formAxRecordingBranch = function (axRecording) {
    ...
    recordPlayUi +=
      '<a class="recordingPlusMinusLink"><span class="recordingMinus"></span></a>';
    recordPlayUi += '<a class="recordingPageLink" nodeurl="home.html">';
    recordPlayUi += '<span class="recordingPageIcon"></span>';
    recordPlayUi +=
      '<span class="recordingPageName">' +
      axRecording["recordingName"] +
      "</span>";
    recordPlayUi += "</a>";
    ...
    for (var axRecordingIdx in axRecordingList) {
      if (axRecordingList[axRecordingIdx].recordingId === axRecording) {
        for (var eventIdx in axRecordingList[axRecordingIdx].eventList) {
          if (
            axRecordingList[axRecordingIdx].eventList[eventIdx].timeStamp ===
            timeStamp
          ) {
            ...
            if (
              lowerEventType === "onclick" ||
              lowerEventType === "onmousein"
            ) {
              thisEventInfo = {};
              thisEventInfo = tackItOn(thisEventInfo, thisEvent.eventInfo, [
                "cursor",
                "timeStamp",
                "srcElement",
              ]);
  ...
  var _generateRecordPlay = function () {
    ...
    recordPlayUi +=
      "<a id='recordButton' title='Start a Recording' class='recordPlayButton'></a>";
    recordPlayUi +=
      "<a id='playButton' title='Play Back a Recording' class='recordPlayButton'></a>";
    recordPlayUi +=
      "<a id='stopButton' title='Stop' class='recordPlayButton'></a>";
    recordPlayUi +=
      "<a id='deleteButton' title='Delete All Recordings' class='recordPlayButton'></a>";
    recordPlayUi += "</div>";

    recordPlayUi +=
      "<div id='recordingContainer'><li class='recordingNode recordingRootNode'></li></div>";
    recordPlayUi += "</div>";
