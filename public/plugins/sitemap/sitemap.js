...
var openNextPage = ($axure.player.openNextPage = function () {
  ...
  $('.sitemapPageLink[nodeUrl="' + nextNodeUrl + '"]')
    .parent()
    .mousedown();
});
...
(function () {
  ...
  $(window.document).ready(function () {
    ...
    $axure.page.bind("load.sitemap", function () {
      ...
      currentNodeUrl = decodedPageLoc.substr(
        decodedPageLoc.lastIndexOf("/")
          ? decodedPageLoc.lastIndexOf("/") + 1
          : 0
      );
      currentPlayerLoc = $(location).attr("href").split("#")[0].split("?")[0];
      currentPageHashString =
        "#p=" + currentNodeUrl.substr(0, currentNodeUrl.lastIndexOf("."));

      $axure.player.setVarInCurrentUrlHash(
        PAGE_ID_NAME,
        $axure.player.getPageIdByUrl(currentNodeUrl)
      );
      $axure.player.setVarInCurrentUrlHash(
        PAGE_URL_NAME,
        currentNodeUrl.substring(0, currentNodeUrl.lastIndexOf(".html"))
      );

      $("#sitemapTreeContainer")
        .find(".sitemapHighlight")
        .removeClass("sitemapHighlight");
      var $currentNode = $(
        '.sitemapPageLink[nodeUrl="' + currentNodeUrl + '"]'
      );
      ...
      if ($currentNode.length > 0) {
        var expandableParents = $currentNode
          .closest(".sitemapNode")
          .parents(".sitemapExpandableNode");
      ...
      var hiVal = $axure.player.getHashStringVar(
        HIGHLIGHT_INTERACTIVE_VAR_NAME
      );
      if (hiVal.length > 0 && hiVal == 1) {
        $("#showHotspotsOption")
          .find(".overflowOptionCheckbox")
          .addClass("selected");
        if ($("#projectOptionsHotspotsCheckbox").length > 0)
          $("#projectOptionsHotspotsCheckbox").addClass("selected");
      ...
      if (viewStr.length > 0) {
        ...
        var $viewOption = $(".selectedRadioButton").parents(
          ".adaptiveViewOption"
        );
        $viewOption.click();
      }
      ...
      if (!$axure.player.isViewOverridden())
    ...
    if ($axure.player.zoomValues) {
      ...
      $.each($axure.player.zoomValues, function (index, value) {
        zoomValues +=
          '<div class="vpZoomValue" val=' +
          value +
          ' ><div class="zoomValue"></div>' +
          value +
          "%</div>";
      });
    ...
    var scaleOptions =
      '<div class="vpScaleOption" val="0"><div class="scaleRadioButton"><div class="selectedRadioButtonFill"></div></div>Default Scale</div>';
    scaleOptions +=
      '<div class="vpScaleOption" val="1"><div class="scaleRadioButton"><div class="selectedRadioButtonFill"></div></div>Scale to Width</div>';
    scaleOptions +=
      '<div class="vpScaleOption" val="2"><div class="scaleRadioButton"><div class="selectedRadioButtonFill"></div></div>Scale to Fit</div>';
    scaleOptions +=
      '<div class="vpScaleOption" val="3" hidden><div class="scaleRadioButton"><div class="selectedRadioButtonFill"></div></div>User Scale</div>';
    $(scaleOptions).appendTo($vpContainer);
    $("#scaleMenuContainer").append($vpContainer);
    $vpContainer.show();

    $("#overflowMenuContainer").append(
      '<div id="showHotspotsOption" class="showOption" style="order: 1"><div class="overflowOptionCheckbox"></div>Show Hotspots</div>'
    );
    ...
    if (MOBILE_DEVICE) {
      var scaleOptions =
        '<div class="projectOptionsScaleRow" val="1"><div class="scaleRadioButton"><div class="selectedRadioButtonFill"></div></div>Scale to fit width</div>';
      scaleOptions +=
        '<div class="projectOptionsScaleRow" val="0"><div class="scaleRadioButton"><div class="selectedRadioButtonFill"></div></div>Original size (100%)</div>';
      scaleOptions +=
        '<div class="projectOptionsScaleRow" val="2" style="border-bottom: solid 1px #c7c7c7"><div class="scaleRadioButton"><div class="selectedRadioButtonFill"></div></div>Fit all to screen</div>';
  ...
  function generateAdaptiveViews(forProjectOptions) {
    var $container = forProjectOptions
      ? $("#projectOptionsAdaptiveViewsContainer")
      : $("#interfaceAdaptiveViewsListContainer");
    var $viewSelect = forProjectOptions
      ? $("#projectOptionsViewSelect")
      : $("#viewSelect");
    var adaptiveViewOptionClass = forProjectOptions
      ? "projectOptionsAdaptiveViewRow"
      : "adaptiveViewOption";
    ...
    var viewsList =
      '<div class="' +
      adaptiveViewOptionClass +
      '" val="auto"><div class="adapViewRadioButton selectedRadioButton"><div class="selectedRadioButtonFill"></div></div>Adaptive</div>';
    ...
    if (typeof $axure.page.defaultAdaptiveView.name != "undefined") {
      ...
      var viewString =
        defaultViewName + " (" + widthString + " x " + heightString + ")";
      ...
      viewsList +=
        ...
        " " +
        currentViewClass +
        '" val="default"data-dim="' +
        defaultView.size.width +
        "x" +
        defaultView.size.height +
        '">' +
    ...
    if (useViews) {
      for (
        var viewIndex = 0;
        viewIndex < $axure.page.adaptiveViews.length;
        viewIndex++
      ) {
        ...
        var viewString =
          currView.name + " (" + widthString + " x " + heightString + ")";
        viewsList +=
          '<div class="' +
          adaptiveViewOptionClass +
          (forProjectOptions &&
          viewIndex == $axure.page.adaptiveViews.length - 1
            ? '" style="border-bottom: solid 1px #c7c7c7; margin-bottom: 15px;'
            : "") +
          '" val="' +
          currView.id +
          '"  data-dim="' +
          ...
          "</div>";
        viewSelect +=
          '<option value="' + currView.id + '">' + viewString + "</option>";
  ...
  function collapse_click(event) {
    if ($(this).children(".sitemapPlus").length > 0) {
      ...
      $(this)
        .children(".sitemapMinus")
        .removeClass("sitemapMinus")
        .addClass("sitemapPlus")
        .end()
        .closest("li")
        .children("ul")
        .hide(SHOW_HIDE_ANIMATION_DURATION);
    }
    event.stopPropagation();
  }
  ...
  function expand_click($this) {
    $this
      .children(".sitemapPlus")
      .removeClass("sitemapPlus")
      .addClass("sitemapMinus")
      .end()
      .closest("li")
      .children("ul")
      .show(SHOW_HIDE_ANIMATION_DURATION);
  }
  ...
  function node_click(event) {
    ...
    $axure.page.navigate(
      $(this).children(".sitemapPageLink")[0].getAttribute("nodeUrl"),
      true
    );
  }
  ...
  $axure.messageCenter.addMessageListener(function (message, data) {
    if (message == "adaptiveViewChange") {
      ...
      if (data.viewId) {
        $('.adaptiveViewOption[val="' + data.viewId + '"]').addClass(
          "currentAdaptiveView"
        );
      } else
        $('.adaptiveViewOption[val="default"]').addClass("currentAdaptiveView");
      ...
      if (data.forceSwitchTo) {
        ...
        $('div[val="' + data.forceSwitchTo + '"]')
          .find(".adapViewRadioButton")
          .addClass("selectedRadioButton");
        $('div[val="' + data.forceSwitchTo + '"]')
          .find(".selectedRadioButtonFill")
          .show();
      }
  ...
  $axure.player.toggleHotspots = function (val) {
    var overflowMenuCheckbox = $("#showHotspotsOption").find(
  ...
  function showHotspots_click(event) {
    ...
    if ($(overflowMenuCheckbox).hasClass("selected")) {
      ...
      if (projOptionsCheckbox.length > 0)
        projOptionsCheckbox.removeClass("selected");
      ...
      if (projOptionsCheckbox.length > 0)
        projOptionsCheckbox.addClass("selected");
      $axure.messageCenter.postMessage("highlightInteractive", true);
  ...
  function adaptiveViewOption_click(event) {
    ...
    $(".adaptiveViewOption").removeClass("currentAdaptiveView");
    if (currVal) {
      $('.adaptiveViewOption[val="' + currVal + '"]').addClass(
        "currentAdaptiveView"
      );
    } else
      $('.adaptiveViewOption[val="default"]').addClass("currentAdaptiveView");
  ...
  var selectAdaptiveView = ($axure.player.selectAdaptiveView = function (
    currVal
  ) {
    if (currVal == "auto") {
      $axure.messageCenter.postMessage("setAdaptiveViewForSize", {
      ...
      currentPageLoc = $axure.page.location.split("#")[0];
      var decodedPageLoc = decodeURI(currentPageLoc);
      var nodeUrl = decodedPageLoc.substr(
        decodedPageLoc.lastIndexOf("/")
          ? decodedPageLoc.lastIndexOf("/") + 1
          : 0
      );
  ...
  $axure.player.updateAdaptiveViewHeader = updateAdaptiveViewHeader =
    function () {
      var hasDefinedDim = true;
      var dimensionlessViewStr = "(any x any)";

      var viewString = $(".adaptiveViewOption.currentAdaptiveView").text();
      if (viewString != null && viewString.indexOf(dimensionlessViewStr) >= 0)
        hasDefinedDim = false;
  ...
  function vpScaleOption_click(event) {
    ...
    var $selectedScaleOption = $(
      '.vpScaleOption[val="' +
        scaleVal +
        '"], .projectOptionsScaleRow[val="' +
        scaleVal +
        '"]'
    );
    var $allScaleOptions = $(".vpScaleOption, .projectOptionsScaleRow");
    $allScaleOptions
      .find(".scaleRadioButton")
      .removeClass("selectedRadioButton");
  ...
  function search_input_keyup(event) {
    ...
    if (searchVal == "") {
      ...
      $(".sitemapPageName")
        .addClass("sitemapGreyedName")
        .each(function () {
          var nodeName = $(this).text().toLowerCase();
          if (nodeName.indexOf(searchVal) != -1) {
            $(this)
              .removeClass("sitemapGreyedName")
              .parents(".sitemapNode:first")
              .show()
              .parents(".sitemapExpandableNode")
              .show();
          }
  ...
  function generateSitemap() {
    ...
    treeUl +=
      '<div id="searchDiv"><span id="searchIcon" class="sitemapToolbarButton"></span><input id="searchBox" type="text"/></div>';
    ...
    treeUl +=
      "<div class='sitemapPluginNameHeader pluginNameHeader'>" +
      sitemapTitle +
      "</div>";
    ...
    if (!MOBILE_DEVICE) {
      treeUl +=
        "<div id='changePageInstructions' class='pageSwapInstructions'>Use  ";
  ...
  function generateNode(node, level) {
    ...
    if (hasChildren) {
      ...
      if (isFolder) {
        returnVal =
          "<li class='sitemapNode sitemapExpandableNode'><div class='sitemapHover'><div class='sitemapPageLinkContainer sitemapPlusMinusLink' style='margin-left:" +
          margin +
          "px'><span class='sitemapPlus'></span>";
      } else {
        returnVal =
          "<li class='sitemapNode sitemapExpandableNode'><div class='sitemapHover'><div class='sitemapPageLinkContainer' style='margin-left:" +
          margin +
          "px'><a class='sitemapPlusMinusLink'><span class='sitemapMinus'></span></a>";
      }
    } else {
      margin = 19 + level * 17;
      returnVal =
        "<li class='sitemapNode sitemapLeafNode'><div class='sitemapHover'><div class='sitemapPageLinkContainer' style='margin-left:" +
        margin +
        "px'>";
    }
